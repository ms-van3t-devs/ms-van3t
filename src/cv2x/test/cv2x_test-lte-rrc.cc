...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteRrcTest");
 ...
 */ 
class cv2x_LteRrcConnectionEstablishmentTestCase : public TestCase
{
public:
  /**
   *
   ...
   */
  cv2x_LteRrcConnectionEstablishmentTestCase (uint32_t nUes,
                                         uint32_t nBearers,
                                         uint32_t tConnBase,
...
std::string
cv2x_LteRrcConnectionEstablishmentTestCase::BuildNameString (uint32_t nUes,
                                                        uint32_t nBearers,
                                                        uint32_t tConnBase,
                                                        uint32_t tConnIncrPerUe,
                                                        uint32_t delayDiscStart,
...
cv2x_LteRrcConnectionEstablishmentTestCase::cv2x_LteRrcConnectionEstablishmentTestCase (
...
void
cv2x_LteRrcConnectionEstablishmentTestCase::DoRun ()
{
  NS_LOG_FUNCTION (this << GetName ());
  Config::Reset ();
  ...
  if (m_nUes < 25)
    {
      Config::SetDefault ("ns3::cv2x_LteEnbRrc::SrsPeriodicity", UintegerValue (40));
    }
  else if (m_nUes < 60)
    {
    ...
    {
      Config::SetDefault ("ns3::cv2x_LteEnbRrc::SrsPeriodicity", UintegerValue (160));
    }
  else
    {
      Config::SetDefault ("ns3::cv2x_LteEnbRrc::SrsPeriodicity", UintegerValue (320));
    }
  ...
  for (NetDeviceContainer::Iterator it = ueDevs.Begin (); it != ueDevs.End (); ++it)
    {
      ...
      //void (cv2x_LteHelper::* overloadedAttachFunctionPointer) (Ptr<NetDevice>, Ptr<NetDevice>) = &cv2x_LteHelper::Attach;
      //Simulator::Schedule (MilliSeconds (tc), overloadedAttachFunctionPointer, lteHelper, *it, enbDevice);
      Simulator::Schedule (MilliSeconds (tc), &cv2x_LteRrcConnectionEstablishmentTestCase::Connect, this, ueDevice, enbDevice);

      Simulator::Schedule (MilliSeconds (tcc), &cv2x_LteRrcConnectionEstablishmentTestCase::CheckConnected, this, *it, enbDevice);
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/cv2x_LteUeRrc/ConnectionEstablished",
                   MakeCallback (&cv2x_LteRrcConnectionEstablishmentTestCase::ConnectionEstablishedCallback,
                                 this));
  Config::Connect ("/NodeList/*/DeviceList/*/cv2x_LteUeRrc/ConnectionTimeout",
                   MakeCallback (&cv2x_LteRrcConnectionEstablishmentTestCase::ConnectionTimeoutCallback,
                                 this));
...
void
cv2x_LteRrcConnectionEstablishmentTestCase::Connect (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
...
void
cv2x_LteRrcConnectionEstablishmentTestCase::CheckConnected (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  Ptr<cv2x_LteUeNetDevice> ueLteDevice = ueDevice->GetObject<cv2x_LteUeNetDevice> ();
  ...
  NS_TEST_ASSERT_MSG_EQ (ueRrc->GetState (), cv2x_LteUeRrc::CONNECTED_NORMALLY,
  ...
  if (hasContext)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (ueManager->GetState (),
                             cv2x_UeManager::CONNECTED_NORMALLY,
...
void
cv2x_LteRrcConnectionEstablishmentTestCase::CheckNotConnected (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  Ptr<cv2x_LteUeNetDevice> ueLteDevice = ueDevice->GetObject<cv2x_LteUeNetDevice> ();
  Ptr<cv2x_LteUeRrc> ueRrc = ueLteDevice->GetRrc ();
  const uint64_t imsi = ueLteDevice->GetImsi ();
  ...
  bool ueStateIsConnectedNormally = (cv2x_LteUeRrc::CONNECTED_NORMALLY == ueRrc->GetState ());
  ...
  if (hasContext)
    {
      Ptr<cv2x_UeManager> ueManager = enbRrc->GetUeManager (rnti);
      NS_ASSERT (ueManager != 0);
      contextStateIsConnectedNormally = (cv2x_UeManager::CONNECTED_NORMALLY == ueManager->GetState ());
    }
...
void
cv2x_LteRrcConnectionEstablishmentTestCase::ConnectionEstablishedCallback (
...
void
cv2x_LteRrcConnectionEstablishmentTestCase::ConnectionTimeoutCallback (
    std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti)
{
  NS_LOG_FUNCTION (this << imsi << cellId);
}
 ...
 */ 
class cv2x_LteRrcConnectionEstablishmentErrorTestCase
  : public cv2x_LteRrcConnectionEstablishmentTestCase
{
public:
  /**
   *
   ...
   * \param description additional description of the test case
   */
  cv2x_LteRrcConnectionEstablishmentErrorTestCase (Time jumpAwayTime,
                                              std::string description = "");
protected:
  virtual void DoRun (void);
...
cv2x_LteRrcConnectionEstablishmentErrorTestCase::cv2x_LteRrcConnectionEstablishmentErrorTestCase (
  Time jumpAwayTime, std::string description)
  : cv2x_LteRrcConnectionEstablishmentTestCase (1, 1, 0, 0, 1, true, false, true,
                                           description),
...
void
cv2x_LteRrcConnectionEstablishmentErrorTestCase::DoRun ()
{
  NS_LOG_FUNCTION (this << GetName ());
  Config::Reset ();
  ...
  for (NetDeviceContainer::Iterator it = ueDevs.Begin (); it != ueDevs.End (); ++it)
    {
      ...
      //void (cv2x_LteHelper::* overloadedAttachFunctionPointer) (Ptr<NetDevice>, Ptr<NetDevice>) = &cv2x_LteHelper::Attach;
      //Simulator::Schedule (MilliSeconds (tc), overloadedAttachFunctionPointer, lteHelper, *it, enbDevice);
      Simulator::Schedule (MilliSeconds (tc), &cv2x_LteRrcConnectionEstablishmentErrorTestCase::Connect, this, ueDevice, enbDevice);

      Simulator::Schedule (MilliSeconds (tcc), &cv2x_LteRrcConnectionEstablishmentErrorTestCase::CheckConnected, this, *it, enbDevice);
  ...
  // Connect to trace sources in UEs
  Config::Connect ("/NodeList/*/DeviceList/*/cv2x_LteUeRrc/ConnectionEstablished",
                   MakeCallback (&cv2x_LteRrcConnectionEstablishmentErrorTestCase::ConnectionEstablishedCallback,
                                 this));
  Config::Connect ("/NodeList/*/DeviceList/*/cv2x_LteUeRrc/ConnectionTimeout",
                   MakeCallback (&cv2x_LteRrcConnectionEstablishmentErrorTestCase::ConnectionTimeoutCallback,
                                 this));


  Simulator::Schedule (m_jumpAwayTime,
                       &cv2x_LteRrcConnectionEstablishmentErrorTestCase::JumpAway,
                       this);
  Simulator::Schedule (m_jumpAwayTime + MilliSeconds (99),
                       &cv2x_LteRrcConnectionEstablishmentErrorTestCase::CheckNotConnected,
                       this, ueDevs.Get (0), enbDevs.Get (0));
  Simulator::Schedule (m_jumpAwayTime + MilliSeconds (100),
                       &cv2x_LteRrcConnectionEstablishmentErrorTestCase::JumpBack,
                       this);
 ...
 */ 
class cv2x_LteRrcTestSuite : public TestSuite
{
public:
  cv2x_LteRrcTestSuite ();
};


cv2x_LteRrcTestSuite::cv2x_LteRrcTestSuite ()
  : TestSuite ("lte-rrc", SYSTEM)
{
//  LogComponentEnableAll (LOG_PREFIX_ALL);
//  LogComponentEnable ("LteRrcTest", LOG_LEVEL_ALL);
//  LogComponentEnable ("cv2x_LteEnbRrc", LOG_INFO);
//  LogComponentEnable ("cv2x_LteUeRrc", LOG_INFO);
  ...
  for (uint32_t useIdealRrc = 0; useIdealRrc <= 1; ++useIdealRrc)
    {
      ...
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  1,     0,      0,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  1,     0,    100,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  1,     1,      0,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  1,     1,    100,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  1,     2,      0,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  1,     2,    100,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     0,     20,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     0,     20,          10,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     0,     20,         100,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     1,     20,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     1,     20,          10,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     1,     20,         100,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     2,     20,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     2,     20,          10,           1, false, useIdealRrc, true), TestCase::QUICK);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     2,     20,         100,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  3,     0,     20,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  4,     0,     20,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  4,     0,     20,         300,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase ( 20,     0,     10,           1,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase ( 50,     0,      0,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);

      // Test cases to check admitRrcConnectionRequest=false
      //                                                     nUes      tConnBase            delayDiscStart         useIdealRrc
      //                                                        nBearers       tConnIncrPerUe       errorExpected               admitRrcConnectionRequest
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  1,     0,      0,           0,           1, false, useIdealRrc, false), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  1,     2,    100,           0,           1, false, useIdealRrc, false), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     0,     20,           0,           1, false, useIdealRrc, false), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  2,     1,     20,           0,           1, false, useIdealRrc, false), TestCase::QUICK);
      AddTestCase (new cv2x_LteRrcConnectionEstablishmentTestCase (  3,     0,     20,           0,           1, false, useIdealRrc, false), TestCase::EXTENSIVE);
    }

  // Test cases with transmission error
  AddTestCase (new cv2x_LteRrcConnectionEstablishmentErrorTestCase (
                   Seconds (0.020214),
...
static cv2x_LteRrcTestSuite g_lteRrcTestSuiteInstance;
