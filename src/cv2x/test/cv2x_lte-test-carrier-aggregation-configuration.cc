 ...
 * done in cv2x_LteHelper for the sake of creating PHY and MAC instances equal to the number
...
NS_LOG_COMPONENT_DEFINE ("cv2x_TestCarrierAggregationConfig");

class cv2x_CarrierAggregationConfigTestCase : public TestCase
{
public:
  /**
   * Constructor
   ...
   */
  cv2x_CarrierAggregationConfigTestCase (uint32_t numberOfNodes, uint16_t numberOfComponentCarriers, std::vector<cv2x_ConfigToCheck> configToCheck, Time simulationDuration)
...
private:
  ...
  std::string BuildNameString (uint32_t numberOfNodes, uint16_t numberOfComponentCarriers, std::vector<cv2x_ConfigToCheck> configToCheck, Time simulationDuration);
  void Evaluate (std::string context, Ptr<cv2x_LteUeRrc> ueRrc, std::list<cv2x_LteRrcSap::SCellToAddMod> sCellToAddModList);
  std::vector<std::map<uint16_t, cv2x_ConfigToCheck> > EquallySpacedCcs ();

  uint32_t m_numberOfNodes;
  uint16_t m_numberOfComponentCarriers;
  std::vector<cv2x_ConfigToCheck> m_configToCheck;
  uint32_t m_connectionCounter;
  Time m_simulationDuration;
  std::vector<std::map<uint16_t, cv2x_ConfigToCheck> > m_configToCheckContainer;
};

std::string
cv2x_CarrierAggregationConfigTestCase::BuildNameString (uint32_t numberOfNodes, uint16_t numberOfComponentCarriers, std::vector<cv2x_ConfigToCheck> configToCheck, Time simulationDuration)
{
...
std::vector<std::map<uint16_t, cv2x_ConfigToCheck> >
cv2x_CarrierAggregationConfigTestCase::EquallySpacedCcs ()
{
  std::vector<std::map<uint16_t, cv2x_ConfigToCheck> > configToCheckContainer;

  for (auto &it: m_configToCheck)
    {
      std::map<uint16_t, cv2x_ConfigToCheck> ccmap;
...
void
cv2x_CarrierAggregationConfigTestCase::Evaluate (std::string context, Ptr<cv2x_LteUeRrc> ueRrc, std::list<cv2x_LteRrcSap::SCellToAddMod> sCellToAddModList)
{
  ...
  std::map<uint16_t, cv2x_ConfigToCheck> configToCheckMap;
  ...
  cv2x_ConfigToCheck pCConfig = configToCheckMap[0]; // Primary Carrier
  cv2x_ConfigToCheck sCConfig; // Secondary Carriers
  ...
  uint32_t cv2x_ConfigToCheckMapIndex = 1;

  for (auto scell: sCellToAddModList)
    {
      sCConfig = configToCheckMap[cv2x_ConfigToCheckMapIndex];
      ...
      cv2x_ConfigToCheckMapIndex++;
    }
}

void
cv2x_CarrierAggregationConfigTestCase::DoRun ()
{
  ...
  cv2x_ConfigToCheck configuration;
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/cv2x_LteUeRrc/SCarrierConfigured",
                   MakeCallback (&cv2x_CarrierAggregationConfigTestCase::Evaluate, this));
...
class cv2x_CarrierAggregationConfigTestSuite : public TestSuite
{
public:
  cv2x_CarrierAggregationConfigTestSuite ();
};

cv2x_CarrierAggregationConfigTestSuite::cv2x_CarrierAggregationConfigTestSuite ()
  : TestSuite ("lte-carrier-aggregation-configuration", SYSTEM)
{
  std::vector<cv2x_ConfigToCheck> configToCheck;

  // Test1 with 1 eNB and 1 UE.
  // We put a configuration different than the default configuration done in cv2x_LteHelper for the sake of
  // creating PHY and MAC instances equal to the number of component carriers.

  cv2x_ConfigToCheck configToCheckTest1;
  ...
  AddTestCase (new cv2x_CarrierAggregationConfigTestCase (numberOfNodes, numberOfComponentCarriers, configToCheck, simulationDuration), TestCase::QUICK);
...
//   configToCheck.erase(configToCheck.begin(), configToCheck.end());
  ...
  cv2x_ConfigToCheck configToCheckTest2;
...
static cv2x_CarrierAggregationConfigTestSuite g_carrierAggregationConfigTestSuite;
