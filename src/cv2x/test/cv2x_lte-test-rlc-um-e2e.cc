...
#include "cv2x_lte-test-rlc-um-e2e.h"


using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("cv2x_LteRlcUmE2eTest");
...
cv2x_LteRlcUmE2eTestSuite::cv2x_LteRlcUmE2eTestSuite ()
  : TestSuite ("lte-rlc-um-e2e", SYSTEM)
{
  // NS_LOG_INFO ("Creating cv2x_LteRlcUmE2eTestSuite");
      ...
      for ( uint32_t s = 0 ; s < (sizeof (seeds) / sizeof (uint32_t)) ; s++ )
        {
          ...
          AddTestCase (new cv2x_LteRlcUmE2eTestCase (name.str (), seeds[s], losses[l]), testDuration);
        }
    }
}

static cv2x_LteRlcUmE2eTestSuite lteRlcUmE2eTestSuite;
...
cv2x_LteRlcUmE2eTestCase::cv2x_LteRlcUmE2eTestCase (std::string name, uint32_t seed, double losses)
  : TestCase (name)
{
  // NS_LOG_UNCOND ("Creating cv2x_LteRlcUmTestingTestCase: " + name);
...
cv2x_LteRlcUmE2eTestCase::~cv2x_LteRlcUmE2eTestCase ()
{
}


void
cv2x_LteRlcUmE2eTestCase::DlDropEvent (Ptr<const Packet> p)
{
...
void
cv2x_LteRlcUmE2eTestCase::UlDropEvent (Ptr<const Packet> p)
{
  // NS_LOG_FUNCTION (this);
  m_ulDrops++;
}
...
cv2x_LteRlcUmE2eTestCase::DoRun (void)
{
  ...
  ueLteDevs.Get (0)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&cv2x_LteRlcUmE2eTestCase::DlDropEvent, this));
  enbLteDevs.Get (0)->SetAttribute ("ReceiveErrorModel", PointerValue (ulEm));
  enbLteDevs.Get (0)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&cv2x_LteRlcUmE2eTestCase::UlDropEvent, this));
  ...
  lteSimpleHelper->m_enbMac->SetTxOpportunityMode (cv2x_LteTestMac::RANDOM_MODE);
  ...
  lteSimpleHelper->m_ueMac->SetTxOpportunityMode (cv2x_LteTestMac::RANDOM_MODE);

  // Start/Stop pseudo-application at eNB RRC
  Simulator::Schedule (Seconds (0.100), &cv2x_LteTestRrc::Start, lteSimpleHelper->m_enbRrc);
  Simulator::Schedule (Seconds (10.100), &cv2x_LteTestRrc::Stop, lteSimpleHelper->m_enbRrc);

  // Start/Stop pseudo-application at UE RRC
  Simulator::Schedule (Seconds (20.100), &cv2x_LteTestRrc::Start, lteSimpleHelper->m_ueRrc);
  Simulator::Schedule (Seconds (30.100), &cv2x_LteTestRrc::Stop, lteSimpleHelper->m_ueRrc);
