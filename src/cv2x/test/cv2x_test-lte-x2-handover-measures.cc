...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteX2HandoverMeasuresTest");

/**
 * \ingroup lte-test
 * \ingroup tests
 *
 * \brief cv2x_CheckPointEvent structure
 */
struct cv2x_CheckPointEvent
{
   ...
   */ 
  cv2x_CheckPointEvent (Time start, Time stop, Time interval, uint32_t ueIndex, uint32_t enbIndex)
 ...
 * \ingroup tests
 *
 * \brief Test different X2 handover measures and algorithms, e.g. cv2x_A2A4RsrqHandoverAlgorithm and 
 * cv2x_A3RsrpHandoverAlgorithm. Test defines different handover parameters and scenario configurations.
 */
class cv2x_LteX2HandoverMeasuresTestCase : public TestCase
{
public:
  /**
   *
   ...
   * \param schedulerType type of scheduler to be used (e.g. "ns3::cv2x_PfFfMacScheduler")
   * \param handoverAlgorithmType type of handover algorithm to be used (e.g. "ns3::cv2x_A3RsrpHandoverAlgorithm")
   * \param admitHo
   * \param useIdealRrc true if ideal RRC is to be used, false if real RRC is to be used
   *
   * \return
   */
  cv2x_LteX2HandoverMeasuresTestCase (uint32_t nEnbs, uint32_t nUes, uint32_t nDedicatedBearers,
                                 std::list<cv2x_CheckPointEvent> checkPointEventList,
...
private:
  ...
  std::list<cv2x_CheckPointEvent> m_checkPointEventList; ///< check point event list
...
std::string
cv2x_LteX2HandoverMeasuresTestCase::BuildNameString (uint32_t nEnbs, uint32_t nUes, uint32_t nDedicatedBearers,
                                                std::string checkPointEventListName,
                                                bool useUdp, std::string schedulerType,
                                                std::string handoverAlgorithmType, bool admitHo,
                                                bool useIdealRrc)
...
cv2x_LteX2HandoverMeasuresTestCase::cv2x_LteX2HandoverMeasuresTestCase (uint32_t nEnbs, uint32_t nUes, uint32_t nDedicatedBearers,
                                                              std::list<cv2x_CheckPointEvent> checkPointEventList,
                                                              std::string checkPointEventListName,
                                                              bool useUdp, std::string schedulerType,
                                                              std::string handoverAlgorithmType, bool admitHo,
...
void
cv2x_LteX2HandoverMeasuresTestCase::DoRun ()
{
  ...
  Config::SetDefault ("ns3::cv2x_LteEnbRrc::HandoverJoiningTimeoutDuration", TimeValue (MilliSeconds (200)));
  Config::SetDefault ("ns3::cv2x_LteEnbPhy::TxPower", DoubleValue (20));
  ...
  if (m_handoverAlgorithmType == "ns3::cv2x_A2A4RsrqHandoverAlgorithm")
    {
      m_lteHelper->SetHandoverAlgorithmType ("ns3::cv2x_A2A4RsrqHandoverAlgorithm");
  ...
  else if (m_handoverAlgorithmType == "ns3::cv2x_A3RsrpHandoverAlgorithm")
    {
      m_lteHelper->SetHandoverAlgorithmType ("ns3::cv2x_A3RsrpHandoverAlgorithm");
  ...
  for (NetDeviceContainer::Iterator it = ueDevices.Begin (); it != ueDevices.End (); ++it)
    {
      ...
      Simulator::Schedule (maxRrcConnectionEstablishmentDuration,
                           &cv2x_LteX2HandoverMeasuresTestCase::CheckConnected,
  ...
  for (std::list<cv2x_CheckPointEvent>::iterator checkPointEventIt = m_checkPointEventList.begin ();
       ...
       ++checkPointEventIt)
    {
      for (Time checkPointTime = checkPointEventIt->checkStartTime;
        ...
        {
          Simulator::Schedule (checkPointTime, &cv2x_LteX2HandoverMeasuresTestCase::CheckConnected,
          ...
          Simulator::Schedule (saveStatsTime, &cv2x_LteX2HandoverMeasuresTestCase::SaveStats,
                               this, checkPointEventIt->ueDeviceIndex);

          Time checkStats = saveStatsTime + m_statsDuration;
          Simulator::Schedule (checkStats, &cv2x_LteX2HandoverMeasuresTestCase::CheckStats,
                               this, checkPointEventIt->ueDeviceIndex);
...
void
cv2x_LteX2HandoverMeasuresTestCase::CheckConnected (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
...
void
cv2x_LteX2HandoverMeasuresTestCase::SaveStats (uint32_t ueIndex)
{
...
void
cv2x_LteX2HandoverMeasuresTestCase::CheckStats (uint32_t ueIndex)
{
  NS_LOG_FUNCTION (ueIndex);
  uint32_t b = 1;
  for (std::list<BearerData>::iterator it = m_ueDataVector.at (ueIndex).bearerDataList.begin ();
 ...
 */
class cv2x_LteX2HandoverMeasuresTestSuite : public TestSuite
{
public:
  cv2x_LteX2HandoverMeasuresTestSuite ();
};


cv2x_LteX2HandoverMeasuresTestSuite::cv2x_LteX2HandoverMeasuresTestSuite ()
  : TestSuite ("lte-x2-handover-measures", SYSTEM)
{
  Time checkInterval = Seconds (1);

  std::string cel1name ("ho: 0 -> 1");
  std::list<cv2x_CheckPointEvent> cel1;
  cel1.push_back (cv2x_CheckPointEvent (Seconds (1), Seconds (10.1), checkInterval, 0, 0));
  cel1.push_back (cv2x_CheckPointEvent (Seconds (11), Seconds (17), checkInterval, 0, 1));

  std::string cel2name ("ho: 0 -> 1 -> 2");
  std::list<cv2x_CheckPointEvent> cel2;
  cel2.push_back (cv2x_CheckPointEvent (Seconds (1), Seconds (10.1), checkInterval, 0, 0));
  cel2.push_back (cv2x_CheckPointEvent (Seconds (11), Seconds (17.1), checkInterval, 0, 1));
  cel2.push_back (cv2x_CheckPointEvent (Seconds (18), Seconds (24), checkInterval, 0, 2));

  std::string cel3name ("ho: 0 -> 1 -> 2 -> 3");
  std::list<cv2x_CheckPointEvent> cel3;
  cel3.push_back (cv2x_CheckPointEvent (Seconds (1), Seconds (10.1), checkInterval,  0, 0));
  cel3.push_back (cv2x_CheckPointEvent (Seconds (11), Seconds (17.1), checkInterval, 0, 1));
  cel3.push_back (cv2x_CheckPointEvent (Seconds (18), Seconds (24.1), checkInterval, 0, 2));
  cel3.push_back (cv2x_CheckPointEvent (Seconds (25), Seconds (37), checkInterval, 0, 3));


  int32_t useIdealRrc;
  std::string sched = "ns3::cv2x_PfFfMacScheduler";
  std::string ho = "ns3::cv2x_A2A4RsrqHandoverAlgorithm";
  for (useIdealRrc = 1; useIdealRrc >= 0; --useIdealRrc)
    {
      //                                          nEnbs, nUes, nDBearers, celist, name, useUdp, sched, ho, admitHo, idealRrc
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (2,   1,    0,    cel1, cel1name, true, sched, ho, true, useIdealRrc), TestCase::TAKES_FOREVER);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (2,   1,    1,    cel1, cel1name, true, sched, ho, true, useIdealRrc), TestCase::QUICK);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (2,   1,    2,    cel1, cel1name, true, sched, ho, true, useIdealRrc), TestCase::TAKES_FOREVER);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (3,   1,    0,    cel2, cel2name, true, sched, ho, true, useIdealRrc), TestCase::TAKES_FOREVER);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (3,   1,    1,    cel2, cel2name, true, sched, ho, true, useIdealRrc), TestCase::TAKES_FOREVER);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (3,   1,    2,    cel2, cel2name, true, sched, ho, true, useIdealRrc), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (4,   1,    0,    cel3, cel3name, true, sched, ho, true, useIdealRrc), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (4,   1,    1,    cel3, cel3name, true, sched, ho, true, useIdealRrc), TestCase::TAKES_FOREVER);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (4,   1,    2,    cel3, cel3name, true, sched, ho, true, useIdealRrc), TestCase::TAKES_FOREVER);
    }

  sched = "ns3::cv2x_RrFfMacScheduler";
  for (useIdealRrc = 1; useIdealRrc >= 0; --useIdealRrc)
    {
      //                                          nEnbs, nUes, nDBearers, celist, name, useUdp, sched, admitHo, idealRrc
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (2,   1,    0,    cel1, cel1name, true, sched, ho, true, useIdealRrc), TestCase::EXTENSIVE);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (3,   1,    0,    cel2, cel2name, true, sched, ho, true, useIdealRrc), TestCase::TAKES_FOREVER);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (4,   1,    0,    cel3, cel3name, true, sched, ho, true, useIdealRrc), TestCase::TAKES_FOREVER);
    }

  ho = "ns3::cv2x_A3RsrpHandoverAlgorithm";
  sched = "ns3::cv2x_PfFfMacScheduler";
  for (useIdealRrc = 1; useIdealRrc >= 0; --useIdealRrc)
    {
      //                                          nEnbs, nUes, nDBearers, celist, name, useUdp, sched, admitHo, idealRrc
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (2,   1,    0,    cel1, cel1name, true, sched, ho, true, useIdealRrc), TestCase::EXTENSIVE);
  ...
  for (useIdealRrc = 1; useIdealRrc >= 0; --useIdealRrc)
    {
      //                                          nEnbs, nUes, nDBearers, celist, name, useUdp, sched, admitHo, idealRrc
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (2,   1,    0,    cel1, cel1name, true, sched, ho, true, useIdealRrc), TestCase::QUICK);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (3,   1,    0,    cel2, cel2name, true, sched, ho, true, useIdealRrc), TestCase::TAKES_FOREVER);
      AddTestCase (new cv2x_LteX2HandoverMeasuresTestCase (4,   1,    0,    cel3, cel3name, true, sched, ho, true, useIdealRrc), TestCase::EXTENSIVE);
    }

} // end of cv2x_LteX2HandoverMeasuresTestSuite ()

static cv2x_LteX2HandoverMeasuresTestSuite g_lteX2HandoverMeasuresTestSuiteInstance;
