...
#include "cv2x_lte-test-link-adaptation.h"


using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("cv2x_LteLinkAdaptationTest");
...
void
LteTestDlSchedulingCallback (cv2x_LteLinkAdaptationTestCase *testcase, std::string path,
...
cv2x_LteLinkAdaptationTestSuite::cv2x_LteLinkAdaptationTestSuite ()
  ...
  for ( int i = 0 ; i < numOfTests; i++ )
    {
      ...
      AddTestCase (new cv2x_LteLinkAdaptationTestCase (name.str (),  snrEfficiencyMcs[i].snrDb, lossDb, snrEfficiencyMcs[i].mcsIndex), TestCase::QUICK);
    }

}

static cv2x_LteLinkAdaptationTestSuite lteLinkAdaptationTestSuite;
...
cv2x_LteLinkAdaptationTestCase::cv2x_LteLinkAdaptationTestCase (std::string name, double snrDb, double loss, uint16_t mcsIndex)
  : TestCase (name),
    m_snrDb (snrDb),
    m_loss (loss),
    m_mcsIndex (mcsIndex)
{
  ...
  NS_LOG_INFO ("Creating cv2x_LteLinkAdaptationTestCase: " + sstream1.str ());
}

cv2x_LteLinkAdaptationTestCase::~cv2x_LteLinkAdaptationTestCase ()
{
}

void
cv2x_LteLinkAdaptationTestCase::DoRun (void)
{
  Config::Reset ();
  ...
  Config::SetDefault ("ns3::cv2x_LteEnbRrc::SrsPeriodicity", UintegerValue (2));
...
void
cv2x_LteLinkAdaptationTestCase::DlScheduling (cv2x_DlSchedulingCallbackInfo dlInfo)
{
