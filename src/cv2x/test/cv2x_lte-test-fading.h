...
#ifndef CV2X_LTE_TEST_FADING_H
#define CV2X_LTE_TEST_FADING_H
...
class cv2x_LteFadingTestSuite : public TestSuite
{
  public:
    cv2x_LteFadingTestSuite ();
};
 ...
 * (150 < freq < 1500 MHz).
 */
class cv2x_LteFadingTestCase : public TestCase
{
  public:
    /**
     ...
     */
    cv2x_LteFadingTestCase (Ptr<BuildingsMobilityModel> m1, Ptr<BuildingsMobilityModel> m2, double refValue, std::string name);
    virtual ~cv2x_LteFadingTestCase ();
  ...
  private:
    ...
    Ptr<cv2x_TraceFadingLossModel> m_fadingModule; ///< fading loss model
 ...
 */
class cv2x_LteFadingSystemTestCase : public TestCase
{
  public:
    /**
     ...
     */
    cv2x_LteFadingSystemTestCase (std::string name, double snrDb, double dist, uint16_t mcsIndex);
    cv2x_LteFadingSystemTestCase ();
    virtual ~cv2x_LteFadingSystemTestCase ();
