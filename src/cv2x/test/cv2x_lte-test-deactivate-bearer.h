...
#ifndef CV2X_LENA_TEST_DEACTIVATE_BEARER_H
#define CV2X_LENA_TEST_DEACTIVATE_BEARER_H
...
class cv2x_LenaDeactivateBearerTestCase : public TestCase
{
public:
  /**
   * Constructor
   ...
   */
  cv2x_LenaDeactivateBearerTestCase (std::vector<uint16_t> dist, std::vector<uint32_t> estThrPssDl, std::vector<uint16_t> packetSize, uint16_t interval, bool  errorModelEnabled, bool useIdealRrc);
  virtual ~cv2x_LenaDeactivateBearerTestCase ();
 ...
 * \ingroup tests
 *
 * \brief The test suite class for the cv2x_LenaDeactivateBearerTestCase.
 */

class cv2x_LenaTestBearerDeactivateSuite : public TestSuite
{
public:
  cv2x_LenaTestBearerDeactivateSuite ();
};
