...
#ifndef CV2X_LTE_SIMPLE_HELPER_H
#define CV2X_LTE_SIMPLE_HELPER_H
 ...
 * \ingroup tests
 *
 * \brief A simplified version of cv2x_LteHelper, that 
 * is used for creation and configuration of LTE entities for testing purposes 
 * when just a limited cv2x_LteHelper functionality is wanted.
 *
 */
class cv2x_LteSimpleHelper : public Object
{
public:
  cv2x_LteSimpleHelper (void);
  virtual ~cv2x_LteSimpleHelper (void);
...
public:
  ...
  Ptr<cv2x_LteTestRrc> m_enbRrc; ///< ENB RRC
  Ptr<cv2x_LteTestRrc> m_ueRrc; ///< UE RRC

  Ptr<cv2x_LteTestMac> m_enbMac; ///< ENB MAC
  Ptr<cv2x_LteTestMac> m_ueMac; ///< UE MAC

private:

  Ptr<cv2x_LtePdcp>    m_enbPdcp; ///< ENB PDCP
  Ptr<cv2x_LteRlc>     m_enbRlc; ///< ENB RLC

  Ptr<cv2x_LtePdcp>    m_uePdcp; ///< UE PDCP
  Ptr<cv2x_LteRlc>     m_ueRlc; ///< UE RLC

  ObjectFactory   m_enbDeviceFactory; ///< ENB device factory
  ObjectFactory   m_ueDeviceFactory; ///< UE device factory

  /// cv2x_LteRlcEntityType_t enumeration
  enum cv2x_LteRlcEntityType_t {RLC_UM = 1,
...
#endif // CV2X_LTE_SIMPLE_HELPER_H
