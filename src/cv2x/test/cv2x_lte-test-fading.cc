...
#include "ns3/cv2x_lte-test-ue-phy.h"
#include "ns3/cv2x_lte-chunk-processor.h"
#include "ns3/cv2x_lte-test-fading.h"
...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteFadingTest");
...
cv2x_LteFadingTestSuite::cv2x_LteFadingTestSuite ()
: TestSuite ("lte-fading-model", SYSTEM)
{
  ...
  // NS_LOG_INFO ("Creating cv2x_LteDownlinkSinrTestSuite");
...
//   double freq = 869e6; // E_UTRA BAND #5 see table 5.5-1 of 36.101
  ...
  AddTestCase (new cv2x_LteFadingTestCase (mm1, mm2, 137.93, "OH Urban Large city"), TestCase::QUICK);
    
  
}

static cv2x_LteFadingTestSuite lteFadingTestSuite;
...
cv2x_LteFadingTestCase::cv2x_LteFadingTestCase (Ptr<BuildingsMobilityModel> m1, Ptr<BuildingsMobilityModel> m2, double refValue, std::string name)
...
cv2x_LteFadingTestCase::~cv2x_LteFadingTestCase ()
{
}

void
cv2x_LteFadingTestCase::DoRun (void)
{
  //   LogLevel logLevel = (LogLevel)(LOG_PREFIX_FUNC | LOG_PREFIX_TIME | LOG_LEVEL_ALL);
  
  //   LogComponentEnable ("cv2x_LteEnbRrc", logLevel);
  //   LogComponentEnable ("cv2x_LteUeRrc", logLevel);
  //   LogComponentEnable ("cv2x_LteEnbMac", logLevel);
  //   LogComponentEnable ("cv2x_LteUeMac", logLevel);
  //   LogComponentEnable ("cv2x_LteRlc", logLevel);
  //   LogComponentEnable ("RrPacketScheduler", logLevel);
  // 
  //   LogComponentEnable ("cv2x_LtePhy", logLevel);
  //   LogComponentEnable ("cv2x_LteEnbPhy", logLevel);
  //   LogComponentEnable ("cv2x_LteUePhy", logLevel);
  // 
  //   LogComponentEnable ("cv2x_LteSpectrumPhy", logLevel);
  //   LogComponentEnable ("cv2x_LteInterference", logLevel);
  //   LogComponentEnable ("cv2x_LteChunkProcessor", logLevel);
  // 
  ...
  // 
  //   LogComponentEnable ("cv2x_LteNetDevice", logLevel);
  //   LogComponentEnable ("cv2x_LteUeNetDevice", logLevel);
...
//   LogComponentEnable ("BuildingsPropagationLossModel", LOG_LEVEL_ALL);
  ...
  m_fadingModule = CreateObject<cv2x_TraceFadingLossModel> ();
...
//   NS_LOG_INFO ("A ver " << (*outPsd1)[0] << " " << (*outPsd1)[1]);
  ...
  while (time<0.010)
    {
      Time t = Seconds (time);
      Simulator::Schedule(t, &cv2x_LteFadingTestCase::GetFadingSample, this);
...
void
cv2x_LteFadingTestCase::GetFadingSample ()
{
  Ptr<SpectrumModel> sm;
