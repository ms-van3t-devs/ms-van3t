...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteHandoverTargetTest");
 ...
 *
 * \sa ns3::cv2x_LteHandoverTargetTestCase
 */
class cv2x_LteHandoverTargetTestCase : public TestCase
{
public:
  /**
   ...
   * `cv2x_LteEnbRrc::HandoverStart` trace source.
   *
...
private:
  ...
  Ptr<cv2x_LteEnbNetDevice> m_sourceEnbDev; ///< source ENB device
  bool m_hasHandoverOccurred; ///< has handover occurred?

}; // end of class cv2x_LteHandoverTargetTestCase


cv2x_LteHandoverTargetTestCase::cv2x_LteHandoverTargetTestCase (std::string name, Vector uePosition,
                                                      uint8_t gridSizeX, uint8_t gridSizeY,
                                                      uint16_t sourceCellId, uint16_t targetCellId,
                                                      std::string handoverAlgorithmType)
  : TestCase (name),
...
cv2x_LteHandoverTargetTestCase::~cv2x_LteHandoverTargetTestCase ()
{
...
void
cv2x_LteHandoverTargetTestCase::HandoverStartCallback (std::string context, uint64_t imsi,
                                                  uint16_t sourceCellId, uint16_t rnti,
                                                  uint16_t targetCellId)
{
...
void
cv2x_LteHandoverTargetTestCase::CellShutdownCallback ()
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_sourceEnbDev != 0)
    {
      ...
      Ptr<cv2x_LteEnbPhy> phy = m_sourceEnbDev->GetPhy ();
      phy->SetTxPower (1);
    }
}
...
void
cv2x_LteHandoverTargetTestCase::DoRun ()
{
  NS_LOG_INFO (this << " " << GetName ());

  Config::SetDefault ("ns3::cv2x_LteEnbPhy::TxPower", DoubleValue (38)); // micro cell
  ...
  if (m_handoverAlgorithmType == "ns3::cv2x_A2A4RsrqHandoverAlgorithm")
    {
      lteHelper->SetHandoverAlgorithmType ("ns3::cv2x_A2A4RsrqHandoverAlgorithm");
    ...
    {
      lteHelper->SetHandoverAlgorithmType ("ns3::cv2x_A3RsrpHandoverAlgorithm");
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/cv2x_LteEnbRrc/HandoverStart",
                   MakeCallback (&cv2x_LteHandoverTargetTestCase::HandoverStartCallback,
                                 this));

  // Get the source eNodeB
  Ptr<NetDevice> sourceEnb = enbDevs.Get (m_sourceCellId - 1);
  m_sourceEnbDev = sourceEnb->GetObject<cv2x_LteEnbNetDevice> ();
  ...
  Simulator::Schedule (Seconds (0.5),
                       &cv2x_LteHandoverTargetTestCase::CellShutdownCallback, this);
...
void
cv2x_LteHandoverTargetTestCase::DoTeardown ()
{
  NS_LOG_FUNCTION (this);
 ...
 * - A2-A4-RSRQ handover algorithm (ns3::cv2x_A2A4RsrqHandoverAlgorithm)
 * - Strongest cell handover algorithm (ns3::cv2x_A3RsrpHandoverAlgorithm)
 */
class cv2x_LteHandoverTargetTestSuite : public TestSuite
{
public:
  cv2x_LteHandoverTargetTestSuite ();
};


cv2x_LteHandoverTargetTestSuite::cv2x_LteHandoverTargetTestSuite ()
  : TestSuite ("lte-handover-target", SYSTEM)
{
  ...
  // LogComponentEnable ("cv2x_A2A4RsrqHandoverAlgorithm", LOG_PREFIX_ALL);
  // LogComponentEnable ("cv2x_A2A4RsrqHandoverAlgorithm", LOG_LEVEL_ALL);
  // LogComponentEnable ("cv2x_A3RsrpHandoverAlgorithm", LOG_PREFIX_ALL);
   ...
   *    1 --- 2   o = UE
   */
  AddTestCase (new cv2x_LteHandoverTargetTestCase ("4 cells and A2-A4-RSRQ algorithm",
                                              Vector (20, 40, 0), 2, 2, 1, 3,
                                              "ns3::cv2x_A2A4RsrqHandoverAlgorithm"),
               TestCase::QUICK);
  AddTestCase (new cv2x_LteHandoverTargetTestCase ("4 cells and strongest cell algorithm",
                                              Vector (20, 40, 0), 2, 2, 1, 3,
                                              "ns3::cv2x_A3RsrpHandoverAlgorithm"),
               TestCase::QUICK);
   ...
   *    1 --- 2 --- 3   o = UE
   */
  AddTestCase (new cv2x_LteHandoverTargetTestCase ("6 cells and A2-A4-RSRQ algorithm",
                                              Vector (150, 90, 0), 3, 2, 5, 2,
                                              "ns3::cv2x_A2A4RsrqHandoverAlgorithm"),
               TestCase::EXTENSIVE);
  AddTestCase (new cv2x_LteHandoverTargetTestCase ("6 cells and strongest cell algorithm",
                                              Vector (150, 90, 0), 3, 2, 5, 2,
                                              "ns3::cv2x_A3RsrpHandoverAlgorithm"),
               TestCase::EXTENSIVE);

} // end of cv2x_LteHandoverTargetTestSuite ()


static cv2x_LteHandoverTargetTestSuite g_lteHandoverTargetTestSuiteInstance;
