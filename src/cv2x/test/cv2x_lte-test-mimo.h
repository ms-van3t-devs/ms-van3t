...
#ifndef CV2X_LENA_TEST_MIMO_H
#define CV2X_LENA_TEST_MIMO_H
 ...
 * when MIMO is used.
 */
class cv2x_LenaMimoTestCase : public TestCase
{
public:
  /**
   * Constructor
   ...
   */
  cv2x_LenaMimoTestCase (uint16_t dist, std::vector<uint32_t> estThrDl, std::string schedulerType, bool useIdealRrc);
  virtual ~cv2x_LenaMimoTestCase ();
...
private:
   ...
   * \param rlcStats Ptr<cv2x_RadioBearerStatsCalculator>
   * \param imsi the IMSI
   * \param rnti the RNTI
   */
  void GetRlcBufferSample (Ptr<cv2x_RadioBearerStatsCalculator> rlcStats, uint64_t imsi, uint8_t rnti);
...
class cv2x_LenaTestMimoSuite : public TestSuite
{
public:
  cv2x_LenaTestMimoSuite ();
};

#endif /* CV2X_LENA_TEST_MIMO_H */
