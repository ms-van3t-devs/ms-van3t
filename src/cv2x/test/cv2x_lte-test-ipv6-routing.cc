...
class cv2x_LteIpv6RoutingTestCase : public TestCase
{
public:
  cv2x_LteIpv6RoutingTestCase ();
  virtual ~cv2x_LteIpv6RoutingTestCase ();
...
cv2x_LteIpv6RoutingTestCase::cv2x_LteIpv6RoutingTestCase ()
...
void cv2x_LteIpv6RoutingTestCase::SentAtClient (Ptr<const Packet> p, Ptr<Ipv6> ipv6, uint32_t interface)
{
...
void cv2x_LteIpv6RoutingTestCase::ReceivedAtClient (Ptr<const Packet> p, Ptr<Ipv6> ipv6, uint32_t interface)
{
  Ipv6Header ipv6Header;
  p->PeekHeader (ipv6Header);
  if (ipv6Header.GetNextHeader () == UdpL4Protocol::PROT_NUMBER)
...
void cv2x_LteIpv6RoutingTestCase::EnbToPgw (Ptr<Packet> p)
{
  Ipv6Header ipv6Header;
  p->PeekHeader (ipv6Header);
  if (ipv6Header.GetNextHeader () == UdpL4Protocol::PROT_NUMBER)
...
void cv2x_LteIpv6RoutingTestCase::TunToPgw (Ptr<Packet> p)
{
  Ipv6Header ipv6Header;
  p->PeekHeader (ipv6Header);
  if (ipv6Header.GetNextHeader () == UdpL4Protocol::PROT_NUMBER)
...
void cv2x_LteIpv6RoutingTestCase::Checker ()
{
  bool b = false;
  bool check = true;
      ...
      for (std::list<Ptr<Packet> >::iterator it2 = m_clientTxPkts.begin (); it2 != m_clientTxPkts.end (); it2++)
        {
          ...
          if ((p2->GetUid () == p1->GetUid ()) && sorceAddress == ipv6header1.GetDestinationAddress () && destinationAddress == ipv6header1.GetSourceAddress () && sourcePort == udpHeader1.GetDestinationPort () && destinationPort == udpHeader1.GetSourcePort ())
            {
              b = true;
              break;
            }
...
void
cv2x_LteIpv6RoutingTestCase::DoRun (void)
{
  ...
  ipL3->TraceConnectWithoutContext ("Tx", MakeCallback (&cv2x_LteIpv6RoutingTestCase::SentAtClient, this));
  ipL3->TraceConnectWithoutContext ("Rx", MakeCallback (&cv2x_LteIpv6RoutingTestCase::ReceivedAtClient, this));
  ...
  appPgw->TraceConnectWithoutContext ("RxFromS1u", MakeCallback (&cv2x_LteIpv6RoutingTestCase::EnbToPgw, this));
  appPgw->TraceConnectWithoutContext ("RxFromTun", MakeCallback (&cv2x_LteIpv6RoutingTestCase::TunToPgw, this));

  Simulator::Schedule (Time (Seconds (12.0)), &cv2x_LteIpv6RoutingTestCase::Checker, this);
 ...
 * \brief test suite 1
 */
class cv2x_LteIpv6RoutingTestSuite : public TestSuite
{
public:
  cv2x_LteIpv6RoutingTestSuite ();
};

cv2x_LteIpv6RoutingTestSuite::cv2x_LteIpv6RoutingTestSuite ()
  : TestSuite ("lte-ipv6-routing-test", UNIT)
{
  AddTestCase (new cv2x_LteIpv6RoutingTestCase, TestCase::QUICK);
}

static cv2x_LteIpv6RoutingTestSuite lteipv6testsuite;
