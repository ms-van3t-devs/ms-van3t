...
#include "cv2x_lte-test-aggregation-throughput-scale.h"
...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteAggregationThroughputScaleTest");

cv2x_LteAggregationThroughputScaleTestSuite::cv2x_LteAggregationThroughputScaleTestSuite ()
  : TestSuite ("lte-aggregation-throughput-scale", SYSTEM)
{
  AddTestCase (new cv2x_LteAggregationThroughputScaleTestCase ("Carrier aggregation throughput scale"), TestCase::QUICK);
}

static cv2x_LteAggregationThroughputScaleTestSuite g_lteAggregationThroughputScaleTestSuite;

cv2x_LteAggregationThroughputScaleTestCase::cv2x_LteAggregationThroughputScaleTestCase (std::string name)
  : TestCase (name)
{
...
cv2x_LteAggregationThroughputScaleTestCase::~cv2x_LteAggregationThroughputScaleTestCase ()
{
  NS_LOG_FUNCTION (this << GetName ());
}

double
cv2x_LteAggregationThroughputScaleTestCase::GetThroughput (uint8_t numberOfComponentCarriers)
{
  NS_LOG_FUNCTION (this << GetName ());
  ...
  Config::SetDefault ("ns3::cv2x_LteEnbNetDevice::DlBandwidth", UintegerValue (25));
  Config::SetDefault ("ns3::cv2x_LteEnbNetDevice::UlBandwidth", UintegerValue (25));
  ...
  lteHelper->SetAttribute ("EnbComponentCarrierManager", StringValue ("ns3::cv2x_RrComponentCarrierManager"));
...
void
cv2x_LteAggregationThroughputScaleTestCase::DoRun ()
{
