...
#ifndef CV2X_LTE_TEST_CQI_GENERATION_H
#define CV2X_LTE_TEST_CQI_GENERATION_H
...
class cv2x_LteCqiGenerationTestSuite : public TestSuite
{
public:
  cv2x_LteCqiGenerationTestSuite ();
};
...
class cv2x_LteCqiGenerationTestCase : public TestCase
{
public:
  /**
   * \Constructor
   ...
   * \param ulMcs UL MCS
   */
  cv2x_LteCqiGenerationTestCase (std::string name, bool usePdcchForCqiGeneration,
                            uint16_t dlMcs, uint16_t ulMcs);
  virtual ~cv2x_LteCqiGenerationTestCase ();
 ...
 * \ingroup tests
 *
 * \brief This test is very similar to cv2x_LteCqiGenerationTestCase. The difference is that in this
 * test is enabled the downlink power control.
 */

class cv2x_LteCqiGenerationDlPowerControlTestCase : public TestCase
{
public:
  /**
   * Constructor
   ...
   * \param ulMcs UL MCS
   */
  cv2x_LteCqiGenerationDlPowerControlTestCase (std::string name, uint8_t cell0Pa, uint8_t cell1Pa,
                                          uint16_t dlMcs, uint16_t ulMcs);
  virtual ~cv2x_LteCqiGenerationDlPowerControlTestCase ();
...
#endif /* CV2X_LTE_TEST_CQI_GENERATION_H */
