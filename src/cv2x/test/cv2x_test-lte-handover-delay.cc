...
class cv2x_LteHandoverDelayTestCase : public TestCase
{
public:
  /**
   * Constructor
   ...
   */
  cv2x_LteHandoverDelayTestCase (uint8_t numberOfComponentCarriers, bool useIdealRrc, Time handoverTime,
...
void
cv2x_LteHandoverDelayTestCase::DoRun ()
{
  ...
  auto ccHelper = CreateObject<cv2x_CcHelper> ();
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/cv2x_LteUeRrc/HandoverStart",
      MakeCallback (&cv2x_LteHandoverDelayTestCase::UeHandoverStartCallback, this));
  Config::Connect ("/NodeList/*/DeviceList/*/cv2x_LteUeRrc/HandoverEndOk",
      MakeCallback (&cv2x_LteHandoverDelayTestCase::UeHandoverEndOkCallback, this));

  Config::Connect ("/NodeList/*/DeviceList/*/cv2x_LteEnbRrc/HandoverStart",
      MakeCallback (&cv2x_LteHandoverDelayTestCase::EnbHandoverStartCallback, this));
  Config::Connect ("/NodeList/*/DeviceList/*/cv2x_LteEnbRrc/HandoverEndOk",
      MakeCallback (&cv2x_LteHandoverDelayTestCase::EnbHandoverEndOkCallback, this));
...
} // end of void cv2x_LteHandoverDelayTestCase::DoRun ()


void
cv2x_LteHandoverDelayTestCase::UeHandoverStartCallback (std::string context,
...
void
cv2x_LteHandoverDelayTestCase::UeHandoverEndOkCallback (std::string context,
...
void
cv2x_LteHandoverDelayTestCase::EnbHandoverStartCallback (std::string context,
    uint64_t imsi, uint16_t cellid, uint16_t rnti, uint16_t targetCellId)
{
  NS_LOG_FUNCTION (this << context);
...
void
cv2x_LteHandoverDelayTestCase::EnbHandoverEndOkCallback (std::string context,
    uint64_t imsi, uint16_t cellid, uint16_t rnti)
{
  NS_LOG_FUNCTION (this << context);
      ...
      : TestSuite ("lte-handover-delay", TestSuite::SYSTEM)
  {
    ...
    for (Time handoverTime = Seconds (0.100); handoverTime < Seconds (0.110);
        handoverTime += Seconds (0.001))
      {
        ...
        AddTestCase (
            new cv2x_LteHandoverDelayTestCase (1, true, handoverTime, Seconds (0.005),
    ...
    for (Time handoverTime = Seconds (0.100); handoverTime < Seconds (0.110);
        handoverTime += Seconds (0.001))
      {
        // arguments: useIdealRrc, handoverTime, delayThreshold, simulationDuration
        AddTestCase (
            new cv2x_LteHandoverDelayTestCase (1, false, handoverTime, Seconds (0.020),
                Seconds (0.200)), TestCase::QUICK);
      }
  }
