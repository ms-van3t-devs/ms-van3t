...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteEpcE2eData");
...
/// cv2x_BearerTestData structure
struct cv2x_BearerTestData
{
  /**
   * Constructor
   *
   ...
   */
  cv2x_BearerTestData (uint32_t n, uint32_t s, double i);
  ...
  cv2x_BearerTestData::cv2x_BearerTestData (uint32_t n, uint32_t s, double i)
  : numPkts (n),
    pktSize (s),
...
/// cv2x_UeTestData structure
struct cv2x_UeTestData
{
  std::vector<cv2x_BearerTestData> bearers; ///< the bearer test data
};

/// cv2x_EnbTestData structure
struct cv2x_EnbTestData
{
  std::vector<cv2x_UeTestData> ues; ///< the list of UEs
};
...
class cv2x_LteEpcE2eDataTestCase : public TestCase
{
public:
  /**
   * Constructor
   ...
   */
  cv2x_LteEpcE2eDataTestCase (std::string name, std::vector<cv2x_EnbTestData> v);
  virtual ~cv2x_LteEpcE2eDataTestCase ();

private:
  virtual void DoRun (void);
  std::vector<cv2x_EnbTestData> m_enbTestData; ///< the ENB test data
};


cv2x_LteEpcE2eDataTestCase::cv2x_LteEpcE2eDataTestCase (std::string name, std::vector<cv2x_EnbTestData> v)
  : TestCase (name),
    m_enbTestData (v)
{
...
cv2x_LteEpcE2eDataTestCase::~cv2x_LteEpcE2eDataTestCase ()
{
}

void 
cv2x_LteEpcE2eDataTestCase::DoRun ()
{
  ...
  for (std::vector<cv2x_EnbTestData>::iterator enbit = m_enbTestData.begin ();
      ...
      for (uint32_t u = 0; u < ues.GetN (); ++u)
        {
          ...
          for (uint32_t b = 0; b < enbit->ues.at (u).bearers.size (); ++b)
            {              
              cv2x_BearerTestData& bearerTestData = enbit->ues.at (u).bearers.at (b);
              ...
              cv2x_EpsBearer epsBearer (cv2x_EpsBearer::NGBR_VOICE_VIDEO_GAMING);
  ...
  Config::Set ("/NodeList/*/DeviceList/*/cv2x_LteEnbRrc/UeMap/*/RadioBearerMap/*/cv2x_LteRlc/MaxTxBufferSize",
               UintegerValue (2 * 1024 * 1024));
  Config::Set ("/NodeList/*/DeviceList/*/cv2x_LteUeRrc/RadioBearerMap/*/cv2x_LteRlc/MaxTxBufferSize",
               UintegerValue (2 * 1024 * 1024));
  ...
  for (std::vector<cv2x_EnbTestData>::iterator enbit = m_enbTestData.begin ();
       ...
       ++enbit)
    {
      for (std::vector<cv2x_UeTestData>::iterator ueit = enbit->ues.begin ();
           ueit < enbit->ues.end ();
           ++ueit)
        {                    
 ...
 */
class cv2x_LteEpcE2eDataTestSuite : public TestSuite
{
public:
  cv2x_LteEpcE2eDataTestSuite ();
  
} g_lteEpcE2eDataTestSuite; ///< the test suite

cv2x_LteEpcE2eDataTestSuite::cv2x_LteEpcE2eDataTestSuite ()
  : TestSuite ("lte-epc-e2e-data", SYSTEM)
{  
  std::vector<cv2x_EnbTestData> v1;  
  cv2x_EnbTestData e1;
  cv2x_UeTestData u1;
  cv2x_BearerTestData f1 (1, 100, 0.01);
  u1.bearers.push_back (f1);
  e1.ues.push_back (u1);
  v1.push_back (e1);
  AddTestCase (new cv2x_LteEpcE2eDataTestCase ("1 eNB, 1UE", v1), TestCase::QUICK);

  std::vector<cv2x_EnbTestData> v2;  
  cv2x_EnbTestData e2;
  cv2x_UeTestData u2_1;
  cv2x_BearerTestData f2_1 (1, 100, 0.01);
  u2_1.bearers.push_back (f2_1);
  e2.ues.push_back (u2_1);
  cv2x_UeTestData u2_2;
  cv2x_BearerTestData f2_2 (2, 200, 0.01);
  u2_2.bearers.push_back (f2_2);
  e2.ues.push_back (u2_2);
  v2.push_back (e2);
  AddTestCase (new cv2x_LteEpcE2eDataTestCase ("1 eNB, 2UEs", v2), TestCase::EXTENSIVE);

  std::vector<cv2x_EnbTestData> v3;  
  v3.push_back (e1);
  v3.push_back (e2);
  AddTestCase (new cv2x_LteEpcE2eDataTestCase ("2 eNBs", v3), TestCase::EXTENSIVE);

  cv2x_EnbTestData e4;
  cv2x_UeTestData u4_1;
  cv2x_BearerTestData f4_1 (3, 50, 0.01);
  u4_1.bearers.push_back (f4_1);
  e4.ues.push_back (u4_1);
  cv2x_UeTestData u4_2;
  cv2x_BearerTestData f4_2 (5, 1400, 0.01);
  u4_2.bearers.push_back (f4_2);
  e4.ues.push_back (u4_2);
  cv2x_UeTestData u4_3;
  cv2x_BearerTestData f4_3 (1, 12, 0.01);
  u4_3.bearers.push_back (f4_3);
  e4.ues.push_back (u4_3);
  std::vector<cv2x_EnbTestData> v4;  
  v4.push_back (e4);
  v4.push_back (e1);
  v4.push_back (e2);
  AddTestCase (new cv2x_LteEpcE2eDataTestCase ("3 eNBs", v4), TestCase::EXTENSIVE);

  cv2x_EnbTestData e5;
  cv2x_UeTestData u5;
  cv2x_BearerTestData f5 (5, 1000, 0.01);
  u5.bearers.push_back (f5);
  e5.ues.push_back (u5);
  std::vector<cv2x_EnbTestData> v5;
  v5.push_back (e5);
  AddTestCase (new cv2x_LteEpcE2eDataTestCase ("1 eNB, 1UE with 1000 byte packets", v5), TestCase::EXTENSIVE);


  cv2x_EnbTestData e6;
  cv2x_UeTestData u6;
  cv2x_BearerTestData f6 (5, 1400, 0.01);
  u6.bearers.push_back (f6);
  e6.ues.push_back (u6);
  std::vector<cv2x_EnbTestData> v6;
  v6.push_back (e6);
  AddTestCase (new cv2x_LteEpcE2eDataTestCase ("1 eNB, 1UE with 1400 byte packets", v6), TestCase::EXTENSIVE);

  cv2x_EnbTestData e7;
  cv2x_UeTestData u7;
  cv2x_BearerTestData f7_1 (1, 1400, 0.01);
  u7.bearers.push_back (f7_1);
  cv2x_BearerTestData f7_2 (1, 100, 0.01);
  u7.bearers.push_back (f7_2);
  e7.ues.push_back (u7);
  std::vector<cv2x_EnbTestData> v7;
  v7.push_back (e7);
  AddTestCase (new cv2x_LteEpcE2eDataTestCase ("1 eNB, 1UE with 2 bearers", v7), TestCase::EXTENSIVE);

  cv2x_EnbTestData e8;
  cv2x_UeTestData u8;
  cv2x_BearerTestData f8 (50, 8000, 0.02); // watch out for ns3::cv2x_LteRlcUm::MaxTxBufferSize
  u8.bearers.push_back (f8);
  e8.ues.push_back (u8);
  std::vector<cv2x_EnbTestData> v8;
  v8.push_back (e8);
  AddTestCase (new cv2x_LteEpcE2eDataTestCase ("1 eNB, 1UE with fragmentation", v8), TestCase::EXTENSIVE);


  cv2x_EnbTestData e9;
  cv2x_UeTestData u9;
  cv2x_BearerTestData f9 (1000, 20, 0.0001);
  u9.bearers.push_back (f9);
  e9.ues.push_back (u9);
  std::vector<cv2x_EnbTestData> v9;
  v9.push_back (e9);
  AddTestCase (new cv2x_LteEpcE2eDataTestCase ("1 eNB, 1UE with aggregation", v9), TestCase::EXTENSIVE);
