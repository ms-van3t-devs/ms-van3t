...
#include "cv2x_lte-test-rlc-am-e2e.h"
...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteRlcAmE2eTest");

cv2x_LteRlcAmE2eTestSuite::cv2x_LteRlcAmE2eTestSuite ()
  : TestSuite ("lte-rlc-am-e2e", SYSTEM)
{
  // NS_LOG_INFO ("Creating cv2x_LteRlcAmE2eTestSuite");
          ...
          for (uint32_t sduArrivalType = 0; sduArrivalType <= 1; ++sduArrivalType)
            {
              ...
              AddTestCase (new cv2x_LteRlcAmE2eTestCase (name.str (), runs[s], losses[l], bulkSduArrival), testDuration);
            }
        }
    }
}

static cv2x_LteRlcAmE2eTestSuite lteRlcAmE2eTestSuite;

cv2x_LteRlcAmE2eTestCase::cv2x_LteRlcAmE2eTestCase (std::string name, uint32_t run, double losses, bool bulkSduArrival)
   : TestCase (name),
     m_run (run),
     m_losses (losses),
     ...
     m_ulDrops (0)
{
  NS_LOG_INFO ("Creating cv2x_LteRlcAmTestingTestCase: " + name);
}

cv2x_LteRlcAmE2eTestCase::~cv2x_LteRlcAmE2eTestCase ()
{
}


void
cv2x_LteRlcAmE2eTestCase::DlDropEvent (Ptr<const Packet> p)
{
...
void
cv2x_LteRlcAmE2eTestCase::UlDropEvent (Ptr<const Packet> p)
{
  // NS_LOG_FUNCTION (this);
  m_ulDrops++;
}
...
cv2x_LteRlcAmE2eTestCase::DoRun (void)
{
  ...
  Config::SetDefault ("ns3::cv2x_LteRlcAm::PollRetransmitTimer", TimeValue (MilliSeconds (20)));
  Config::SetDefault ("ns3::cv2x_LteRlcAm::ReorderingTimer", TimeValue (MilliSeconds (10)));
  Config::SetDefault ("ns3::cv2x_LteRlcAm::StatusProhibitTimer", TimeValue (MilliSeconds (40)));

  Ptr<cv2x_LteSimpleHelper> lteSimpleHelper = CreateObject<cv2x_LteSimpleHelper> ();
  ...
  //   lteSimpleHelper->ActivateEpsBearer (ueLteDevs, cv2x_EpsBearer (cv2x_EpsBearer::NGBR_VIDEO_TCP_DEFAULT), cv2x_EpcTft::Default ());
...
//   ueEm->SetAttribute ("ErrorUnit", StringValue ("ERROR_UNIT_PACKET"));
  ...
  ueLteDevs.Get (0)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&cv2x_LteRlcAmE2eTestCase::DlDropEvent, this));
//   enbLteDevs.Get (0)->SetAttribute ("ReceiveErrorModel", PointerValue (enbEm));
//   enbLteDevs.Get (0)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&cv2x_LteRlcAmE2eTestCase::EnbDropEvent, this));
  ...
  lteSimpleHelper->m_enbMac->SetTxOpportunityMode (cv2x_LteTestMac::AUTOMATIC_MODE);

  // MAC sends transmission opportunities (TxOpp)
  lteSimpleHelper->m_ueMac->SetTxOppSize (ulTxOppSizeBytes);
  lteSimpleHelper->m_ueMac->SetTxOppTime (Seconds (ulTxOpprTimeSeconds));
  lteSimpleHelper->m_ueMac->SetTxOpportunityMode (cv2x_LteTestMac::AUTOMATIC_MODE);

  // Start/Stop pseudo-application at RRC layer
  Simulator::Schedule (Seconds (sduStartTimeSeconds), &cv2x_LteTestRrc::Start, lteSimpleHelper->m_enbRrc);
  Simulator::Schedule (Seconds (sduStopTimeSeconds), &cv2x_LteTestRrc::Stop, lteSimpleHelper->m_enbRrc);
