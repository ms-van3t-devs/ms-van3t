...
#include "cv2x_lte-phy.h"
#include "cv2x_lte-net-device.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("cv2x_LtePhy");

NS_OBJECT_ENSURE_REGISTERED (cv2x_LtePhy);


cv2x_LtePhy::cv2x_LtePhy ()
{
  NS_LOG_FUNCTION (this);
  NS_FATAL_ERROR ("This constructor should not be called");
}

cv2x_LtePhy::cv2x_LtePhy (Ptr<cv2x_LteSpectrumPhy> dlPhy, Ptr<cv2x_LteSpectrumPhy> ulPhy)
...
TypeId
cv2x_LtePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_LtePhy")
    .SetParent<Object> ()
    .SetGroupName("Lte")
  ;
  return tid;
...
void
cv2x_LtePhy::SetDevice (Ptr<cv2x_LteNetDevice> d)
{
...
Ptr<cv2x_LteNetDevice>
cv2x_LtePhy::GetDevice () const
{
  NS_LOG_FUNCTION (this);
  return m_netDevice;
}
...
cv2x_LtePhy::GetDownlinkSpectrumPhy ()
{
  return m_downlinkSpectrumPhy;
}

Ptr<cv2x_LteSpectrumPhy> 
cv2x_LtePhy::GetUplinkSpectrumPhy ()
{
...
void
cv2x_LtePhy::SetDownlinkChannel (Ptr<SpectrumChannel> c)
{
...
void
cv2x_LtePhy::SetUplinkChannel (Ptr<SpectrumChannel> c)
{
  NS_LOG_FUNCTION (this << c);
...
uint16_t
cv2x_LtePhy::GetSrsPeriodicity (uint16_t srcCi) const
{
...
uint16_t
cv2x_LtePhy::GetSrsSubframeOffset (uint16_t srcCi) const
{
  // from 3GPP TS 36.213 table 8.2-1 UE Specific SRS Periodicity
...
uint8_t
cv2x_LtePhy::GetRbgSize (void) const
{
  return m_rbgSize;
}

void
cv2x_LtePhy::SetMacPdu (Ptr<Packet> p)
{
  m_packetBurstQueue.at (m_packetBurstQueue.size () - 1)->AddPacket (p);
}

Ptr<PacketBurst>
cv2x_LtePhy::GetPacketBurst (void)
{
...
void
cv2x_LtePhy::SetControlMessages (Ptr<cv2x_LteControlMessage> m)
{
...
std::list<Ptr<cv2x_LteControlMessage> >
cv2x_LtePhy::GetControlMessages (void)
{
  NS_LOG_FUNCTION (this);
  if (m_controlMessagesQueue.at (0).size () > 0)
    {
      std::list<Ptr<cv2x_LteControlMessage> > ret = m_controlMessagesQueue.at (0);
      m_controlMessagesQueue.erase (m_controlMessagesQueue.begin ());
      std::list<Ptr<cv2x_LteControlMessage> > newlist;
  ...
  else
    {
      ...
      std::list<Ptr<cv2x_LteControlMessage> > newlist;
      m_controlMessagesQueue.push_back (newlist);
      std::list<Ptr<cv2x_LteControlMessage> > emptylist;
      return (emptylist);
    }
}
...
void
cv2x_LtePhy::DoSetCellId (uint16_t cellId)
{
  m_cellId = cellId;
...
void
cv2x_LtePhy::SetComponentCarrierId (uint8_t index)
{
...
uint8_t
cv2x_LtePhy::GetComponentCarrierId ()
{
