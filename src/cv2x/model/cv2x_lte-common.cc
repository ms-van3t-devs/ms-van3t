...
#include "cv2x_lte-common.h"
#include <ns3/log.h>
#include <ns3/abort.h>

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("cv2x_LteCommon");

cv2x_LteFlowId_t::cv2x_LteFlowId_t ()
{
}

cv2x_LteFlowId_t::cv2x_LteFlowId_t (const uint16_t a, const uint8_t b)
  : m_rnti (a),
    m_lcId (b)
{
}
 ...
 */
bool
operator == (const cv2x_LteFlowId_t &a, const cv2x_LteFlowId_t &b)
{
...
cv2x_ImsiLcidPair_t::cv2x_ImsiLcidPair_t ()
{
}

cv2x_ImsiLcidPair_t::cv2x_ImsiLcidPair_t (const uint64_t a, const uint8_t b)
  : m_imsi (a),
    m_lcId (b)
{
}
 ...
 */
bool
operator == (const cv2x_ImsiLcidPair_t &a, const cv2x_ImsiLcidPair_t &b)
{
...
cv2x_LteUeConfig_t::cv2x_LteUeConfig_t ()
{
}
 ...
 */
bool
operator == (const cv2x_LteUeConfig_t &a, const cv2x_LteUeConfig_t &b)
{
...
uint16_t
cv2x_LteFfConverter::double2fpS11dot3 (double val)
{
...
double
cv2x_LteFfConverter::fpS11dot3toDouble (uint16_t val)
{
...
double 
cv2x_LteFfConverter::getMinFpS11dot3Value ()
{
...
static const uint32_t cv2x_BufferSizeLevelBsrTable[64] = {
...
uint32_t
cv2x_BufferSizeLevelBsr::BsrId2BufferSize (uint8_t val)
{
  NS_ABORT_MSG_UNLESS (val < 64, "val = " << val << " is out of range");
  return cv2x_BufferSizeLevelBsrTable[val];
}

uint8_t
cv2x_BufferSizeLevelBsr::BufferSize2BsrId (uint32_t val)
{
  int index = 0;
  if (cv2x_BufferSizeLevelBsrTable[63] < val)
    {
      index = 63;
    }
  else
    {
      while (cv2x_BufferSizeLevelBsrTable[index] < val)
        {
          NS_ASSERT (index < 64);
          index++;
        }
...
uint8_t
cv2x_TransmissionModesLayers::TxMode2LayerNum (uint8_t txMode)
{
  uint8_t nLayer = 0;
  switch (txMode)
    {
...
double 
cv2x_EutranMeasurementMapping::RsrpRange2Dbm (uint8_t range)
{
...
uint8_t 
cv2x_EutranMeasurementMapping::Dbm2RsrpRange (double dbm)
{
  // 3GPP TS 36.133 section 9.1.4 RSRP Measurement Report Mapping
...
double 
cv2x_EutranMeasurementMapping::RsrqRange2Db (uint8_t range)
{
...
uint8_t 
cv2x_EutranMeasurementMapping::Db2RsrqRange (double db)
{
  // 3GPP TS 36.133 section 9.1.7 RSRQ Measurement Report Mapping
...
double 
cv2x_EutranMeasurementMapping::QuantizeRsrp (double v)
{
  return RsrpRange2Dbm (Dbm2RsrpRange (v));
}

double 
cv2x_EutranMeasurementMapping::QuantizeRsrq (double v)
{
  return RsrqRange2Db (Db2RsrqRange (v));
}

double
cv2x_EutranMeasurementMapping::IeValue2ActualHysteresis (uint8_t hysteresisIeValue)
{
...
uint8_t
cv2x_EutranMeasurementMapping::ActualHysteresis2IeValue (double hysteresisDb)
{
...
double
cv2x_EutranMeasurementMapping::IeValue2ActualA3Offset (int8_t a3OffsetIeValue)
{
...
int8_t
cv2x_EutranMeasurementMapping::ActualA3Offset2IeValue (double a3OffsetDb)
{
...
double
cv2x_EutranMeasurementMapping::IeValue2ActualQRxLevMin (int8_t qRxLevMinIeValue)
{
...
double
cv2x_EutranMeasurementMapping::IeValue2ActualQQualMin (int8_t qQualMinIeValue)
{
