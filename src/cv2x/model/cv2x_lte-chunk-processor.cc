...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteChunkProcessor");

cv2x_LteChunkProcessor::cv2x_LteChunkProcessor ()
{
...
void
cv2x_LteChunkProcessor::AddCallback (cv2x_LteChunkProcessorCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_LteChunkProcessor::EvaluateChunk (const SpectrumValue& sinr, Time duration)
{
...
cv2x_LteChunkProcessor::End ()
{
  ...
  if (m_totDuration.GetSeconds () > 0)
    {
      std::vector<cv2x_LteChunkProcessorCallback>::iterator it;
...
void
cv2x_LteSpectrumValueCatcher::ReportValue (const SpectrumValue& value)
{
  m_value = value.Copy ();
}

Ptr<SpectrumValue> 
cv2x_LteSpectrumValueCatcher::GetValue ()
{
  return m_value;
}
