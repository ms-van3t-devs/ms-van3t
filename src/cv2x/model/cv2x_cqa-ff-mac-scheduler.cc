...
NS_LOG_COMPONENT_DEFINE ("cv2x_CqaFfMacScheduler");
...
NS_OBJECT_ENSURE_REGISTERED (cv2x_CqaFfMacScheduler);


/// cv2x_qos_rb_and_CQI_assigned_to_lc
struct cv2x_qos_rb_and_CQI_assigned_to_lc
{
...
typedef std::map<CQI_value,cv2x_LteFlowId_t,bool(*)(uint8_t,uint8_t)> t_map_CQIToUE; //sorted
...
typedef std::map<CQI_value,cv2x_LteFlowId_t,bool(*)(uint8_t,uint8_t)>::iterator t_it_CQIToUE; //sorted
...
/// HOL group map typedef
typedef std::multimap<HOL_group,std::set<cv2x_LteFlowId_t>,bool(*)(int,int)> t_map_HOLgroupToUEs;
/// HOL group multi map iterator typedef
typedef std::map<HOL_group,std::set<cv2x_LteFlowId_t> >::iterator t_it_HOLgroupToUEs;

//typedef std::map<RBG_index,CQI_value>  map_RBG_to_CQI;
//typedef std::map<cv2x_LteFlowId_t,map_RBG_to_CQI> map_flowId_to_CQI_map;
...
cv2x_CqaFfMacScheduler::cv2x_CqaFfMacScheduler ()
  :   m_cschedSapUser (0),
    m_schedSapUser (0),
    m_timeWindow (99.0),
    m_nextRntiUl (0)
{
  ...
  m_cschedSapProvider = new cv2x_MemberCschedSapProvider<cv2x_CqaFfMacScheduler> (this);
  m_schedSapProvider = new cv2x_MemberSchedSapProvider<cv2x_CqaFfMacScheduler> (this);
  m_ffrSapProvider = 0;
  m_ffrSapUser = new cv2x_MemberLteFfrSapUser<cv2x_CqaFfMacScheduler> (this);
}
...
void
cv2x_CqaFfMacScheduler::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
TypeId
cv2x_CqaFfMacScheduler::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_CqaFfMacScheduler")
    .SetParent<cv2x_FfMacScheduler> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_CqaFfMacScheduler>()
    .AddAttribute ("CqiTimerThreshold",
                   "The number of TTIs a CQI is valid (default 1000 - 1 sec.)",
                   UintegerValue (1000),
                   MakeUintegerAccessor (&cv2x_CqaFfMacScheduler::m_cqiTimersThreshold),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("CqaMetric",
                   "cv2x_CqaFfMacScheduler metric type that can be: CqaFf, CqaPf",
                   StringValue ("CqaFf"),
                   MakeStringAccessor (&cv2x_CqaFfMacScheduler::m_CqaMetric),
                   MakeStringChecker ())
    .AddAttribute ("HarqEnabled",
                   "Activate/Deactivate the HARQ [by default is active].",
                   BooleanValue (true),
                   MakeBooleanAccessor (&cv2x_CqaFfMacScheduler::m_harqOn),
                   MakeBooleanChecker ())
    .AddAttribute ("UlGrantMcs",
                   "The MCS of the UL grant, must be [0..15] (default 0)",
                   UintegerValue (0),
                   MakeUintegerAccessor (&cv2x_CqaFfMacScheduler::m_ulGrantMcs),
...
void
cv2x_CqaFfMacScheduler::SetFfMacCschedSapUser (cv2x_FfMacCschedSapUser* s)
{
  m_cschedSapUser = s;
}

void
cv2x_CqaFfMacScheduler::SetFfMacSchedSapUser (cv2x_FfMacSchedSapUser* s)
{
  m_schedSapUser = s;
}

cv2x_FfMacCschedSapProvider*
cv2x_CqaFfMacScheduler::GetFfMacCschedSapProvider ()
{
  return m_cschedSapProvider;
}

cv2x_FfMacSchedSapProvider*
cv2x_CqaFfMacScheduler::GetFfMacSchedSapProvider ()
{
  return m_schedSapProvider;
}

void
cv2x_CqaFfMacScheduler::SetLteFfrSapProvider (cv2x_LteFfrSapProvider* s)
{
  m_ffrSapProvider = s;
}

cv2x_LteFfrSapUser*
cv2x_CqaFfMacScheduler::GetLteFfrSapUser ()
{
  return m_ffrSapUser;
}

void
cv2x_CqaFfMacScheduler::DoCschedCellConfigReq (const struct cv2x_FfMacCschedSapProvider::CschedCellConfigReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_CqaFfMacScheduler::DoCschedUeConfigReq (const struct cv2x_FfMacCschedSapProvider::CschedUeConfigReqParameters& params)
{
...
void
cv2x_CqaFfMacScheduler::DoCschedLcConfigReq (const struct cv2x_FfMacCschedSapProvider::CschedLcConfigReqParameters& params)
{
  ...
  std::map <uint16_t, cv2x_CqasFlowPerf_t>::iterator it;
      ...
      if (it == m_flowStatsDl.end ())
        {
          ...
          cv2x_CqasFlowPerf_t flowStatsDl;
          ...
          m_flowStatsDl.insert (std::pair<uint16_t, cv2x_CqasFlowPerf_t> (params.m_rnti, flowStatsDl));
          cv2x_CqasFlowPerf_t flowStatsUl;
          ...
          m_flowStatsUl.insert (std::pair<uint16_t, cv2x_CqasFlowPerf_t> (params.m_rnti, flowStatsUl));
        }
      else
        {
...
void
cv2x_CqaFfMacScheduler::DoCschedLcReleaseReq (const struct cv2x_FfMacCschedSapProvider::CschedLcReleaseReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_CqaFfMacScheduler::DoCschedUeReleaseReq (const struct cv2x_FfMacCschedSapProvider::CschedUeReleaseReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_CqaFfMacScheduler::DoSchedDlRlcBufferReq (const struct cv2x_FfMacSchedSapProvider::SchedDlRlcBufferReqParameters& params)
{
...
void
cv2x_CqaFfMacScheduler::DoSchedDlPagingBufferReq (const struct cv2x_FfMacSchedSapProvider::SchedDlPagingBufferReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_CqaFfMacScheduler::DoSchedDlMacBufferReq (const struct cv2x_FfMacSchedSapProvider::SchedDlMacBufferReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  NS_FATAL_ERROR ("method not implemented");
  return;
...
int
cv2x_CqaFfMacScheduler::GetRbgSize (int dlbandwidth)
{
  for (int i = 0; i < 4; i++)
    {
...
unsigned int
cv2x_CqaFfMacScheduler::LcActivePerFlow (uint16_t rnti)
{
  std::map <cv2x_LteFlowId_t, cv2x_FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
...
uint8_t
cv2x_CqaFfMacScheduler::HarqProcessAvailability (uint16_t rnti)
{
...
uint8_t
cv2x_CqaFfMacScheduler::UpdateHarqProcessId (uint16_t rnti)
{
  NS_LOG_FUNCTION (this << rnti);
...
void
cv2x_CqaFfMacScheduler::RefreshHarqProcesses ()
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_CqaFfMacScheduler::DoSchedDlTriggerReq (const struct cv2x_FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  std::map <uint16_t, std::multimap <uint8_t, cv2x_qos_rb_and_CQI_assigned_to_lc> > allocationMapPerRntiPerLCId;
  std::map <uint16_t, std::multimap <uint8_t, cv2x_qos_rb_and_CQI_assigned_to_lc> >::iterator itMap;
  ...
  std::map<cv2x_LteFlowId_t,int> UEtoHOL;
  ...
  for (itLogicalChannels = m_ueLogicalChannelsConfigList.begin (); itLogicalChannels != m_ueLogicalChannelsConfigList.end (); itLogicalChannels++)
    {
      ...
      std::map <cv2x_LteFlowId_t, cv2x_FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator itRlcBufferReq = m_rlcBufferReq.find (itLogicalChannels->first);
      ...
      UEtoHOL.insert (std::pair<cv2x_LteFlowId_t,int>(itLogicalChannels->first,delay));

      if (itLogicalChannels->second.m_qosBearerType == itLogicalChannels->second.QBT_NON_GBR )
        {
          if (map_nonGBRHOLgroupToUE.count (group)==0)
            {
              std::set<cv2x_LteFlowId_t> v;
              v.insert (itRlcBufferReq->first);
              map_nonGBRHOLgroupToUE.insert (std::pair<int,std::set<cv2x_LteFlowId_t> >(group,v));
            }
          else
            {
          ...
          if (map_GBRHOLgroupToUE.count (group)==0)
            {
              std::set<cv2x_LteFlowId_t> v;
              v.insert (itRlcBufferReq->first);
              map_GBRHOLgroupToUE.insert (std::pair<int,std::set<cv2x_LteFlowId_t> >(group,v));
            }
          else
            {
  ...
  std::map<cv2x_LteFlowId_t, int> UeToAmountOfDataToTransfer;
  //Initialize the map per UE, how much resources is already assigned to the user
  std::map<cv2x_LteFlowId_t, int> UeToAmountOfAssignedResources;
  ...
  for( std::map <cv2x_LteFlowId_t, cv2x_FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator itrbr = m_rlcBufferReq.begin ();
       itrbr!=m_rlcBufferReq.end (); itrbr++)
    {

      cv2x_LteFlowId_t flowId = itrbr->first;                // Prepare data for the scheduling mechanism
      ...
      UeToAmountOfDataToTransfer.insert (std::pair<cv2x_LteFlowId_t,int>(flowId,amountOfDataToTransfer));
      UeToAmountOfAssignedResources.insert (std::pair<cv2x_LteFlowId_t,int>(flowId,0));
      ...
      while (availableRBGs.size ()>0 and itCurrentGroup->second.size ()>0)
        {
          ...
          std::map<cv2x_LteFlowId_t, CQI_value> UeToCQIValue;
          std::map<cv2x_LteFlowId_t, double > UeToCoitaMetric;
          std::map<cv2x_LteFlowId_t, bool> UeHasReachedGBR;
          double maximumValueMetric = 0;
          cv2x_LteFlowId_t userWithMaximumMetric;
          UeToCQIValue.clear ();
          ...
          for (std::set<cv2x_LteFlowId_t>::iterator it=itCurrentGroup->second.begin (); it!=itCurrentGroup->second.end (); it++)
            {
              cv2x_LteFlowId_t flowId = *it;
              ...
              cv2x_LogicalChannelConfigListElement_s lc = m_ueLogicalChannelsConfigList.find (flowId)->second;
              std::map <uint16_t,cv2x_SbMeasResult_s>::iterator itRntiCQIsMap = m_a30CqiRxed.find (flowId.m_rnti);

              std::map <uint16_t, cv2x_CqasFlowPerf_t>::iterator itStats;
              ...
              if (itRntiCQIsMap != m_a30CqiRxed.end ())
                {
                  ...
                  UeToCQIValue.insert (std::pair<cv2x_LteFlowId_t,CQI_value>(flowId,cqi_value));
                  UeToCoitaMetric.insert (std::pair<cv2x_LteFlowId_t, double>(flowId,coita_metric));
                }
              ...
              if (allocationMapPerRntiPerLCId.find (flowId.m_rnti)==allocationMapPerRntiPerLCId.end ())
                {
                  ...
                  for (std::multimap <uint8_t, cv2x_qos_rb_and_CQI_assigned_to_lc>::iterator itRBG = allocationMapPerRntiPerLCId.find (flowId.m_rnti)->second.begin ();
                       itRBG!=allocationMapPerRntiPerLCId.find (flowId.m_rnti)->second.end (); itRBG++)
                    {
                      cv2x_qos_rb_and_CQI_assigned_to_lc e = itRBG->second;
              ...
              cv2x_FfMacSchedSapProvider::SchedDlRlcBufferReqParameters lcBufferInfo = m_rlcBufferReq.find (flowId)->second;

              if (UeToAmountOfDataToTransfer.find (flowId)->second - UeToAmountOfAssignedResources.find (flowId)->second < 0)
                {
                  UeHasReachedGBR.insert (std::pair<cv2x_LteFlowId_t,bool>(flowId,false));
                }
              ...
              if(bitRateWithNewRBG > lc.m_eRabGuaranteedBitrateDl)
                {
                  UeHasReachedGBR.insert (std::pair<cv2x_LteFlowId_t,bool>(flowId,true));
                }
              else
                {
                  UeHasReachedGBR.insert (std::pair<cv2x_LteFlowId_t,bool>(flowId,false));
          ...
          cv2x_qos_rb_and_CQI_assigned_to_lc s;
          ...
          if (itMap == allocationMapPerRntiPerLCId.end ())
            {
              std::multimap <uint8_t, cv2x_qos_rb_and_CQI_assigned_to_lc> tempMap;
              tempMap.insert (std::pair<uint8_t, cv2x_qos_rb_and_CQI_assigned_to_lc> (userWithMaximumMetric.m_lcId,s));
              allocationMapPerRntiPerLCId.insert (std::pair <uint16_t, std::multimap <uint8_t,cv2x_qos_rb_and_CQI_assigned_to_lc> > (userWithMaximumMetric.m_rnti, tempMap));
            }
          else
            {
              itMap->second.insert (std::pair<uint8_t,cv2x_qos_rb_and_CQI_assigned_to_lc> (userWithMaximumMetric.m_lcId,s));
            }
  ...
  while (itMap != allocationMapPerRntiPerLCId.end ())
    {
      ...
      for ( std::multimap <uint8_t, cv2x_qos_rb_and_CQI_assigned_to_lc> ::iterator it = (*itMap).second.begin (); it != (*itMap).second.end (); it++)
        {
      ...
      uint32_t rbgMask = 0;
      std::multimap <uint8_t, cv2x_qos_rb_and_CQI_assigned_to_lc> ::iterator itRBGsPerRNTI;
      ...
      // NOTE: In this first version of cv2x_CqaFfMacScheduler, it is assumed one flow per user.
  ...
  for (std::map <uint16_t, std::multimap <uint8_t, cv2x_qos_rb_and_CQI_assigned_to_lc> >::iterator itMap = allocationMapPerRntiPerLCId.begin (); itMap!=allocationMapPerRntiPerLCId.end (); itMap++)
    {
...
void
cv2x_CqaFfMacScheduler::DoSchedDlRachInfoReq (const struct cv2x_FfMacSchedSapProvider::SchedDlRachInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_CqaFfMacScheduler::DoSchedDlCqiInfoReq (const struct cv2x_FfMacSchedSapProvider::SchedDlCqiInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
double
cv2x_CqaFfMacScheduler::EstimateUlSinr (uint16_t rnti, uint16_t rb)
{
...
void
cv2x_CqaFfMacScheduler::DoSchedUlTriggerReq (const struct cv2x_FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
...
void
cv2x_CqaFfMacScheduler::DoSchedUlNoiseInterferenceReq (const struct cv2x_FfMacSchedSapProvider::SchedUlNoiseInterferenceReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  return;
}
...
void
cv2x_CqaFfMacScheduler::DoSchedUlSrInfoReq (const struct cv2x_FfMacSchedSapProvider::SchedUlSrInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  return;
}
...
void
cv2x_CqaFfMacScheduler::DoSchedUlMacCtrlInfoReq (const struct cv2x_FfMacSchedSapProvider::SchedUlMacCtrlInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_CqaFfMacScheduler::DoSchedUlCqiInfoReq (const struct cv2x_FfMacSchedSapProvider::SchedUlCqiInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_CqaFfMacScheduler::RefreshDlCqiMaps (void)
{
...
void
cv2x_CqaFfMacScheduler::RefreshUlCqiMaps (void)
{
  // refresh UL CQI  Map
...
void
cv2x_CqaFfMacScheduler::UpdateDlRlcBufferInfo (uint16_t rnti, uint8_t lcid, uint16_t size)
{
  std::map<cv2x_LteFlowId_t, cv2x_FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
...
void
cv2x_CqaFfMacScheduler::UpdateUlRlcBufferInfo (uint16_t rnti, uint16_t size)
{
...
void
cv2x_CqaFfMacScheduler::TransmissionModeConfigurationUpdate (uint16_t rnti, uint8_t txMode)
{
