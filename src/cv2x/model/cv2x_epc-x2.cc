...
NS_LOG_COMPONENT_DEFINE ("cv2x_EpcX2");

cv2x_X2IfaceInfo::cv2x_X2IfaceInfo (Ipv4Address remoteIpAddr, Ptr<Socket> localCtrlPlaneSocket, Ptr<Socket> localUserPlaneSocket)
{
...
cv2x_X2IfaceInfo::~cv2x_X2IfaceInfo (void)
{
...
cv2x_X2IfaceInfo& 
cv2x_X2IfaceInfo::operator= (const cv2x_X2IfaceInfo& value)
{
...
cv2x_X2CellInfo::cv2x_X2CellInfo (uint16_t localCellId, uint16_t remoteCellId)
{
  m_localCellId = localCellId;
  m_remoteCellId = remoteCellId;
}

cv2x_X2CellInfo::~cv2x_X2CellInfo (void)
{
  m_localCellId = 0;
  m_remoteCellId = 0;
}
...
cv2x_X2CellInfo& 
cv2x_X2CellInfo::operator= (const cv2x_X2CellInfo& value)
{
  NS_LOG_FUNCTION (this);
...
NS_OBJECT_ENSURE_REGISTERED (cv2x_EpcX2);

cv2x_EpcX2::cv2x_EpcX2 ()
  : m_x2cUdpPort (4444),
    m_x2uUdpPort (2152)
{
  NS_LOG_FUNCTION (this);

  m_x2SapProvider = new cv2x_EpcX2SpecificEpcX2SapProvider<cv2x_EpcX2> (this);
}
...
TypeId
cv2x_EpcX2::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_EpcX2")
    .SetParent<Object> ()
    .SetGroupName("Lte");
  return tid;
}
...
void
cv2x_EpcX2::SetEpcX2SapUser (cv2x_EpcX2SapUser * s)
{
  NS_LOG_FUNCTION (this << s);
  m_x2SapUser = s;
}

cv2x_EpcX2SapProvider*
cv2x_EpcX2::GetEpcX2SapProvider ()
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_EpcX2::AddX2Interface (uint16_t localCellId, Ipv4Address localX2Address, uint16_t remoteCellId, Ipv4Address remoteX2Address)
{
  ...
  NS_ASSERT (retval == 0);
  localX2cSocket->SetRecvCallback (MakeCallback (&cv2x_EpcX2::RecvFromX2cSocket, this));
  ...
  NS_ASSERT (retval == 0);
  localX2uSocket->SetRecvCallback (MakeCallback (&cv2x_EpcX2::RecvFromX2uSocket, this));
  ...
  m_x2InterfaceSockets [remoteCellId] = Create<cv2x_X2IfaceInfo> (remoteX2Address, localX2cSocket, localX2uSocket);
  ...
  m_x2InterfaceCellIds [localX2cSocket] = Create<cv2x_X2CellInfo> (localCellId, remoteCellId);
  ...
  m_x2InterfaceCellIds [localX2uSocket] = Create<cv2x_X2CellInfo> (localCellId, remoteCellId);
}


void 
cv2x_EpcX2::RecvFromX2cSocket (Ptr<Socket> socket)
{
  ...
  Ptr<cv2x_X2CellInfo> cellsInfo = m_x2InterfaceCellIds [socket];

  cv2x_EpcX2Header x2Header;
  ...
  if (procedureCode == cv2x_EpcX2Header::HandoverPreparation)
    {
      if (messageType == cv2x_EpcX2Header::InitiatingMessage)
        {
          NS_LOG_LOGIC ("Recv X2 message: HANDOVER REQUEST");

          cv2x_EpcX2HandoverRequestHeader x2HoReqHeader;
          packet->RemoveHeader (x2HoReqHeader);

          NS_LOG_INFO ("X2 HandoverRequest header: " << x2HoReqHeader);

          cv2x_EpcX2SapUser::HandoverRequestParams params;
      ...
      else if (messageType == cv2x_EpcX2Header::SuccessfulOutcome)
        {
          NS_LOG_LOGIC ("Recv X2 message: HANDOVER REQUEST ACK");

          cv2x_EpcX2HandoverRequestAckHeader x2HoReqAckHeader;
          packet->RemoveHeader (x2HoReqAckHeader);

          NS_LOG_INFO ("X2 HandoverRequestAck header: " << x2HoReqAckHeader);

          cv2x_EpcX2SapUser::HandoverRequestAckParams params;          
      ...
      else // messageType == cv2x_EpcX2Header::UnsuccessfulOutcome
        {
          NS_LOG_LOGIC ("Recv X2 message: HANDOVER PREPARATION FAILURE");

          cv2x_EpcX2HandoverPreparationFailureHeader x2HoPrepFailHeader;
          packet->RemoveHeader (x2HoPrepFailHeader);

          NS_LOG_INFO ("X2 HandoverPreparationFailure header: " << x2HoPrepFailHeader);

          cv2x_EpcX2SapUser::HandoverPreparationFailureParams params;          
  ...
  else if (procedureCode == cv2x_EpcX2Header::LoadIndication)
    {
      if (messageType == cv2x_EpcX2Header::InitiatingMessage)
        {
          NS_LOG_LOGIC ("Recv X2 message: LOAD INFORMATION");

          cv2x_EpcX2LoadInformationHeader x2LoadInfoHeader;
  ...
  else if (procedureCode == cv2x_EpcX2Header::SnStatusTransfer)
    {
      if (messageType == cv2x_EpcX2Header::InitiatingMessage)
        {
          NS_LOG_LOGIC ("Recv X2 message: SN STATUS TRANSFER");

            cv2x_EpcX2SnStatusTransferHeader x2SnStatusXferHeader;
            packet->RemoveHeader (x2SnStatusXferHeader);

            NS_LOG_INFO ("X2 SnStatusTransfer header: " << x2SnStatusXferHeader);

            cv2x_EpcX2SapUser::SnStatusTransferParams params;
  ...
  else if (procedureCode == cv2x_EpcX2Header::UeContextRelease)
    {
      if (messageType == cv2x_EpcX2Header::InitiatingMessage)
        {
          NS_LOG_LOGIC ("Recv X2 message: UE CONTEXT RELEASE");

          cv2x_EpcX2UeContextReleaseHeader x2UeCtxReleaseHeader;
          packet->RemoveHeader (x2UeCtxReleaseHeader);

          NS_LOG_INFO ("X2 UeContextRelease header: " << x2UeCtxReleaseHeader);

          cv2x_EpcX2SapUser::UeContextReleaseParams params;
  ...
  else if (procedureCode == cv2x_EpcX2Header::ResourceStatusReporting)
    {
      if (messageType == cv2x_EpcX2Header::InitiatingMessage)
        {
          NS_LOG_LOGIC ("Recv X2 message: RESOURCE STATUS UPDATE");

          cv2x_EpcX2ResourceStatusUpdateHeader x2ResStatUpdHeader;
          packet->RemoveHeader (x2ResStatUpdHeader);

          NS_LOG_INFO ("X2 ResourceStatusUpdate header: " << x2ResStatUpdHeader);

          cv2x_EpcX2SapUser::ResourceStatusUpdateParams params;
...
void 
cv2x_EpcX2::RecvFromX2uSocket (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  ...
  cv2x_EpcX2SapUser::UeDataParams params;
  params.sourceCellId = cellsInfo->m_remoteCellId;
  params.targetCellId = cellsInfo->m_localCellId;
...
//
void
cv2x_EpcX2::DoSendHandoverRequest (cv2x_EpcX2SapProvider::HandoverRequestParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  Ptr<cv2x_X2IfaceInfo> socketInfo = m_x2InterfaceSockets [params.targetCellId];
  ...
  cv2x_EpcX2Header x2Header;
  x2Header.SetMessageType (cv2x_EpcX2Header::InitiatingMessage);
  x2Header.SetProcedureCode (cv2x_EpcX2Header::HandoverPreparation);
...
void
cv2x_EpcX2::DoSendHandoverRequestAck (cv2x_EpcX2SapProvider::HandoverRequestAckParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  // Build the X2 message
  cv2x_EpcX2HandoverRequestAckHeader x2HoAckHeader;
  ...
  cv2x_EpcX2Header x2Header;
  x2Header.SetMessageType (cv2x_EpcX2Header::SuccessfulOutcome);
  x2Header.SetProcedureCode (cv2x_EpcX2Header::HandoverPreparation);
...
void
cv2x_EpcX2::DoSendHandoverPreparationFailure (cv2x_EpcX2SapProvider::HandoverPreparationFailureParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  cv2x_EpcX2Header x2Header;
  x2Header.SetMessageType (cv2x_EpcX2Header::UnsuccessfulOutcome);
  x2Header.SetProcedureCode (cv2x_EpcX2Header::HandoverPreparation);
...
void
cv2x_EpcX2::DoSendSnStatusTransfer (cv2x_EpcX2SapProvider::SnStatusTransferParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  cv2x_EpcX2Header x2Header;
  x2Header.SetMessageType (cv2x_EpcX2Header::InitiatingMessage);
  x2Header.SetProcedureCode (cv2x_EpcX2Header::SnStatusTransfer);
...
void
cv2x_EpcX2::DoSendUeContextRelease (cv2x_EpcX2SapProvider::UeContextReleaseParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  cv2x_EpcX2Header x2Header;
  x2Header.SetMessageType (cv2x_EpcX2Header::InitiatingMessage);
  x2Header.SetProcedureCode (cv2x_EpcX2Header::UeContextRelease);
...
void
cv2x_EpcX2::DoSendLoadInformation (cv2x_EpcX2SapProvider::LoadInformationParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  cv2x_EpcX2Header x2Header;
  x2Header.SetMessageType (cv2x_EpcX2Header::InitiatingMessage);
  x2Header.SetProcedureCode (cv2x_EpcX2Header::LoadIndication);
...
void
cv2x_EpcX2::DoSendResourceStatusUpdate (cv2x_EpcX2SapProvider::ResourceStatusUpdateParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  // Build the X2 message
  cv2x_EpcX2ResourceStatusUpdateHeader x2ResourceStatUpdHeader;
  ...
  cv2x_EpcX2Header x2Header;
  x2Header.SetMessageType (cv2x_EpcX2Header::InitiatingMessage);
  x2Header.SetProcedureCode (cv2x_EpcX2Header::ResourceStatusReporting);
...
void
cv2x_EpcX2::DoSendUeData (cv2x_EpcX2SapProvider::UeDataParams params)
{
  NS_LOG_FUNCTION (this);
  ...
  gtpu.SetLength (params.ueData->GetSize () + gtpu.GetSerializedSize () - 8); /// \todo This should be done in cv2x_GtpuHeader
