...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteAmc");

NS_OBJECT_ENSURE_REGISTERED (cv2x_LteAmc);
...
TypeId
cv2x_LteAmc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_LteAmc")
  .SetParent<Object> ()
  .SetGroupName("Lte")
  .AddConstructor<cv2x_LteAmc> ()
  .AddAttribute ("Ber",
                 "The requested BER in assigning MCS (default is 0.00005).",
                 DoubleValue (0.00005),
                 MakeDoubleAccessor (&cv2x_LteAmc::m_ber),
                 MakeDoubleChecker<double> ())
  .AddAttribute ("AmcModel",
                "AMC model used to assign CQI",
                 EnumValue (cv2x_LteAmc::MiErrorModel),
                 MakeEnumAccessor (&cv2x_LteAmc::m_amcModel),
                 MakeEnumChecker (cv2x_LteAmc::MiErrorModel, "Vienna",
                                  cv2x_LteAmc::PiroEW2010, "PiroEW2010",
                                  cv2x_LteAmc::NistErrorModel, "Nist"));
  return tid;
}


int
cv2x_LteAmc::GetCqiFromSpectralEfficiency (double s)
{
  NS_LOG_FUNCTION (s);
...
int
cv2x_LteAmc::GetMcsFromCqi (int cqi)
{
  NS_LOG_FUNCTION (cqi);
...
int
cv2x_LteAmc::GetDlTbSizeFromMcs (int mcs, int nprb)
{
  NS_LOG_FUNCTION (mcs);
...
int
cv2x_LteAmc::GetUlTbSizeFromMcs (int mcs, int nprb)
{
  NS_LOG_FUNCTION (mcs);
...
double
cv2x_LteAmc::GetSpectralEfficiencyFromCqi (int cqi)
{
  NS_LOG_FUNCTION (cqi);
  NS_ASSERT_MSG (cqi >= 0 && cqi <= 15, "CQI must be in [0..15] = " << cqi);
...
std::vector<int>
cv2x_LteAmc::CreateCqiFeedbacks (const SpectrumValue& sinr, uint8_t rbgSize)
{
  ...
  if (m_amcModel == PiroEW2010)
  ...
  else if (m_amcModel == MiErrorModel)
    {
      ...
      NS_ASSERT_MSG (rbgSize > 0, " cv2x_LteAmc-Vienna: RBG size must be greater than 0");
