...
#ifndef CV2X_LTE_ENB_RRC_H
#define CV2X_LTE_ENB_RRC_H
...
/**
   ...
   */
  class cv2x_LteEnbRrcSl : public Object
  {
  ...
  public:
    ...
    //  */
    // void SetSystemInformationBlockType18 (cv2x_LteRrcSap::SystemInformationBlockType18 sib18);

    /**
     * Gets the SIB 18 information
     * \return The SIB 18
     */
    cv2x_LteRrcSap::SystemInformationBlockType18 GetSystemInformationType18 ();
     ...
     */
    void AddPreconfiguredDedicatedPool (uint32_t group, cv2x_LteRrcSap::SlCommTxResourcesSetup pool);

    /**
     * Sets the pool to be used in exceptional cases
     * \param pool The pool information
     */
    void SetCommTxPoolExceptional (cv2x_LteRrcSap::SlCommTxPoolList pool);
    
  private:

    bool IsPoolInList (cv2x_LteRrcSap::SlCommResourcePool pool, cv2x_LteRrcSap::SlCommResourcePool *pools, int nbPool);
    ...
    //std::map <uint16_t, cv2x_LteRrcSap::SlCommTxResourcesSetup> m_dedicatedPoolMap;

    //Pre-provisioned dedicated pools using the group L2 address
    std::map <uint32_t, cv2x_LteRrcSap::SlCommTxResourcesSetup> m_preconfigDedicatedPoolMap;

    //Pre-provisioned pool for exceptional cases
    cv2x_LteRrcSap::SlCommTxPoolList m_preconfigCommTxPoolExceptional;
    
    ///NIST: System Information Block Type 18 currently broadcasted
    cv2x_LteRrcSap::SystemInformationBlockType18 m_sib18;
    ...
    struct ActivePoolInfo {
      ...
      cv2x_LteRrcSap::SlCommTxResourcesSetup m_poolSetup; //pool in a different format
      std::set<uint16_t> m_rntiSet; //list of UEs assigned to the pool
    };

    std::map <uint32_t, cv2x_LteEnbRrcSl::ActivePoolInfo> m_activePoolMap;
    
  }; //end of 'class cv2x_LteEnbRrcSl
 ...
 * single UE.
 */
class cv2x_UeManager : public Object
{
  /// allow cv2x_LtePdcpSpecificLtePdcpSapUser<cv2x_UeManager> class friend access
  friend class cv2x_LtePdcpSpecificLtePdcpSapUser<cv2x_UeManager>;

public:


  /**
   * The state of the cv2x_UeManager at the eNB RRC
   * 
   */
  enum State
  {
  ...
  /** 
   * cv2x_UeManager constructor
   * 
   * \param rrc pointer to the cv2x_LteEnbRrc holding this cv2x_UeManager
   * \param rnti RNTI of the UE
   * \param s initial state of the cv2x_UeManager
   * \param componentCarrierId primary component carrier ID
   * 
   * \return 
   */
  cv2x_UeManager (Ptr<cv2x_LteEnbRrc> rrc, uint16_t rnti, State s, uint8_t componentCarrierId);

  virtual ~cv2x_UeManager (void);
...
public: 
   ...
   * 
   */
  void SetupDataRadioBearer (cv2x_EpsBearer bearer, uint8_t bearerId, uint32_t gtpTeid, Ipv4Address transportLayerAddress);
   ...
   * \param params 
   */
  void RecvHandoverRequestAck (cv2x_EpcX2SapUser::HandoverRequestAckParams params);
   ...
   */
  cv2x_LteRrcSap::RadioResourceConfigDedicated GetRadioResourceConfigForHandoverPreparationInfo ();
   ...
   */
  cv2x_LteRrcSap::RrcConnectionReconfiguration GetRrcConnectionReconfigurationForHandover ();
   ...
   */
  std::vector<cv2x_EpcX2Sap::ErabToBeSetupItem> GetErabList ();
   ...
   */
  void RecvSnStatusTransfer (cv2x_EpcX2SapUser::SnStatusTransferParams params);
   ...
   * \param params the SN STATUS
   */
  void RecvUeContextRelease (cv2x_EpcX2SapUser::UeContextReleaseParams params); 


  // METHODS FORWARDED FROM ENB RRC SAP ///////////////////////////////////////

  /**
   * Implement the cv2x_LteEnbRrcSapProvider::CompleteSetupUe interface.
   * \param params CompleteSetupUeParameters
   */
  void CompleteSetupUe (cv2x_LteEnbRrcSapProvider::CompleteSetupUeParameters params);
  /**
   * Implement the cv2x_LteEnbRrcSapProvider::RecvRrcConnectionRequest interface.
   * \param msg the RRC connection request message
   */
  void RecvRrcConnectionRequest (cv2x_LteRrcSap::RrcConnectionRequest msg);
  /**
   * Implement the cv2x_LteEnbRrcSapProvider::RecvRrcConnectionSetupCompleted interface.
   * \param msg RRC connection setup completed message
   */
  void RecvRrcConnectionSetupCompleted (cv2x_LteRrcSap::RrcConnectionSetupCompleted msg);
  /**
   * Implement the cv2x_LteEnbRrcSapProvider::RecvRrcConnectionReconfigurationCompleted interface.
   * \param msg RRC connection reconfiguration completed message
   */
  void RecvRrcConnectionReconfigurationCompleted (cv2x_LteRrcSap::RrcConnectionReconfigurationCompleted msg);
  /**
   * Implement the cv2x_LteEnbRrcSapProvider::RecvRrcConnectionReestablishmentRequest interface.
   * \param msg the RRC connection reestablishment request message
   */
  void RecvRrcConnectionReestablishmentRequest (cv2x_LteRrcSap::RrcConnectionReestablishmentRequest msg);
  /**
   * Implement the cv2x_LteEnbRrcSapProvider::RecvRrcConnectionReestablishmentComplete interface.
   * \param msg the RRC connection reestablsihment complete message
   */
  void RecvRrcConnectionReestablishmentComplete (cv2x_LteRrcSap::RrcConnectionReestablishmentComplete msg);
  /**
   * Implement the cv2x_LteEnbRrcSapProvider::RecvMeasurementReport interface.
   * \param msg the measurement report
   */
  void RecvMeasurementReport (cv2x_LteRrcSap::MeasurementReport msg);
  /**
   * Part of the RRC protocol. Implement the cv2x_LteEnbRrcSapProvider::RecvSidelinkUeInformation interface.
   * \param msg the sidelink information message
   */
  void RecvSidelinkUeInformation (cv2x_LteRrcSap::SidelinkUeInformation msg);
   ...
   */
  void CmacUeConfigUpdateInd (cv2x_LteEnbCmacSapUser::UeConfig cmacParams);
...
private:
  ...
  /** 
   * Add a new cv2x_LteDataRadioBearerInfo structure to the cv2x_UeManager
   * 
   * \param radioBearerInfo 
   * 
   * \return the id of the newly added data radio bearer structure
   */
  uint8_t AddDataRadioBearerInfo (Ptr<cv2x_LteDataRadioBearerInfo> radioBearerInfo);

  /** 
   * \param drbid the Data Radio Bearer id
   * 
   * \return the corresponding cv2x_LteDataRadioBearerInfo
   */
  Ptr<cv2x_LteDataRadioBearerInfo> GetDataRadioBearerInfo (uint8_t drbid);

  /** 
   * remove the cv2x_LteDataRadioBearerInfo corresponding to a bearer being released
   *
   * \param drbid the Data Radio Bearer id
   */
   ...
   */
  cv2x_LteRrcSap::RrcConnectionReconfiguration BuildRrcConnectionReconfiguration ();
   ...
   * current configuration
   */
  cv2x_LteRrcSap::NonCriticalExtensionConfiguration BuildNonCriticalExtentionConfigurationCa ();
   ...
   * current configuration
   */
  cv2x_LteRrcSap::RadioResourceConfigDedicated BuildRadioResourceConfigDedicated ();
  ...
  /** 
   * Switch the cv2x_UeManager to the given state
   * 
  ...
  cv2x_LteRrcSap::PhysicalConfigDedicated m_physicalConfigDedicated; ///< physical config dedicated
  /// Pointer to the parent eNodeB RRC.
  Ptr<cv2x_LteEnbRrc> m_rrc;
  /// The current cv2x_UeManager state.
  State m_state;
   ...
   * seen by the cv2x_UeManager at the eNB RRC. Exporting IMSI, cell ID, RNTI, old
   * state, and new state.
   */
   ...
   * cv2x_LteEnbRrc::ConnectionRequestTimeout() when it expires. Cancelled when RRC
   * CONNECTION REQUEST is received.
   */
  EventId m_connectionRequestTimeout;
  /**
   * Time limit before a _connection setup timeout_ occurs. Set after an RRC
   * CONNECTION SETUP is sent. Calling cv2x_LteEnbRrc::ConnectionSetupTimeout() when
   ...
   * CONNECTION REJECT is sent. Calling cv2x_LteEnbRrc::ConnectionRejectedTimeout()
   * when it expires.
   */
   ...
   * cv2x_LteEnbRrc::HandoverJoiningTimeout() when it expires. Cancelled when
   ...
   * handover command is sent. Calling cv2x_LteEnbRrc::HandoverLeavingTimeout()
 ...
 */
class cv2x_LteEnbRrc : public Object
{

  /// allow cv2x_EnbRrcMemberLteEnbCmacSapUser class friend access
  friend class cv2x_EnbRrcMemberLteEnbCmacSapUser;
  /// allow cv2x_MemberLteHandoverManagementSapUser<cv2x_LteEnbRrc> class friend access
  friend class cv2x_MemberLteHandoverManagementSapUser<cv2x_LteEnbRrc>;
  /// allow cv2x_MemberLteAnrSapUser<cv2x_LteEnbRrc> class friend access
  friend class cv2x_MemberLteAnrSapUser<cv2x_LteEnbRrc>;
  /// allow cv2x_MemberLteFfrRrcSapUser<cv2x_LteEnbRrc> class friend access
  friend class cv2x_MemberLteFfrRrcSapUser<cv2x_LteEnbRrc>;
  /// allow cv2x_MemberLteEnbRrcSapProvider<cv2x_LteEnbRrc> class friend access
  friend class cv2x_MemberLteEnbRrcSapProvider<cv2x_LteEnbRrc>;
  /// allow cv2x_MemberLteEnbRrcSapProvider<cv2x_LteEnbRrc> class friend access
  friend class cv2x_MemberEpcEnbS1SapUser<cv2x_LteEnbRrc>;
  /// allow cv2x_MemberEpcEnbS1SapUser<cv2x_LteEnbRrc> class friend access
  friend class cv2x_EpcX2SpecificEpcX2SapUser<cv2x_LteEnbRrc>;
  /// allow cv2x_UeManager class friend access
  friend class cv2x_UeManager;
  /// allow  cv2x_MemberLteCcmRrcSapUser<cv2x_LteEnbRrc> class friend access
  friend class cv2x_MemberLteCcmRrcSapUser<cv2x_LteEnbRrc>;
...
public:
   ...
   */
  void SetLteEnbCmacSapProvider (cv2x_LteEnbCmacSapProvider * s);
   ...
   */
  void SetLteEnbCmacSapProvider (cv2x_LteEnbCmacSapProvider * s, uint8_t pos);

  /** 
   * Get the CMAC SAP offered by this RRC
   * \returns the CMAC SAP User interface offered to the MAC by this RRC
   */
  cv2x_LteEnbCmacSapUser* GetLteEnbCmacSapUser ();
   ...
   * \returns the CMAC SAP User interface offered to the MAC by this RRC
   */
  cv2x_LteEnbCmacSapUser* GetLteEnbCmacSapUser (uint8_t pos);
   ...
   */
  void SetLteHandoverManagementSapProvider (cv2x_LteHandoverManagementSapProvider * s);
   ...
   */
  cv2x_LteHandoverManagementSapUser* GetLteHandoverManagementSapUser ();
   ...
   */
  void SetLteCcmRrcSapProvider (cv2x_LteCcmRrcSapProvider * s);
   ...
   */
  cv2x_LteCcmRrcSapUser* GetLteCcmRrcSapUser ();
   ...
   */
  void SetLteAnrSapProvider (cv2x_LteAnrSapProvider * s);
   ...
   *           RRC
   */
  cv2x_LteAnrSapUser* GetLteAnrSapUser ();
   ...
   */
  void SetLteFfrRrcSapProvider (cv2x_LteFfrRrcSapProvider * s);
  /**
   * set the FFR SAP this RRC should interact with
   *
   * \param s the FFR SAP Provider to be used by this RRC
   ...
   */
  void SetLteFfrRrcSapProvider (cv2x_LteFfrRrcSapProvider * s, uint8_t index);
   ...
   *           RRC
   */
  cv2x_LteFfrRrcSapUser* GetLteFfrRrcSapUser ();
  /**
   * Get the FFR SAP offered by this RRC
   * \param index the index
   * \return s the FFR SAP User interface offered to the ANR instance by this
   *           RRC
   */
  cv2x_LteFfrRrcSapUser* GetLteFfrRrcSapUser (uint8_t index);
   ...
   */
  void SetLteEnbRrcSapUser (cv2x_LteEnbRrcSapUser * s);
   ...
   */
  cv2x_LteEnbRrcSapProvider* GetLteEnbRrcSapProvider ();
   ...
   */
  void SetLteEnbCphySapProvider (cv2x_LteEnbCphySapProvider * s);
   ...
   * \param pos the position
   */
  void SetLteEnbCphySapProvider (cv2x_LteEnbCphySapProvider * s, uint8_t pos);
   ...
   */
  cv2x_LteEnbCphySapUser* GetLteEnbCphySapUser ();
   ...
   * \return s the CPHY SAP User interface offered to the PHY by this RRC
   */
  cv2x_LteEnbCphySapUser* GetLteEnbCphySapUser (uint8_t pos);
   ...
   * 
   * \return true if the corresponding cv2x_UeManager instance exists
   */
   ...
   * \param rnti the identifier of an UE
   *
   * \return the corresponding cv2x_UeManager instance
   */
  Ptr<cv2x_UeManager> GetUeManager (uint16_t rnti);
   ...
   */
  uint8_t AddUeMeasReportConfig (cv2x_LteRrcSap::ReportConfigEutra config);
   ...
   *    `cv2x_LteEnbRrc::AddUeMeasReportConfig`;
   ...
   * `cv2x_LteHelper::InstallEnbDevice` (i.e. before the simulation starts).
   *
   * \warning Raises an error when executed more than once.
   *
   * \param ccPhyConf the component carrier configuration
   */
  void ConfigureCell (std::map<uint8_t, Ptr<cv2x_ComponentCarrierEnb>> ccPhyConf);

  /**
   * \brief Configure carriers.
   * \param ccPhyConf the component carrier configuration
   */
  void ConfigureCarriers (std::map<uint8_t, Ptr<cv2x_ComponentCarrierEnb>> ccPhyConf);
   ...
   */
  uint16_t cv2x_ComponentCarrierToCellId (uint8_t componentCarrierId);

  /** 
   * Enqueue an IP data packet on the proper bearer for downlink
   * transmission. Normally expected to be called by the NetDevice
   * forwarding a packet coming from the cv2x_EpcEnbApplication 
   * 
   * \param p the packet
   * 
   ...
   * \param [in] rnti
   * \param [in] report
   * \todo The \c cv2x_LteRrcSap::MeasurementReport argument should be
   * changed to a const reference since the argument is large.
   */
  typedef void (*ReceiveReportTracedCallback)
    (const uint64_t imsi, const uint16_t cellId, const uint16_t rnti,
     const cv2x_LteRrcSap::MeasurementReport report);
...
private:
  ...
  /**
   * Part of the RRC protocol. Forwarding cv2x_LteEnbRrcSapProvider::CompleteSetupUe interface to cv2x_UeManager::CompleteSetupUe
   * \param rnti the RNTI
   * \param params the cv2x_LteEnbRrcSapProvider::CompleteSetupUeParameters
   */
  void DoCompleteSetupUe (uint16_t rnti, cv2x_LteEnbRrcSapProvider::CompleteSetupUeParameters params);
  /**
   * Part of the RRC protocol. Forwarding cv2x_LteEnbRrcSapProvider::RecvRrcConnectionRequest interface to cv2x_UeManager::RecvRrcConnectionRequest
   *
   * \param rnti the RNTI
   * \param msg the cv2x_LteRrcSap::RrcConnectionRequest
   */
  void DoRecvRrcConnectionRequest (uint16_t rnti, cv2x_LteRrcSap::RrcConnectionRequest msg);
  /**
   * Part of the RRC protocol. Forwarding cv2x_LteEnbRrcSapProvider::RecvRrcConnectionSetupCompleted interface to cv2x_UeManager::RecvRrcConnectionSetupCompleted
   *
   * \param rnti the RNTI
   * \param msg the cv2x_LteRrcSap::RrcConnectionSetupCompleted
   */
  void DoRecvRrcConnectionSetupCompleted (uint16_t rnti, cv2x_LteRrcSap::RrcConnectionSetupCompleted msg);
  /**
   * Part of the RRC protocol. Forwarding cv2x_LteEnbRrcSapProvider::RecvRrcConnectionReconfigurationCompleted interface to cv2x_UeManager::RecvRrcConnectionReconfigurationCompleted
   *
   * \param rnti the RNTI
   * \param msg the cv2x_LteRrcSap::RrcConnectionReconfigurationCompleted
   */
  void DoRecvRrcConnectionReconfigurationCompleted (uint16_t rnti, cv2x_LteRrcSap::RrcConnectionReconfigurationCompleted msg);
  /**
   * Part of the RRC protocol. Forwarding cv2x_LteEnbRrcSapProvider::RecvRrcConnectionReestablishmentRequest interface to cv2x_UeManager::RecvRrcConnectionReestablishmentRequest
   *
   * \param rnti the RNTI
   * \param msg the cv2x_LteRrcSap::RrcConnectionReestablishmentRequest
   */
  void DoRecvRrcConnectionReestablishmentRequest (uint16_t rnti, cv2x_LteRrcSap::RrcConnectionReestablishmentRequest msg);
  /**
   * Part of the RRC protocol. Forwarding cv2x_LteEnbRrcSapProvider::RecvRrcConnectionReestablishmentComplete interface to cv2x_UeManager::RecvRrcConnectionReestablishmentComplete
   *
   * \param rnti the RNTI
   * \param msg the cv2x_LteRrcSap::RrcConnectionReestablishmentComplete
   */
  void DoRecvRrcConnectionReestablishmentComplete (uint16_t rnti, cv2x_LteRrcSap::RrcConnectionReestablishmentComplete msg);
  /**
   * Part of the RRC protocol. Forwarding cv2x_LteEnbRrcSapProvider::RecvMeasurementReport interface to cv2x_UeManager::RecvMeasurementReport
   *
   * \param rnti the RNTI
   * \param msg the cv2x_LteRrcSap::MeasurementReport
   */
  void DoRecvMeasurementReport (uint16_t rnti, cv2x_LteRrcSap::MeasurementReport msg);
  /**
   * Part of the RRC protocol. Forwarding cv2x_LteEnbRrcSapProvider::RecvSidelinkUeInformation interface to cv2x_UeManager::RecvSidelinkUeInformation.
   * \param rnti the RNTI
   * \param msg the cv2x_LteRrcSap::SidelinkUeInformation
   */
  void DoRecvSidelinkUeInformation (uint16_t rnti, cv2x_LteRrcSap::SidelinkUeInformation msg);
   ...
   *
   * \param params cv2x_EpcX2SapUser::HandoverRequestParams
   */
  void DoRecvHandoverRequest (cv2x_EpcX2SapUser::HandoverRequestParams params);
  /**
   * Receive handover request acknowledge function
   *
   * \param params cv2x_EpcX2SapUser::HandoverRequestAckParams
   */
  void DoRecvHandoverRequestAck (cv2x_EpcX2SapUser::HandoverRequestAckParams params);
  /**
   * Receive handover preparation failure function
   *
   * \param params cv2x_EpcX2SapUser::HandoverPreparationFailureParams
   */
  void DoRecvHandoverPreparationFailure (cv2x_EpcX2SapUser::HandoverPreparationFailureParams params);
  /**
   * Receive SN status transfer function
   *
   * \param params cv2x_EpcX2SapUser::SnStatusTransferParams
   */
  void DoRecvSnStatusTransfer (cv2x_EpcX2SapUser::SnStatusTransferParams params);
  /**
   * Receive UE context release function
   *
   * \param params cv2x_EpcX2SapUser::UeContextReleaseParams
   */
  void DoRecvUeContextRelease (cv2x_EpcX2SapUser::UeContextReleaseParams params);
  /**
   * Receive load information function
   *
   * \param params cv2x_EpcX2SapUser::LoadInformationParams
   */
  void DoRecvLoadInformation (cv2x_EpcX2SapUser::LoadInformationParams params);
  /**
   * Receive resource status update function
   *
   * \param params cv2x_EpcX2SapUser::ResourceStatusUpdateParams
   */
  void DoRecvResourceStatusUpdate (cv2x_EpcX2SapUser::ResourceStatusUpdateParams params);
  /**
   * Receive UE data function
   *
   * \param params cv2x_EpcX2SapUser::UeDataParams
   */
  void DoRecvUeData (cv2x_EpcX2SapUser::UeDataParams params);
   ...
   *
   * \param params cv2x_LteEnbCmacSapUser::UeConfig
   */
  void DoRrcConfigurationUpdateInd (cv2x_LteEnbCmacSapUser::UeConfig params);
   ...
   *
   * \param reportConfig cv2x_LteRrcSap::ReportConfigEutra
   * \returns measure ID
   */
  uint8_t DoAddUeMeasReportConfigForHandover (cv2x_LteRrcSap::ReportConfigEutra reportConfig);
  /**
   ...
   * \returns measure ID
   */
  uint8_t DoAddUeMeasReportConfigForComponentCarrier (cv2x_LteRrcSap::ReportConfigEutra reportConfig);
   ...
   * \returns measure ID
   */
  uint8_t DoAddUeMeasReportConfigForAnr (cv2x_LteRrcSap::ReportConfigEutra reportConfig);
   ...
   * \returns measure ID
   */
  uint8_t DoAddUeMeasReportConfigForFfr (cv2x_LteRrcSap::ReportConfigEutra reportConfig);
  /**
   * Set PDSCH config dedicated function
   *
   * \param rnti the RNTI
   * \param pa cv2x_LteRrcSap::PdschConfigDedicated
   */
  void DoSetPdschConfigDedicated (uint16_t rnti, cv2x_LteRrcSap::PdschConfigDedicated pa);
  /**
   * Send load information function
   *
   * \param params cv2x_EpcX2Sap::LoadInformationParams
   */
  void DoSendLoadInformation (cv2x_EpcX2Sap::LoadInformationParams params);
   ...
   *
   * \param state the initial state of the cv2x_UeManager
   * \param componentCarrierId primary component carrier ID of the cv2x_UeManager
   *
   * \return the newly allocated RNTI
   */
  uint16_t AddUe (cv2x_UeManager::State state, uint8_t componentCarrierId);
   ...
   */
  TypeId GetRlcType (cv2x_EpsBearer bearer);
...
private:
   ...
   */
  uint8_t GetLogicalChannelGroup (cv2x_EpsBearer bearer);
   ...
   */
  uint8_t GetLogicalChannelPriority (cv2x_EpsBearer bearer);
  ...
  cv2x_EpcX2SapUser* m_x2SapUser;
  /// Interface to send messages to neighbour eNodeB over the X2 interface.
  cv2x_EpcX2SapProvider* m_x2SapProvider;

  /// Receive API calls from the eNodeB MAC instance.
  std::vector<cv2x_LteEnbCmacSapUser*> m_cmacSapUser;
  /// Interface to the eNodeB MAC instance.
  std::vector<cv2x_LteEnbCmacSapProvider*> m_cmacSapProvider;

  /// Receive API calls from the handover algorithm instance.
  cv2x_LteHandoverManagementSapUser* m_handoverManagementSapUser;
  /// Interface to the handover algorithm instance.
  cv2x_LteHandoverManagementSapProvider* m_handoverManagementSapProvider;

  /// Receive API calls from the cv2x_LteEnbComponentCarrierManager instance.
  cv2x_LteCcmRrcSapUser* m_ccmRrcSapUser;
  /// Interface to the cv2x_LteEnbComponentCarrierManager instance.
  cv2x_LteCcmRrcSapProvider* m_ccmRrcSapProvider;

  /// Receive API calls from the ANR instance.
  cv2x_LteAnrSapUser* m_anrSapUser;
  /// Interface to the ANR instance.
  cv2x_LteAnrSapProvider* m_anrSapProvider;

  /// Receive API calls from the FFR algorithm instance.
  std::vector<cv2x_LteFfrRrcSapUser*> m_ffrRrcSapUser;
  /// Interface to the FFR algorithm instance.
  std::vector<cv2x_LteFfrRrcSapProvider*> m_ffrRrcSapProvider;

  /// Interface to send messages to UE over the RRC protocol.
  cv2x_LteEnbRrcSapUser* m_rrcSapUser;
  /// Interface to receive messages from UE over the RRC protocol.
  cv2x_LteEnbRrcSapProvider* m_rrcSapProvider;

  /// Interface to the eNodeB MAC instance, to be used by RLC instances.
  cv2x_LteMacSapProvider* m_macSapProvider;

  /// Interface to send messages to core network over the S1 protocol.
  cv2x_EpcEnbS1SapProvider* m_s1SapProvider;
  /// Interface to receive messages from core network over the S1 protocol.
  cv2x_EpcEnbS1SapUser* m_s1SapUser;

  /// Receive API calls from the eNodeB PHY instances.
  std::vector<cv2x_LteEnbCphySapUser*> m_cphySapUser;
  /// Interface to the eNodeB PHY instances.
  std::vector<cv2x_LteEnbCphySapProvider*> m_cphySapProvider;
  ...
  std::vector<cv2x_LteRrcSap::SystemInformationBlockType1> m_sib1;

  /**
   * The `UeMap` attribute. List of cv2x_UeManager by C-RNTI.
   */
  std::map<uint16_t, Ptr<cv2x_UeManager> > m_ueMap;

  /**
   * List of measurement configuration which are active in every UE attached to
   * this eNodeB instance.
   */
  cv2x_LteRrcSap::MeasConfig m_ueMeasConfig;
  ...
  /**
   * The `cv2x_EpsBearerToRlcMapping` attribute. Specify which type of RLC will be
   ...
   */
  TracedCallback<uint64_t, uint16_t, uint16_t, cv2x_LteRrcSap::MeasurementReport> m_recvMeasurementReportTrace;

  uint16_t m_numberOfComponentCarriers; ///< number of component carriers

  bool m_carriersConfigured; ///< are carriers configured

  std::map<uint8_t, Ptr<cv2x_ComponentCarrierEnb>> m_componentCarrierPhyConf; ///< component carrier phy configuration

  /**
   * Sidelink configuration
   */
  Ptr<cv2x_LteEnbRrcSl> m_sidelinkConfiguration;

}; // end of `class cv2x_LteEnbRrc`


} // namespace ns3

#endif // CV2X_LTE_ENB_RRC_H
