...
NS_LOG_COMPONENT_DEFINE ("cv2x_GtpuHeader");

/********************************************************
 *        GTP-U-v1 Header
 ********************************************************/

NS_OBJECT_ENSURE_REGISTERED (cv2x_GtpuHeader);

TypeId
cv2x_GtpuHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_GtpuHeader")
    .SetParent<Header> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_GtpuHeader> ();
  return tid;
}
cv2x_GtpuHeader::cv2x_GtpuHeader ()
  : m_version (1),
    m_protocolType (true),
...
TypeId
cv2x_GtpuHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

uint32_t
cv2x_GtpuHeader::GetSerializedSize (void) const
{
  return 12;
}
void
cv2x_GtpuHeader::Serialize (Buffer::Iterator start) const
{
...
uint32_t
cv2x_GtpuHeader::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;
...
void
cv2x_GtpuHeader::Print (std::ostream &os) const
{
...
bool
cv2x_GtpuHeader::GetExtensionHeaderFlag () const
{
  return m_extensionHeaderFlag;
}

uint16_t
cv2x_GtpuHeader::GetLength () const
{
  return m_length;
}

uint8_t
cv2x_GtpuHeader::GetMessageType () const
{
  return m_messageType;
}

uint8_t
cv2x_GtpuHeader::GetNPduNumber () const
{
  return m_nPduNumber;
}

bool
cv2x_GtpuHeader::GetNPduNumberFlag () const
{
  return m_nPduNumberFlag;
}

uint8_t
cv2x_GtpuHeader::GetNextExtensionType () const
{
  return m_nextExtensionType;
}

bool
cv2x_GtpuHeader::GetProtocolType () const
{
  return m_protocolType;
}

uint16_t
cv2x_GtpuHeader::GetSequenceNumber () const
{
  return m_sequenceNumber;
}

bool
cv2x_GtpuHeader::GetSequenceNumberFlag () const
{
  return m_sequenceNumberFlag;
}

uint32_t
cv2x_GtpuHeader::GetTeid () const
{
  return m_teid;
}

uint8_t
cv2x_GtpuHeader::GetVersion () const
{
  return m_version;
}

void
cv2x_GtpuHeader::SetExtensionHeaderFlag (bool m_extensionHeaderFlag)
{
  this->m_extensionHeaderFlag = m_extensionHeaderFlag;
}

void
cv2x_GtpuHeader::SetLength (uint16_t m_length)
{
  this->m_length = m_length;
}

void
cv2x_GtpuHeader::SetMessageType (uint8_t m_messageType)
{
  this->m_messageType = m_messageType;
}

void
cv2x_GtpuHeader::SetNPduNumber (uint8_t m_nPduNumber)
{
  this->m_nPduNumber = m_nPduNumber;
}

void
cv2x_GtpuHeader::SetNPduNumberFlag (bool m_nPduNumberFlag)
{
  this->m_nPduNumberFlag = m_nPduNumberFlag;
}

void
cv2x_GtpuHeader::SetNextExtensionType (uint8_t m_nextExtensionType)
{
  this->m_nextExtensionType = m_nextExtensionType;
}

void
cv2x_GtpuHeader::SetProtocolType (bool m_protocolType)
{
  this->m_protocolType = m_protocolType;
}

void
cv2x_GtpuHeader::SetSequenceNumber (uint16_t m_sequenceNumber)
{
  this->m_sequenceNumber = m_sequenceNumber;
}

void
cv2x_GtpuHeader::SetSequenceNumberFlag (bool m_sequenceNumberFlag)
{
  this->m_sequenceNumberFlag = m_sequenceNumberFlag;
}

void
cv2x_GtpuHeader::SetTeid (uint32_t m_teid)
{
  this->m_teid = m_teid;
}

void
cv2x_GtpuHeader::SetVersion (uint8_t m_version)
{
...
bool
cv2x_GtpuHeader::operator == (const cv2x_GtpuHeader& b) const
{
