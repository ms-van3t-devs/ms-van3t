...
cv2x_LteHarqPhy::cv2x_LteHarqPhy ()
{
  ...
  std::vector <cv2x_HarqProcessInfoList_t> dlHarqLayer0;
  dlHarqLayer0.resize (8);
  std::vector <cv2x_HarqProcessInfoList_t> dlHarqLayer1;
...
cv2x_LteHarqPhy::SubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  ...
  std::map <uint16_t, std::vector <cv2x_HarqProcessInfoList_t> >::iterator it;
  for (it = m_miUlHarqProcessesInfoMap.begin (); it != m_miUlHarqProcessesInfoMap.end (); it++)
    {
      (*it).second.erase ((*it).second.begin ());
      cv2x_HarqProcessInfoList_t h;
...
cv2x_LteHarqPhy::GetAccumulatedMiDl (uint8_t harqProcId, uint8_t layer)
{
  ...
  cv2x_HarqProcessInfoList_t list = m_miDlHarqProcessesInfoMap.at (layer).at (harqProcId);
  double mi = 0.0;
...
cv2x_LteHarqPhy::GetAccumulatedMiUl (uint16_t rnti)
{
  ...
  cv2x_HarqProcessInfoList_t list = (*it).second.at (0);
  double mi = 0.0;
  for (uint8_t i = 0; i < list.size (); i++)
    {
      mi += list.at (i).m_mi;
...
cv2x_LteHarqPhy::GetHarqProcessInfoUl (uint16_t rnti, uint8_t harqProcId)
{
  ...
  if (it==m_miUlHarqProcessesInfoMap.end ())
    {
      // new entry
      std::vector <cv2x_HarqProcessInfoList_t> harqList;
      harqList.resize (8);
      m_miUlHarqProcessesInfoMap.insert (std::pair <uint16_t, std::vector <cv2x_HarqProcessInfoList_t> > (rnti, harqList));
...
cv2x_LteHarqPhy::GetHarqProcessInfoSl (uint16_t rnti, uint8_t l1dst)
{
  ...
  if (it==m_miSlHarqProcessesInfoMap.end ())
    {
      // new entry
      cv2x_HarqProcessInfoList_t harqInfo;
      std::map <uint8_t, cv2x_HarqProcessInfoList_t> map;
      map.insert (std::pair <uint8_t, cv2x_HarqProcessInfoList_t> (l1dst, harqInfo));
      m_miSlHarqProcessesInfoMap.insert (std::pair <uint16_t, std::map <uint8_t, cv2x_HarqProcessInfoList_t> > (rnti, map));
      return (harqInfo);
    }
  else
    {
      ...
      if (it2==it->second.end ())
        {
          cv2x_HarqProcessInfoList_t harqInfo;
          std::map <uint8_t, cv2x_HarqProcessInfoList_t> map;
          it->second.insert (std::pair <uint8_t, cv2x_HarqProcessInfoList_t> (l1dst, harqInfo));
          return (harqInfo);
        }
      else
        {
...
{
  NS_LOG_FUNCTION (this << rnti);
  std::map <uint16_t, cv2x_HarqProcessInfoList_t>::iterator it;
  it = m_miSlV2XHarqProcessesInfoMap.find (rnti);

  // new entry
  cv2x_HarqProcessInfoList_t harqInfo;
  m_miSlV2XHarqProcessesInfoMap.insert (std::pair <uint16_t, cv2x_HarqProcessInfoList_t> (rnti, harqInfo));
  return (harqInfo);
}
  ...
  if (it==m_miDiscHarqProcessesInfoMap.end ())
    {
      ...
      cv2x_HarqProcessInfoList_t harqInfo;
      std::map <uint8_t, cv2x_HarqProcessInfoList_t> map;
      map.insert (std::pair <uint8_t, cv2x_HarqProcessInfoList_t> (resPsdch, harqInfo));
      m_miDiscHarqProcessesInfoMap.insert (std::pair <uint16_t, std::map <uint8_t, cv2x_HarqProcessInfoList_t> > (rnti, map));
      return (harqInfo);
    }
  else
    {
      ...
      if (it2==it->second.end ())
        {
          cv2x_HarqProcessInfoList_t harqInfo;
          std::map <uint8_t, cv2x_HarqProcessInfoList_t> map;
          it->second.insert (std::pair <uint8_t, cv2x_HarqProcessInfoList_t> (resPsdch, harqInfo));
          return (harqInfo);
        }
      else
        {
