...
#include "cv2x_lte-anr.h"
#include <ns3/log.h>
#include <ns3/uinteger.h>

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("cv2x_LteAnr");

NS_OBJECT_ENSURE_REGISTERED (cv2x_LteAnr);


cv2x_LteAnr::cv2x_LteAnr (uint16_t servingCellId)
  : m_anrSapUser (0),
    m_threshold (0),
    m_measId (0),
    m_servingCellId (servingCellId)
{
  ...
  m_anrSapProvider = new cv2x_MemberLteAnrSapProvider<cv2x_LteAnr> (this);
}


cv2x_LteAnr::~cv2x_LteAnr ()
{
...
TypeId
cv2x_LteAnr::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::cv2x_LteAnr")
    .SetParent<Object> ()
    .SetGroupName("Lte")
    .AddAttribute ("Threshold",
                   "Minimum RSRQ range value required for detecting a neighbour cell",
                   UintegerValue (0),
                   MakeUintegerAccessor (&cv2x_LteAnr::m_threshold),
...
void
cv2x_LteAnr::AddNeighbourRelation (uint16_t cellId)
{
...
void
cv2x_LteAnr::RemoveNeighbourRelation (uint16_t cellId)
{
  NS_LOG_FUNCTION (this << m_servingCellId << cellId);
...
void
cv2x_LteAnr::SetLteAnrSapUser (cv2x_LteAnrSapUser* s)
{
...
cv2x_LteAnrSapProvider*
cv2x_LteAnr::GetLteAnrSapProvider ()
{
...
cv2x_LteAnr::DoInitialize ()
{
  ...
  reportConfig.eventId = cv2x_LteRrcSap::ReportConfigEutra::EVENT_A4;
  ...
  reportConfig.reportInterval = cv2x_LteRrcSap::ReportConfigEutra::MS480;
  m_measId = m_anrSapUser->AddUeMeasReportConfigForAnr (reportConfig);
}


void
cv2x_LteAnr::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_LteAnr::DoReportUeMeas (cv2x_LteRrcSap::MeasResults measResults)
{
...
void
cv2x_LteAnr::DoAddNeighbourRelation (uint16_t cellId)
{
...
bool
cv2x_LteAnr::DoGetNoRemove (uint16_t cellId) const
{
  NS_LOG_FUNCTION (this << m_servingCellId << cellId);
...
bool
cv2x_LteAnr::DoGetNoHo (uint16_t cellId) const
{
  NS_LOG_FUNCTION (this << m_servingCellId << cellId);
...
bool
cv2x_LteAnr::DoGetNoX2 (uint16_t cellId) const
{
  NS_LOG_FUNCTION (this << m_servingCellId << cellId);
...
const cv2x_LteAnr::NeighbourRelation_t *
cv2x_LteAnr::Find (uint16_t cellId) const
{
