...
#include "cv2x_no-op-component-carrier-manager.h"
#include <ns3/log.h>
#include <ns3/random-variable-stream.h>

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("cv2x_NoOpComponentCarrierManager");
NS_OBJECT_ENSURE_REGISTERED (cv2x_NoOpComponentCarrierManager);
  
cv2x_NoOpComponentCarrierManager::cv2x_NoOpComponentCarrierManager ()
{
  NS_LOG_FUNCTION (this);
  m_ccmRrcSapProvider = new cv2x_MemberLteCcmRrcSapProvider<cv2x_NoOpComponentCarrierManager> (this);
  m_ccmMacSapUser = new cv2x_MemberLteCcmMacSapUser<cv2x_NoOpComponentCarrierManager> (this);
  m_macSapProvider = new cv2x_EnbMacMemberLteMacSapProvider <cv2x_NoOpComponentCarrierManager> (this);
  m_ccmRrcSapUser  = 0;
}
...
void
cv2x_NoOpComponentCarrierManager::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
TypeId
cv2x_NoOpComponentCarrierManager::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::cv2x_NoOpComponentCarrierManager")
          .SetParent<cv2x_LteEnbComponentCarrierManager> ()
          .SetGroupName("Lte")
          .AddConstructor<cv2x_NoOpComponentCarrierManager> ()
          ;
  return tid;
}
...
void
cv2x_NoOpComponentCarrierManager::DoInitialize ()
{
  NS_LOG_FUNCTION (this);
  cv2x_LteEnbComponentCarrierManager::DoInitialize ();
}
...
void
cv2x_NoOpComponentCarrierManager::DoTransmitPdu (cv2x_LteMacSapProvider::TransmitPduParameters params)
{
  NS_LOG_FUNCTION (this);
  std::map <uint8_t, cv2x_LteMacSapProvider*>::iterator it =  m_macSapProvidersMap.find (params.componentCarrierId);
  NS_ASSERT_MSG (it != m_macSapProvidersMap.end (), "could not find Sap for cv2x_ComponentCarrier " << params.componentCarrierId);
  // with this algorithm all traffic is on Primary Carrier
  it->second->TransmitPdu (params);
}

void
cv2x_NoOpComponentCarrierManager::DoReportBufferStatus (cv2x_LteMacSapProvider::ReportBufferStatusParameters params)
{
  NS_LOG_FUNCTION (this);
  auto ueManager = m_ccmRrcSapUser->GetUeManager (params.rnti);
  std::map <uint8_t, cv2x_LteMacSapProvider*>::iterator it = m_macSapProvidersMap.find (ueManager->GetComponentCarrierId ());
...
void
cv2x_NoOpComponentCarrierManager::DoNotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId, uint8_t componentCarrierId, uint16_t rnti, uint8_t lcid)
{
  NS_LOG_FUNCTION (this);
  std::map <uint16_t, std::map<uint8_t, cv2x_LteMacSapUser*> >::iterator rntiIt = m_ueAttached.find (rnti);
...
void
cv2x_NoOpComponentCarrierManager::DoReceivePdu (Ptr<Packet> p, uint16_t rnti, uint8_t lcid)
{
  NS_LOG_FUNCTION (this);
  std::map <uint16_t, std::map<uint8_t, cv2x_LteMacSapUser*> >::iterator rntiIt = m_ueAttached.find (rnti);
  NS_ASSERT_MSG (rntiIt != m_ueAttached.end (), "could not find RNTI" << rnti);
...
void
cv2x_NoOpComponentCarrierManager::DoNotifyHarqDeliveryFailure ()
{
  NS_LOG_FUNCTION (this);
}

void
cv2x_NoOpComponentCarrierManager::DoReportUeMeas (uint16_t rnti,
...
void
cv2x_NoOpComponentCarrierManager::DoAddUe (uint16_t rnti, uint8_t state)
{
  ...
  if (stateIt == m_ueState.end ())
    {
      ...
      // preparing the rnti,lcid,cv2x_LteMacSapUser map
      std::map<uint8_t, cv2x_LteMacSapUser*> empty;
      std::pair <std::map <uint16_t, std::map<uint8_t, cv2x_LteMacSapUser*> >::iterator, bool>
      ret = m_ueAttached.insert (std::pair <uint16_t,  std::map<uint8_t, cv2x_LteMacSapUser*> >
      (rnti, empty));
      NS_LOG_DEBUG (this << "AddUe: UE Pointer cv2x_LteMacSapUser Map " << rnti << " added " << (uint16_t) ret.second);
      NS_ASSERT_MSG (ret.second, "element already present, RNTI already existed");

      //add new rnti in the map
      std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> emptyA;
      std::pair <std::map <uint16_t, std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> >::iterator, bool>
      retA = m_rlcLcInstantiated.insert (std::pair <uint16_t,  std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> >
      (rnti, emptyA));
...
void
cv2x_NoOpComponentCarrierManager::DoAddLc (cv2x_LteEnbCmacSapProvider::LcInfo lcInfo, cv2x_LteMacSapUser* msu)
{
  NS_LOG_FUNCTION (this);
  ...
  m_rlcLcInstantiated.find(lcInfo.rnti)->second.insert(std::pair <uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> (lcInfo.lcId, lcInfo));
}


void
cv2x_NoOpComponentCarrierManager::DoRemoveUe (uint16_t rnti)
{
  NS_LOG_FUNCTION (this);
  std::map<uint16_t, uint8_t>::iterator stateIt;
  std::map<uint16_t, uint8_t>::iterator eccIt; // m_enabledComponentCarrier iterator
...
std::vector<cv2x_LteCcmRrcSapProvider::LcsConfig>
cv2x_NoOpComponentCarrierManager::DoSetupDataRadioBearer (cv2x_EpsBearer bearer, uint8_t bearerId, uint16_t rnti, uint8_t lcid, uint8_t lcGroup, cv2x_LteMacSapUser *msu)
{
  ...
  std::vector<cv2x_LteCcmRrcSapProvider::LcsConfig> res;
  cv2x_LteCcmRrcSapProvider::LcsConfig entry;
  ...
  for (uint16_t ncc = 0; ncc < m_noOfComponentCarriers; ncc++)
    {
      ...
      cv2x_LteEnbCmacSapProvider::LcInfo lci;
      lci.rnti = rnti;
      lci.lcId = lcid;
      lci.lcGroup = lcGroup;
      lci.qci = bearer.qci;
  ...
  std::map <uint16_t, std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> >::iterator rntiIter = m_rlcLcInstantiated.find (rnti);
  ...
  //   {
  //     //add new rnti in the map
  //     std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> empty;
  //     std::pair <std::map <uint16_t, std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> >::iterator, bool>
  //       ret = m_rlcLcInstantiated.insert (std::pair <uint16_t,  std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> >
  //                                         (rnti, empty));
  //     NS_LOG_DEBUG (this << " UE " << rnti << " added " << (uint16_t) ret.second);
  //   }

  std::map <uint16_t, std::map<uint8_t, cv2x_LteMacSapUser*> >::iterator sapIt =  m_ueAttached.find (rnti);
  NS_ASSERT_MSG (sapIt != m_ueAttached.end (), "RNTI not found");
  rntiIter = m_rlcLcInstantiated.find (rnti);
  std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo>::iterator lcidIt = rntiIter->second.find (lcid);
  //std::map<uint8_t, cv2x_LteMacSapUser*>::iterator lcidIt = sapIt->second.find (lcinfo.lcId);
  ...
  if (lcidIt == rntiIter->second.end ())
    {
      ...
      rntiIter->second.insert (std::pair<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> (lcinfo.lcId, lcinfo));
      sapIt->second.insert (std::pair<uint8_t, cv2x_LteMacSapUser*> (lcinfo.lcId, msu));
    }
  else
    {
...
std::vector<uint8_t>
cv2x_NoOpComponentCarrierManager::DoReleaseDataRadioBearer (uint16_t rnti, uint8_t lcid)
{
  NS_LOG_FUNCTION (this);
  ...
  std::map <uint16_t, std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> >::iterator lcsIt;
  ...
  std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo>::iterator lcIt;
  ...
  std::map <uint16_t, std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo> >::iterator rlcInstancesIt = m_rlcLcInstantiated.find (rnti);
  std::map<uint8_t, cv2x_LteEnbCmacSapProvider::LcInfo>::iterator rclLcIt;
...
cv2x_LteMacSapUser*
cv2x_NoOpComponentCarrierManager::DoConfigureSignalBearer(cv2x_LteEnbCmacSapProvider::LcInfo lcinfo,  cv2x_LteMacSapUser* msu)
{
  NS_LOG_FUNCTION (this);
  std::map <uint16_t, std::map<uint8_t, cv2x_LteMacSapUser*> >::iterator itSapUserAtCcm;
  ...
  std::map <uint16_t, std::map<uint8_t, cv2x_LteMacSapUser*> >::iterator rntiIt = m_ueAttached.find (lcinfo.rnti);
...
void
cv2x_NoOpComponentCarrierManager::DoNotifyPrbOccupancy (double prbOccupancy, uint8_t componentCarrierId)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_NoOpComponentCarrierManager::DoUlReceiveMacCe (cv2x_MacCeListElement_s bsr, uint8_t componentCarrierId)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (bsr.m_macCeType == cv2x_MacCeListElement_s::BSR, "Received a Control Message not allowed " << bsr.m_macCeType);
  if ( bsr.m_macCeType == cv2x_MacCeListElement_s::BSR)
    {
      cv2x_MacCeListElement_s newBsr;
      ...
      for (uint16_t i = 0; i < 4; i++)
        {
          ...
          newBsr.m_macCeValue.m_bufferStatus.at (i) = cv2x_BufferSizeLevelBsr::BufferSize2BsrId (buffer);
        }
...
NS_OBJECT_ENSURE_REGISTERED (cv2x_RrComponentCarrierManager);

cv2x_RrComponentCarrierManager::cv2x_RrComponentCarrierManager ()
{
  NS_LOG_FUNCTION (this);
...
TypeId
cv2x_RrComponentCarrierManager::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::cv2x_RrComponentCarrierManager")
                .SetParent<cv2x_NoOpComponentCarrierManager> ()
                .SetGroupName("Lte")
                .AddConstructor<cv2x_RrComponentCarrierManager> ()
                ;
  return tid;
}
...
void
cv2x_RrComponentCarrierManager::DoReportBufferStatus (cv2x_LteMacSapProvider::ReportBufferStatusParameters params)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_RrComponentCarrierManager::DoUlReceiveMacCe (cv2x_MacCeListElement_s bsr, uint8_t componentCarrierId)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (componentCarrierId == 0, "Received BSR from a cv2x_ComponentCarrier not allowed, cv2x_ComponentCarrierId = " << componentCarrierId);
  NS_ASSERT_MSG (bsr.m_macCeType == cv2x_MacCeListElement_s::BSR, "Received a Control Message not allowed " << bsr.m_macCeType);
      ...
      for (uint16_t i = 0; i < 4; i++)
        {
          ...
          uint32_t bufferSize = cv2x_BufferSizeLevelBsr::BsrId2BufferSize (bsrStatusId);
          // here the buffer should be divide among the different sap
          // since the buffer status report are compressed information
          // it is needed to use BsrId2BufferSize to uncompress
          // after the split over all component carriers is is needed to
          // compress again the information to fit cv2x_MacCeListElement_s structure
          // verify how many Component Carrier are enabled per UE
          newBsr.m_macCeValue.m_bufferStatus.at(i) = cv2x_BufferSizeLevelBsr::BufferSize2BsrId (bufferSize/numberOfCarriersForUe);
        }
