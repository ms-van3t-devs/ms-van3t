...
#include "cv2x_a2-a4-rsrq-handover-algorithm.h"
#include <ns3/log.h>
#include <ns3/uinteger.h>

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("cv2x_A2A4RsrqHandoverAlgorithm");

NS_OBJECT_ENSURE_REGISTERED (cv2x_A2A4RsrqHandoverAlgorithm);
...
cv2x_A2A4RsrqHandoverAlgorithm::cv2x_A2A4RsrqHandoverAlgorithm ()
  : m_a2MeasId (0),
    m_a4MeasId (0),
    ...
    m_handoverManagementSapUser (0)
{
  ...
  m_handoverManagementSapProvider = new cv2x_MemberLteHandoverManagementSapProvider<cv2x_A2A4RsrqHandoverAlgorithm> (this);
}
...
TypeId
cv2x_A2A4RsrqHandoverAlgorithm::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::cv2x_A2A4RsrqHandoverAlgorithm")
    .SetParent<cv2x_LteHandoverAlgorithm> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_A2A4RsrqHandoverAlgorithm> ()
    .AddAttribute ("ServingCellThreshold",
                   ...
                   UintegerValue (30),
                   MakeUintegerAccessor (&cv2x_A2A4RsrqHandoverAlgorithm::m_servingCellThreshold),
    ...
    .AddAttribute ("NeighbourCellOffset",
                   ...
                   UintegerValue (1),
                   MakeUintegerAccessor (&cv2x_A2A4RsrqHandoverAlgorithm::m_neighbourCellOffset),
...
void
cv2x_A2A4RsrqHandoverAlgorithm::SetLteHandoverManagementSapUser (cv2x_LteHandoverManagementSapUser* s)
{
...
cv2x_A2A4RsrqHandoverAlgorithm::GetLteHandoverManagementSapProvider ()
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_A2A4RsrqHandoverAlgorithm::DoInitialize ()
{
  NS_LOG_FUNCTION (this);

  NS_LOG_LOGIC (this << " requesting Event A2 measurements"
                     << " (threshold=" << (uint16_t) m_servingCellThreshold << ")");
  cv2x_LteRrcSap::ReportConfigEutra reportConfigA2;
  reportConfigA2.eventId = cv2x_LteRrcSap::ReportConfigEutra::EVENT_A2;
  reportConfigA2.threshold1.choice = cv2x_LteRrcSap::ThresholdEutra::THRESHOLD_RSRQ;
  reportConfigA2.threshold1.range = m_servingCellThreshold;
  reportConfigA2.triggerQuantity = cv2x_LteRrcSap::ReportConfigEutra::RSRQ;
  reportConfigA2.reportInterval = cv2x_LteRrcSap::ReportConfigEutra::MS240;
  ...
  reportConfigA4.threshold1.choice = cv2x_LteRrcSap::ThresholdEutra::THRESHOLD_RSRQ;
  reportConfigA4.threshold1.range = 0; // intentionally very low threshold
  reportConfigA4.triggerQuantity = cv2x_LteRrcSap::ReportConfigEutra::RSRQ;
  reportConfigA4.reportInterval = cv2x_LteRrcSap::ReportConfigEutra::MS480;
...
void
cv2x_A2A4RsrqHandoverAlgorithm::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_A2A4RsrqHandoverAlgorithm::DoReportUeMeas (uint16_t rnti,
...
void
cv2x_A2A4RsrqHandoverAlgorithm::EvaluateHandover (uint16_t rnti,
...
bool
cv2x_A2A4RsrqHandoverAlgorithm::IsValidNeighbour (uint16_t cellId)
{
...
void
cv2x_A2A4RsrqHandoverAlgorithm::UpdateNeighbourMeasurements (uint16_t rnti,
                                                        uint16_t cellId,
                                                        uint8_t rsrq)
{
