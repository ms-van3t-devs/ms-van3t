...
#ifndef CV2X_EPS_BEARER
#define CV2X_EPS_BEARER
 ...
 *
 */
struct cv2x_AllocationRetentionPriority
{
  /** 
   * Default constructor, initializes member variables to zero or equivalent
   */
  cv2x_AllocationRetentionPriority ();
...
struct cv2x_EpsBearer
{
  ...
  cv2x_GbrQosInformation gbrQosInfo; ///< GBR QOS information
  cv2x_AllocationRetentionPriority arp; ///< allocation retention priority
   ...
   *
   */
  cv2x_EpsBearer (Qci x);

  /**
   *
   * @param x the QoS Class Indicator
   * @param y the cv2x_GbrQosInformation
   *
   */
  cv2x_EpsBearer (Qci x, cv2x_GbrQosInformation y);
