...
#ifndef CV2X_NIST_INDOOR_PROPAGATION_LOSS_MODEL_H
#define CV2X_NIST_INDOOR_PROPAGATION_LOSS_MODEL_H
 ...
 * \ingroup nist
 *
 * \brief Indoor propagation model for the 700 MHz frequency (Public Safety use cases)
 * 
 * This class implements the indoor propagation model for 700 MHz based on 3GPP sepcifications:
 * 3GPP TR 36.843 V12.0.1 (2014-03) / Section A.2.1.2 
 */
class cv2x_NistIndoorPropagationLossModel : public PropagationLossModel
{
...
public:
  ...
  cv2x_NistIndoorPropagationLossModel ();
  virtual ~cv2x_NistIndoorPropagationLossModel ();
   ...
   */
  std::pair<double, bool> GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const;
...
#endif // CV2X_NIST_INDOOR_PROPAGATION_LOSS_MODEL_H
