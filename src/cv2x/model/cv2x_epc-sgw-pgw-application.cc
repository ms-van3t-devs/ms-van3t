...
#include "cv2x_epc-sgw-pgw-application.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("cv2x_EpcSgwPgwApplication");
...
cv2x_EpcSgwPgwApplication::UeInfo::UeInfo ()
{
  NS_LOG_FUNCTION (this);
}

void
cv2x_EpcSgwPgwApplication::UeInfo::AddBearer (Ptr<cv2x_EpcTft> tft, uint8_t bearerId, uint32_t teid)
{
...
void
cv2x_EpcSgwPgwApplication::UeInfo::RemoveBearer (uint8_t bearerId)
{
...
uint32_t
cv2x_EpcSgwPgwApplication::UeInfo::Classify (Ptr<Packet> p)
{
  ...
  return m_tftClassifier.Classify (p, cv2x_EpcTft::DOWNLINK);
}

Ipv4Address 
cv2x_EpcSgwPgwApplication::UeInfo::GetEnbAddr ()
{
  return m_enbAddr;
}

void
cv2x_EpcSgwPgwApplication::UeInfo::SetEnbAddr (Ipv4Address enbAddr)
{
  m_enbAddr = enbAddr;
}

Ipv4Address 
cv2x_EpcSgwPgwApplication::UeInfo::GetUeAddr ()
{
  return m_ueAddr;
}

void
cv2x_EpcSgwPgwApplication::UeInfo::SetUeAddr (Ipv4Address ueAddr)
{
  m_ueAddr = ueAddr;
}

Ipv6Address 
cv2x_EpcSgwPgwApplication::UeInfo::GetUeAddr6 ()
{
  return m_ueAddr6;
}

void
cv2x_EpcSgwPgwApplication::UeInfo::SetUeAddr6 (Ipv6Address ueAddr)
{
  m_ueAddr6 = ueAddr;
}

/////////////////////////
// cv2x_EpcSgwPgwApplication
/////////////////////////


TypeId
cv2x_EpcSgwPgwApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_EpcSgwPgwApplication")
    .SetParent<Object> ()
    .SetGroupName("Lte")
    .AddTraceSource ("RxFromTun",
                     "Receive data packets from internet in Tunnel net device",
                     MakeTraceSourceAccessor (&cv2x_EpcSgwPgwApplication::m_rxTunPktTrace),
                     "ns3::cv2x_EpcSgwPgwApplication::RxTracedCallback")
    .AddTraceSource ("RxFromS1u",
                     "Receive data packets from S1 U Socket",
                     MakeTraceSourceAccessor (&cv2x_EpcSgwPgwApplication::m_rxS1uPktTrace),
                     "ns3::cv2x_EpcSgwPgwApplication::RxTracedCallback")
    ;
  return tid;
}
...
void
cv2x_EpcSgwPgwApplication::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
cv2x_EpcSgwPgwApplication::cv2x_EpcSgwPgwApplication (const Ptr<VirtualNetDevice> tunDevice, const Ptr<Socket> s1uSocket)
    ...
    m_s11SapMme (0)
{
  ...
  m_s1uSocket->SetRecvCallback (MakeCallback (&cv2x_EpcSgwPgwApplication::RecvFromS1uSocket, this));
  m_s11SapSgw = new cv2x_MemberEpcS11SapSgw<cv2x_EpcSgwPgwApplication> (this);
}

  
cv2x_EpcSgwPgwApplication::~cv2x_EpcSgwPgwApplication ()
{
  NS_LOG_FUNCTION (this);
}
...
bool
cv2x_EpcSgwPgwApplication::RecvFromTunDevice (Ptr<Packet> packet, const Address& source, const Address& dest, uint16_t protocolNumber)
{
  ...
  if (ipType == 0x04)
    ...
    {
      NS_ABORT_MSG ("cv2x_EpcSgwPgwApplication::RecvFromTunDevice - Unknown IP type...");
    }
...
void 
cv2x_EpcSgwPgwApplication::RecvFromS1uSocket (Ptr<Socket> socket)
{
...
void 
cv2x_EpcSgwPgwApplication::SendToTunDevice (Ptr<Packet> packet, uint32_t teid)
{
  ...
  if (ipType == 0x04)
    ...
    {
      NS_ABORT_MSG ("cv2x_EpcSgwPgwApplication::SendToTunDevice - Unknown IP type...");
    }
}

void 
cv2x_EpcSgwPgwApplication::SendToS1uSocket (Ptr<Packet> packet, Ipv4Address enbAddr, uint32_t teid)
{
...
void 
cv2x_EpcSgwPgwApplication::SetS11SapMme (cv2x_EpcS11SapMme * s)
{
  m_s11SapMme = s;
}

cv2x_EpcS11SapSgw* 
cv2x_EpcSgwPgwApplication::GetS11SapSgw ()
{
  return m_s11SapSgw;
}

void 
cv2x_EpcSgwPgwApplication::AddEnb (uint16_t cellId, Ipv4Address enbAddr, Ipv4Address sgwAddr)
{
...
void 
cv2x_EpcSgwPgwApplication::AddUe (uint64_t imsi)
{
...
void 
cv2x_EpcSgwPgwApplication::SetUeAddress (uint64_t imsi, Ipv4Address ueAddr)
{
...
void 
cv2x_EpcSgwPgwApplication::SetUeAddress6 (uint64_t imsi, Ipv6Address ueAddr)
{
  NS_LOG_FUNCTION (this << imsi << ueAddr);
  std::map<uint64_t, Ptr<UeInfo> >::iterator ueit = m_ueInfoByImsiMap.find (imsi);
  NS_ASSERT_MSG (ueit != m_ueInfoByImsiMap.end (), "unknown IMSI " << imsi); 
...
void 
cv2x_EpcSgwPgwApplication::DoCreateSessionRequest (cv2x_EpcS11SapSgw::CreateSessionRequestMessage req)
{
  ...
  cv2x_EpcS11SapMme::CreateSessionResponseMessage res;
  res.teid = req.imsi; // trick to avoid the need for allocating TEIDs on the S11 interface

  for (std::list<cv2x_EpcS11SapSgw::BearerContextToBeCreated>::iterator bit = req.bearerContextsToBeCreated.begin ();
       ...
       ++bit)
    {
      ...
      cv2x_EpcS11SapMme::BearerContextCreated bearerContext;
...
void 
cv2x_EpcSgwPgwApplication::DoModifyBearerRequest (cv2x_EpcS11SapSgw::ModifyBearerRequestMessage req)
{
  ...
  cv2x_EpcS11SapMme::ModifyBearerResponseMessage res;
  res.teid = imsi; // trick to avoid the need for allocating TEIDs on the S11 interface
  res.cause = cv2x_EpcS11SapMme::ModifyBearerResponseMessage::REQUEST_ACCEPTED;
  m_s11SapMme->ModifyBearerResponse (res);
}
 
void
cv2x_EpcSgwPgwApplication::DoDeleteBearerCommand (cv2x_EpcS11SapSgw::DeleteBearerCommandMessage req)
{
  NS_LOG_FUNCTION (this << req.teid);
  uint64_t imsi = req.teid; // trick to avoid the need for allocating TEIDs on the S11 interface
  std::map<uint64_t, Ptr<UeInfo> >::iterator ueit = m_ueInfoByImsiMap.find (imsi);
  ...
  cv2x_EpcS11SapMme::DeleteBearerRequestMessage res;
  res.teid = imsi;

  for (std::list<cv2x_EpcS11SapSgw::BearerContextToBeRemoved>::iterator bit = req.bearerContextsToBeRemoved.begin ();
       bit != req.bearerContextsToBeRemoved.end ();
       ++bit)
    {
      cv2x_EpcS11SapMme::BearerContextRemoved bearerContext;
      bearerContext.epsBearerId =  bit->epsBearerId;
...
void
cv2x_EpcSgwPgwApplication::DoDeleteBearerResponse (cv2x_EpcS11SapSgw::DeleteBearerResponseMessage req)
{
  NS_LOG_FUNCTION (this << req.teid);
  uint64_t imsi = req.teid; // trick to avoid the need for allocating TEIDs on the S11 interface
  std::map<uint64_t, Ptr<UeInfo> >::iterator ueit = m_ueInfoByImsiMap.find (imsi);
  ...
  for (std::list<cv2x_EpcS11SapSgw::BearerContextRemovedSgwPgw>::iterator bit = req.bearerContextsRemoved.begin ();
