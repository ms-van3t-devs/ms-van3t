...
#include "cv2x_a3-rsrp-handover-algorithm.h"
#include <ns3/log.h>
...
NS_LOG_COMPONENT_DEFINE ("cv2x_A3RsrpHandoverAlgorithm");

NS_OBJECT_ENSURE_REGISTERED (cv2x_A3RsrpHandoverAlgorithm);


cv2x_A3RsrpHandoverAlgorithm::cv2x_A3RsrpHandoverAlgorithm ()
  : m_handoverManagementSapUser (0)
{
  NS_LOG_FUNCTION (this);
  m_handoverManagementSapProvider = new cv2x_MemberLteHandoverManagementSapProvider<cv2x_A3RsrpHandoverAlgorithm> (this);
}
...
TypeId
cv2x_A3RsrpHandoverAlgorithm::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::cv2x_A3RsrpHandoverAlgorithm")
    .SetParent<cv2x_LteHandoverAlgorithm> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_A3RsrpHandoverAlgorithm> ()
    .AddAttribute ("Hysteresis",
                   "Handover margin (hysteresis) in dB "
                   "(rounded to the nearest multiple of 0.5 dB)",
                   DoubleValue (3.0),
                   MakeDoubleAccessor (&cv2x_A3RsrpHandoverAlgorithm::m_hysteresisDb),
    ...
    .AddAttribute ("TimeToTrigger",
                   ...
                   MakeTimeAccessor (&cv2x_A3RsrpHandoverAlgorithm::m_timeToTrigger),
                   MakeTimeChecker ())
  ;
  return tid;
}
...
void
cv2x_A3RsrpHandoverAlgorithm::SetLteHandoverManagementSapUser (cv2x_LteHandoverManagementSapUser* s)
{
...
cv2x_A3RsrpHandoverAlgorithm::GetLteHandoverManagementSapProvider ()
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_A3RsrpHandoverAlgorithm::DoInitialize ()
{
  NS_LOG_FUNCTION (this);

  uint8_t hysteresisIeValue = cv2x_EutranMeasurementMapping::ActualHysteresis2IeValue (m_hysteresisDb);
  ...
  reportConfig.eventId = cv2x_LteRrcSap::ReportConfigEutra::EVENT_A3;
  ...
  reportConfig.triggerQuantity = cv2x_LteRrcSap::ReportConfigEutra::RSRP;
  reportConfig.reportInterval = cv2x_LteRrcSap::ReportConfigEutra::MS1024;
...
void
cv2x_A3RsrpHandoverAlgorithm::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_A3RsrpHandoverAlgorithm::DoReportUeMeas (uint16_t rnti,
...
bool
cv2x_A3RsrpHandoverAlgorithm::IsValidNeighbour (uint16_t cellId)
{
