...
#ifndef CV2X_LTE_SL_INTERFERENCE_H
#define CV2X_LTE_SL_INTERFERENCE_H
...
class cv2x_LteSlChunkProcessor;
 ...
 *
 */
class cv2x_LteSlInterference : public Object
{
public:
  cv2x_LteSlInterference ();
  virtual ~cv2x_LteSlInterference ();
  ...
  /**
   * Add a cv2x_LteSlChunkProcessor that will use the time-vs-frequency SINR
   * calculated by this cv2x_LteSlInterference instance. Note that all the
   * added cv2x_LteSlChunkProcessors will work in parallel.
   *
   * @param p
   */
  void AddSinrChunkProcessor (Ptr<cv2x_LteSlChunkProcessor> p);

  /**
   * Add a cv2x_LteSlChunkProcessor that will use the time-vs-frequency
   *  interference calculated by this cv2x_LteSlInterference instance. Note 
   *  that all the added cv2x_LteSlChunkProcessors will work in parallel.
   *
   * @param p
   */
  void AddInterferenceChunkProcessor (Ptr<cv2x_LteSlChunkProcessor> p);

  /**
   * Add a cv2x_LteSlChunkProcessor that will use the time-vs-frequency
   *  power calculated by this cv2x_LteSlInterference instance. Note
   *  that all the added cv2x_LteSlChunkProcessors will work in parallel.
   *
   * @param p
   */
  void AddRsPowerChunkProcessor (Ptr<cv2x_LteSlChunkProcessor> p);
...
private:
  ...
  std::list<Ptr<cv2x_LteSlChunkProcessor> > m_rsPowerChunkProcessorList;

  /** all the processor instances that need to be notified whenever
      a new SINR chunk is calculated */
  std::list<Ptr<cv2x_LteSlChunkProcessor> > m_sinrChunkProcessorList;

  /** all the processor instances that need to be notified whenever
      a new interference chunk is calculated */
  std::list<Ptr<cv2x_LteSlChunkProcessor> > m_interfChunkProcessorList;
...
#endif /* CV2X_LTE_SL_INTERFERENCE_H */
