...
#ifndef CV2X_LTE_ENB_NET_DEVICE_H
#define CV2X_LTE_ENB_NET_DEVICE_H
...
class cv2x_LteHandoverAlgorithm;
class cv2x_LteAnr;
class cv2x_LteFfrAlgorithm;
class cv2x_LteEnbComponentCarrierManager;
 ...
 */
class cv2x_LteEnbNetDevice : public cv2x_LteNetDevice
{
public:
  /**
  ...
  cv2x_LteEnbNetDevice ();

  virtual ~cv2x_LteEnbNetDevice (void);
   ...
   */
  Ptr<cv2x_LteEnbMac> GetMac (void) const;

  /**
   * \param index CC index
   * \return a pointer to the MAC of the CC addressed by index.
   */
  Ptr<cv2x_LteEnbMac> GetMac (uint8_t index);

  /**
   * \return a pointer to the physical layer of the PCC.
   */
  Ptr<cv2x_LteEnbPhy> GetPhy (void) const;
  
  /**
   * \param index SCC index
   * \return a pointer to the physical layer of the SCC addressed by index.
   */
  Ptr<cv2x_LteEnbPhy> GetPhy (uint8_t index);

  /** 
   * \return a pointer to the Radio Resource Control instance of the eNB
   */
  Ptr<cv2x_LteEnbRrc> GetRrc () const;
  
  /** 
   * \return a pointer to the cv2x_ComponentCarrierManager instance of the eNB
   */
  Ptr<cv2x_LteEnbComponentCarrierManager> GetComponentCarrierManager () const;
   ...
   * \sa cv2x_LteEnbNetDevice::SetCsgId
   */
   ...
   *
   * \sa cv2x_LteEnbNetDevice::SetCsgIndication
   */
  ...
  /**
   * \brief Set the cv2x_ComponentCarrier Map of the Enb
   * \param ccm the map of cv2x_ComponentCarrierEnb
   *
   */

  void SetCcMap (std::map< uint8_t, Ptr<cv2x_ComponentCarrierEnb> > ccm);
  ...
  std::map< uint8_t, Ptr<cv2x_ComponentCarrierEnb> >  GetCcMap (void);
...
private:
   ...
   * the cv2x_LteEnbNetDevice from a user perspective, but are actually used also in
   ...
   * the cv2x_LteEnbNetDevice.
   */
  void UpdateConfig ();

  Ptr<cv2x_LteEnbRrc> m_rrc; ///< the RRC

  Ptr<cv2x_LteHandoverAlgorithm> m_handoverAlgorithm; ///< the handover algorithm
 
  Ptr<cv2x_LteAnr> m_anr; ///< ANR

  Ptr<cv2x_LteFfrAlgorithm> m_ffrAlgorithm; /**< DEPRECATED - It is maintained for backward compatibility after adding CA feature*/
  ...
  std::map < uint8_t, Ptr<cv2x_ComponentCarrierEnb> > m_ccMap; /**< cv2x_ComponentCarrier map */
  
  Ptr<cv2x_LteEnbComponentCarrierManager> m_componentCarrierManager; ///< the component carrier manager of this eNb

}; // end of class cv2x_LteEnbNetDevice

} // namespace ns3

#endif /* CV2X_LTE_ENB_NET_DEVICE_H */
