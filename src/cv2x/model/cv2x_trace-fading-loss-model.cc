...
NS_LOG_COMPONENT_DEFINE ("cv2x_TraceFadingLossModel");

NS_OBJECT_ENSURE_REGISTERED (cv2x_TraceFadingLossModel);
  


cv2x_TraceFadingLossModel::cv2x_TraceFadingLossModel ()
...
TypeId
cv2x_TraceFadingLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_TraceFadingLossModel")
    .SetParent<SpectrumPropagationLossModel> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_TraceFadingLossModel> ()
    .AddAttribute ("TraceFilename",
                   "Name of file to load a trace from.",
                   StringValue (""),
                   MakeStringAccessor (&cv2x_TraceFadingLossModel::SetTraceFileName),
                   MakeStringChecker ())
    .AddAttribute ("TraceLength",
                  "The total length of the fading trace (default value 10 s.)",
                  TimeValue (Seconds (10.0)),
                  MakeTimeAccessor (&cv2x_TraceFadingLossModel::SetTraceLength),
                  MakeTimeChecker ())
    .AddAttribute ("SamplesNum",
                  "The number of samples the trace is made of (default 10000)",
                   UintegerValue (10000),
                   MakeUintegerAccessor (&cv2x_TraceFadingLossModel::m_samplesNum),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("WindowSize",
                  "The size of the window for the fading trace (default value 0.5 s.)",
                  TimeValue (Seconds (0.5)),
                  MakeTimeAccessor (&cv2x_TraceFadingLossModel::m_windowSize),
                  MakeTimeChecker ())
    .AddAttribute ("RbNum",
                    "The number of RB the trace is made of (default 100)",
                    UintegerValue (100),
                   MakeUintegerAccessor (&cv2x_TraceFadingLossModel::m_rbNum),
    ...
    .AddAttribute ("RngStreamSetSize",
                   ...
                   MakeUintegerAccessor (&cv2x_TraceFadingLossModel::m_streamSetSize),
...
void
cv2x_TraceFadingLossModel::SetTraceFileName (std::string fileName)
{
...
void 
cv2x_TraceFadingLossModel::SetTraceLength (Time t)
{
  m_traceLength = t;
}

void 
cv2x_TraceFadingLossModel::DoInitialize ()
{
  LoadTrace ();
}
...
void
cv2x_TraceFadingLossModel::LoadTrace ()
{
...
Ptr<SpectrumValue>
cv2x_TraceFadingLossModel::DoCalcRxPowerSpectralDensity (
...
int64_t
cv2x_TraceFadingLossModel::AssignStreams (int64_t stream)
{
