...
#ifndef CV2X_LTE_INTERFERENCE_H
#define CV2X_LTE_INTERFERENCE_H
 ...
 *
 */
class cv2x_LteInterference : public Object
{
public:
  cv2x_LteInterference ();
  virtual ~cv2x_LteInterference ();
  ...
  /**
   * Add a cv2x_LteChunkProcessor that will use the time-vs-frequency SINR
   * calculated by this cv2x_LteInterference instance. Note that all the
   * added cv2x_LteChunkProcessors will work in parallel.
   *
   * @param p
   */
  void AddSinrChunkProcessor (Ptr<cv2x_LteChunkProcessor> p);

  /**
   * Add a cv2x_LteChunkProcessor that will use the time-vs-frequency
   *  interference calculated by this cv2x_LteInterference instance. Note 
   *  that all the added cv2x_LteChunkProcessors will work in parallel.
   *
   * @param p
   */
  void AddInterferenceChunkProcessor (Ptr<cv2x_LteChunkProcessor> p);

  /**
   * Add a cv2x_LteChunkProcessor that will use the time-vs-frequency
   *  power calculated by this cv2x_LteInterference instance. Note
   *  that all the added cv2x_LteChunkProcessors will work in parallel.
   *
   * @param p
   */
...
private:
  ...
  std::list<Ptr<cv2x_LteChunkProcessor> > m_rsPowerChunkProcessorList;

  /** all the processor instances that need to be notified whenever
      a new SINR chunk is calculated */
  std::list<Ptr<cv2x_LteChunkProcessor> > m_sinrChunkProcessorList;

  /** all the processor instances that need to be notified whenever
      a new interference chunk is calculated */
  std::list<Ptr<cv2x_LteChunkProcessor> > m_interfChunkProcessorList;
