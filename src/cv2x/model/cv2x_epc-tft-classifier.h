...
#ifndef CV2X_EPC_TFT_CLASSIFIER_H
#define CV2X_EPC_TFT_CLASSIFIER_H
 ...
 */
class cv2x_EpcTftClassifier : public SimpleRefCount<cv2x_EpcTftClassifier>
{
public:
  
  cv2x_EpcTftClassifier ();
   ...
   * 
   */
  void Add (Ptr<cv2x_EpcTft> tft, uint32_t id);
   ...
   */
  uint32_t Classify (Ptr<Packet> p, cv2x_EpcTft::Direction direction);
  
protected:
  
  std::map <uint32_t, Ptr<cv2x_EpcTft> > m_tftMap; ///< TFT map
