...
#ifndef CV2X_LTE_CHUNK_PROCESSOR_H
#define CV2X_LTE_CHUNK_PROCESSOR_H
...
typedef Callback< void, const SpectrumValue& > cv2x_LteChunkProcessorCallback;
 ...
 */
class cv2x_LteChunkProcessor : public SimpleRefCount<cv2x_LteChunkProcessor>
{
public:
  cv2x_LteChunkProcessor ();
  virtual ~cv2x_LteChunkProcessor ();
  ...
  /**
    ...
    * cv2x_LteChunkProcessor::End().
    *
    * \param c callback function
    */
  virtual void AddCallback (cv2x_LteChunkProcessorCallback c);
...
private:
  ...
  std::vector<cv2x_LteChunkProcessorCallback> m_lteChunkProcessorCallbacks; ///< chunk processor callback
};


/**
 * A sink to be plugged to the callback of cv2x_LteChunkProcessor allowing
 * to save and later retrieve the latest reported value 
 * 
 */
class cv2x_LteSpectrumValueCatcher
{
public:

  /** 
   * function to be plugged to cv2x_LteChunkProcessor::AddCallback ()
   * 
   * \param value 
   */
   ...
   * 
   * 
   * \return the latest value reported by the cv2x_LteChunkProcessor
   */
...
#endif /* CV2X_LTE_CHUNK_PROCESSOR_H */
