...
#ifndef CV2X_LTE_PHY_H
#define CV2X_LTE_PHY_H
 ...
 * \ingroup lte
 *
 * The cv2x_LtePhy models the physical layer of LTE. It is composed by two
 * cv2x_LteSpectrumPhy, one for the downlink and one for the uplink.
 */
class cv2x_LtePhy : public Object
{
...
public:
   ...
   */
  cv2x_LtePhy (Ptr<cv2x_LteSpectrumPhy> dlPhy, Ptr<cv2x_LteSpectrumPhy> ulPhy);
   ...
   * \param d the device
   */
  void SetDevice (Ptr<cv2x_LteNetDevice> d);
  /**
   * \brief Get the device where the phy layer is attached
   * \return the pointer to the device
   */
  Ptr<cv2x_LteNetDevice> GetDevice () const;

  /** 
   * 
   * \return a pointer to the cv2x_LteSpectrumPhy instance that manages the downlink
   */
  Ptr<cv2x_LteSpectrumPhy> GetDownlinkSpectrumPhy ();


  /** 
   * 
   * \return a pointer to the cv2x_LteSpectrumPhy instance that manages the uplink
   */
  Ptr<cv2x_LteSpectrumPhy> GetUplinkSpectrumPhy ();
  ...
  */
  void SetControlMessages (Ptr<cv2x_LteControlMessage> m);

  /**
  * \returns the list of control messages to be sent
  */
  std::list<Ptr<cv2x_LteControlMessage> > GetControlMessages (void);
...
protected:
  ...
  Ptr<cv2x_LteNetDevice> m_netDevice;

  /**
   * The downlink cv2x_LteSpectrumPhy associated to this cv2x_LtePhy. Also available as
   * attribute `DlSpectrumPhy` in the child classes cv2x_LteEnbPhy and cv2x_LteUePhy.
   */
  Ptr<cv2x_LteSpectrumPhy> m_downlinkSpectrumPhy;
  /**
   * The uplink cv2x_LteSpectrumPhy associated to this cv2x_LtePhy. Also available as
   * attribute `UlSpectrumPhy` in the child classes cv2x_LteEnbPhy and cv2x_LteUePhy.
   */
  Ptr<cv2x_LteSpectrumPhy> m_uplinkSpectrumPhy;

  /**
   * Transmission power in dBm. Also available as attribute `TxPower` in the
   * child classes cv2x_LteEnbPhy and cv2x_LteUePhy.
   */
  double m_txPower;
   ...
   * cv2x_LteEnbPhy and cv2x_LteUePhy.
   *
  ...
  std::vector< std::list<Ptr<cv2x_LteControlMessage> > > m_controlMessagesQueue;
  /**
   ...
   *
   * In cv2x_LteEnbPhy, it is 2 TTIs by default and can be configured through the
   * `MacToChannelDelay` attribute. In cv2x_LteUePhy, it is 4 TTIs.
   */
  uint8_t m_macChTtiDelay;

  /**
   * Cell identifier. In cv2x_LteEnbPhy, this corresponds to the ID of the cell
   * which hosts this PHY layer. In cv2x_LteUePhy, this corresponds to the ID of the
...
}; // end of `class cv2x_LtePhy`


}

#endif /* CV2X_LTE_PHY_H */
