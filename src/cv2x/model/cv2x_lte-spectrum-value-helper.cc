...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteSpectrumValueHelper");
...
double 
cv2x_LteSpectrumValueHelper::GetCarrierFrequency (uint32_t earfcn)
{
...
uint16_t
cv2x_LteSpectrumValueHelper::GetDownlinkCarrierBand (uint32_t nDl)
{
  NS_LOG_FUNCTION (nDl);
...
uint16_t
cv2x_LteSpectrumValueHelper::GetUplinkCarrierBand (uint32_t nUl)
{
  NS_LOG_FUNCTION (nUl);
  for (uint16_t i = 0; i < NUM_EUTRA_BANDS; ++i)
    {
...
double
cv2x_LteSpectrumValueHelper::GetDownlinkCarrierFrequency (uint32_t nDl)
{
  NS_LOG_FUNCTION (nDl);
...
double
cv2x_LteSpectrumValueHelper::GetUplinkCarrierFrequency (uint32_t nUl)
{
  NS_LOG_FUNCTION (nUl);
...
double 
cv2x_LteSpectrumValueHelper::GetChannelBandwidth (uint8_t transmissionBandwidth)
{
...
/// cv2x_LteSpectrumModelId structure
struct cv2x_LteSpectrumModelId
{
  /**
   * Constructor
   *
   ...
   * \param b bandwidth
   */
  cv2x_LteSpectrumModelId (uint32_t f, uint8_t b);
  uint32_t earfcn; ///< EARFCN
  uint8_t  bandwidth; ///< bandwidth
};

cv2x_LteSpectrumModelId::cv2x_LteSpectrumModelId (uint32_t f, uint8_t b)
  : earfcn (f), 
    bandwidth (b)
{
}
 ...
 */
bool
operator < (const cv2x_LteSpectrumModelId& a, const cv2x_LteSpectrumModelId& b)
{
  return ( (a.earfcn < b.earfcn) || ( (a.earfcn == b.earfcn) && (a.bandwidth < b.bandwidth) ) );
}
 

static std::map<cv2x_LteSpectrumModelId, Ptr<SpectrumModel> > g_lteSpectrumModelMap; ///< LTE spectrum model map


Ptr<SpectrumModel>
cv2x_LteSpectrumValueHelper::GetSpectrumModel (uint32_t earfcn, uint8_t txBandwidthConfiguration)
{
  NS_LOG_FUNCTION (earfcn << (uint16_t) txBandwidthConfiguration);
  Ptr<SpectrumModel> ret;
  cv2x_LteSpectrumModelId key (earfcn, txBandwidthConfiguration);
  std::map<cv2x_LteSpectrumModelId, Ptr<SpectrumModel> >::iterator it = g_lteSpectrumModelMap.find (key);
  if (it != g_lteSpectrumModelMap.end ())
  ...
  else
    {
      ...
      g_lteSpectrumModelMap.insert (std::pair<cv2x_LteSpectrumModelId, Ptr<SpectrumModel> > (key, ret));
    }
...
Ptr<SpectrumValue> 
cv2x_LteSpectrumValueHelper::CreateTxPowerSpectralDensity (uint32_t earfcn, uint8_t txBandwidthConfiguration, double powerTx, std::vector <int> activeRbs)
{
...
Ptr<SpectrumValue>
cv2x_LteSpectrumValueHelper::CreateTxPowerSpectralDensity (uint32_t earfcn, uint8_t txBandwidthConfiguration, double powerTx, std::map<int, double> powerTxMap, std::vector <int> activeRbs)
{
...
Ptr<SpectrumValue> 
cv2x_LteSpectrumValueHelper::CreateUlTxPowerSpectralDensity (uint16_t earfcn, uint8_t txBandwidthConfiguration, double powerTx, std::vector <int> activeRbs)
{
  NS_LOG_FUNCTION (earfcn << (uint16_t) txBandwidthConfiguration << powerTx << activeRbs);
...
Ptr<SpectrumValue>
cv2x_LteSpectrumValueHelper::CreateNoisePowerSpectralDensity (uint32_t earfcn, uint8_t txBandwidthConfiguration, double noiseFigure)
{
...
Ptr<SpectrumValue>
cv2x_LteSpectrumValueHelper::CreateNoisePowerSpectralDensity (double noiseFigureDb, Ptr<SpectrumModel> spectrumModel)
{
