...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteRlcHeader");

NS_OBJECT_ENSURE_REGISTERED (cv2x_LteRlcHeader);

cv2x_LteRlcHeader::cv2x_LteRlcHeader ()
  : m_headerLength (0),
    m_framingInfo (0xff),
...
void
cv2x_LteRlcHeader::SetFramingInfo (uint8_t framingInfo)
{
  m_framingInfo = framingInfo & 0x03;
}

void
cv2x_LteRlcHeader::SetSequenceNumber (cv2x_SequenceNumber10 sequenceNumber)
{
  m_sequenceNumber = sequenceNumber;
}

uint8_t
cv2x_LteRlcHeader::GetFramingInfo () const
{
  return m_framingInfo;
}

cv2x_SequenceNumber10
cv2x_LteRlcHeader::GetSequenceNumber () const
{
...
void
cv2x_LteRlcHeader::PushExtensionBit (uint8_t extensionBit)
{
...
void
cv2x_LteRlcHeader::PushLengthIndicator (uint16_t lengthIndicator)
{
...
uint8_t
cv2x_LteRlcHeader::PopExtensionBit (void)
{
...
uint16_t
cv2x_LteRlcHeader::PopLengthIndicator (void)
{
...
TypeId
cv2x_LteRlcHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_LteRlcHeader")
    .SetParent<Header> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_LteRlcHeader> ()
  ;
  return tid;
}

TypeId
cv2x_LteRlcHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

void cv2x_LteRlcHeader::Print (std::ostream &os)  const
{
...
uint32_t cv2x_LteRlcHeader::GetSerializedSize (void) const
{
  return m_headerLength;
}

void cv2x_LteRlcHeader::Serialize (Buffer::Iterator start) const
{
...
uint32_t cv2x_LteRlcHeader::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;
  uint8_t byte_1;
  uint8_t byte_2;
