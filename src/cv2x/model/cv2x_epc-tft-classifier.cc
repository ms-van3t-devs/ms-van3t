 ...
 *
 * Authors: 
 *   Nicola Baldo <nbaldo@cttc.es> (the cv2x_EpcTftClassifier class)
 *   Giuseppe Piro <g.piro@poliba.it> (part of the code in cv2x_EpcTftClassifier::Classify () 
 *       which comes from RrcEntity::Classify of the GSoC 2010 LTE module)
 *
 */

#include "cv2x_epc-tft-classifier.h"
...
NS_LOG_COMPONENT_DEFINE ("cv2x_EpcTftClassifier");

cv2x_EpcTftClassifier::cv2x_EpcTftClassifier ()
{
  NS_LOG_FUNCTION (this);
}

void
cv2x_EpcTftClassifier::Add (Ptr<cv2x_EpcTft> tft, uint32_t id)
{
...
void
cv2x_EpcTftClassifier::Delete (uint32_t id)
{
...
uint32_t 
cv2x_EpcTftClassifier::Classify (Ptr<Packet> p, cv2x_EpcTft::Direction direction)
{
  ...
  if (ipType == 0x04)
    ...
    {
      NS_ABORT_MSG ("cv2x_EpcTftClassifier::Classify - Unknown IP type...");
    }
  ...
  if (ipType == 0x04)
    {
      ...
      std::map <uint32_t, Ptr<cv2x_EpcTft> >::const_reverse_iterator it;
      NS_LOG_LOGIC ("TFT MAP size: " << m_tftMap.size ());

      for (it = m_tftMap.rbegin (); it != m_tftMap.rend (); ++it)
        {
          NS_LOG_LOGIC ("TFT id: " << it->first );
          NS_LOG_LOGIC (" Ptr<cv2x_EpcTft>: " << it->second);
          Ptr<cv2x_EpcTft> tft = it->second;
