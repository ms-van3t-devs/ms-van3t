...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteRlcUm");

NS_OBJECT_ENSURE_REGISTERED (cv2x_LteRlcUm);
...
TypeId
cv2x_LteRlcUm::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_LteRlcUm")
    .SetParent<cv2x_LteRlc> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_LteRlcUm> ()
    .AddAttribute ("MaxTxBufferSize",
                   "Maximum Size of the Transmission Buffer (in Bytes)",
                   UintegerValue (10 * 1024),
                   MakeUintegerAccessor (&cv2x_LteRlcUm::m_maxTxBufferSize),
...
void
cv2x_LteRlcUm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
...
void
cv2x_LteRlcUm::DoNotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId, uint8_t componentCarrierId, uint16_t rnti, uint8_t lcid)
{
  ...
  while ( firstSegment && (firstSegment->GetSize () > 0) && (nextSegmentSize > 0) )
    {
      ...
      if ( (firstSegment->GetSize () > nextSegmentSize) ||
         ...
         )
        {
          ...
          rlcHeader.PushExtensionBit (cv2x_LteRlcHeader::DATA_FIELD_FOLLOWS);
      ...
      else // (firstSegment->GetSize () < m_nextSegmentSize) && (m_txBuffer.size () > 0)
        {
          ...
          rlcHeader.PushExtensionBit (cv2x_LteRlcHeader::E_LI_FIELDS_FOLLOWS);
  ...
  if ( (tag.GetStatus () == cv2x_LteRlcSduStatusTag::FULL_SDU) ||
    ...
    {
      framingInfo |= cv2x_LteRlcHeader::FIRST_BYTE;
    }
  else
    {
      framingInfo |= cv2x_LteRlcHeader::NO_FIRST_BYTE;
    }
  ...
  if ( (tag.GetStatus () == cv2x_LteRlcSduStatusTag::FULL_SDU) ||
    ...
    {
      framingInfo |= cv2x_LteRlcHeader::LAST_BYTE;
    }
  else
    {
      framingInfo |= cv2x_LteRlcHeader::NO_LAST_BYTE;
    }
  ...
  if (! m_txBuffer.empty ())
    {
      m_rbsTimer.Cancel ();
      m_rbsTimer = Simulator::Schedule (MilliSeconds (10), &cv2x_LteRlcUm::ExpireRbsTimer, this);
    }
}

void
cv2x_LteRlcUm::DoNotifyHarqDeliveryFailure ()
{
  NS_LOG_FUNCTION (this);
}

void
cv2x_LteRlcUm::DoReceivePdu (Ptr<Packet> p, uint16_t rnti, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << m_rnti << (uint32_t) m_lcid << p->GetSize ());
  ...
  NS_LOG_LOGIC ("RLC header: " << rlcHeader);
  cv2x_SequenceNumber10 seqNumber = rlcHeader.GetSequenceNumber ();
  ...
  if ( m_rxBuffer.count (m_vrUr.GetValue ()) > 0 )
    {
      ...
      uint16_t newVrUr;
      cv2x_SequenceNumber10 oldVrUr = m_vrUr;
      ...
      if ( m_vrUh > m_vrUr )
        {
          ...
          m_reorderingTimer = Simulator::Schedule (Time ("0.01s"), //Modified due to retransmission problem for vehicular d2d
                                                   &cv2x_LteRlcUm::ExpireReorderingTimer ,this);
          m_vrUx = m_vrUh;
...
bool
cv2x_LteRlcUm::IsInsideReorderingWindow (cv2x_SequenceNumber10 seqNumber)
{
...
void
cv2x_LteRlcUm::ReassembleAndDeliver (Ptr<Packet> packet)
{
  cv2x_LteRlcHeader rlcHeader;
  packet->RemoveHeader (rlcHeader);
  uint8_t framingInfo = rlcHeader.GetFramingInfo ();
  cv2x_SequenceNumber10 currSeqNumber = rlcHeader.GetSequenceNumber ();
  bool expectedSnLost;
      ...
      switch (m_reassemblingState)
        {
          case WAITING_S0_FULL:
                  switch (framingInfo)
                    {
                      case (cv2x_LteRlcHeader::FIRST_BYTE | cv2x_LteRlcHeader::LAST_BYTE):
                              m_reassemblingState = WAITING_S0_FULL;
                      ...
                      case (cv2x_LteRlcHeader::FIRST_BYTE | cv2x_LteRlcHeader::NO_LAST_BYTE):
                      ...
                      case (cv2x_LteRlcHeader::NO_FIRST_BYTE | cv2x_LteRlcHeader::LAST_BYTE):
                              m_reassemblingState = WAITING_S0_FULL;
                      ...
                      case (cv2x_LteRlcHeader::NO_FIRST_BYTE | cv2x_LteRlcHeader::NO_LAST_BYTE):
...
void
cv2x_LteRlcUm::ReassembleOutsideWindow (void)
{
  ...
  while ( (it != m_rxBuffer.end ()) && ! IsInsideReorderingWindow (cv2x_SequenceNumber10 (it->first)) )
    {
...
void
cv2x_LteRlcUm::ReassembleSnInterval (cv2x_SequenceNumber10 lowSeqNumber, cv2x_SequenceNumber10 highSeqNumber)
{
  NS_LOG_LOGIC ("Reassemble SN between " << lowSeqNumber << " and " << highSeqNumber);

  std::map <uint16_t, Ptr<Packet> >::iterator it;

  cv2x_SequenceNumber10 reassembleSn = lowSeqNumber;
...
void
cv2x_LteRlcUm::DoReportBufferStatus (void)
{
  Time holDelay (0);
...
void
cv2x_LteRlcUm::ExpireReorderingTimer (void)
{
  ...
  std::map <uint16_t, Ptr<Packet> >::iterator it;
  cv2x_SequenceNumber10 newVrUr = m_vrUx;
...
void
cv2x_LteRlcUm::ExpireRbsTimer (void)
{
