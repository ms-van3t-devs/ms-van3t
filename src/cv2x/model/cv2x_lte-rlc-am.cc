...
NS_LOG_COMPONENT_DEFINE ("cv2x_LteRlcAm");

NS_OBJECT_ENSURE_REGISTERED (cv2x_LteRlcAm);
...
cv2x_LteRlcAm::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_LteRlcAm")
    ...
    .AddAttribute ("ReportBufferStatusTimer",
                   ...
                   TimeValue (MilliSeconds (20)),
                   MakeTimeAccessor (&cv2x_LteRlcAm::m_rbsTimerValue),
                   MakeTimeChecker ())
    .AddAttribute ("TxOpportunityForRetxAlwaysBigEnough",
                   ...
                   BooleanValue (false),
                   MakeBooleanAccessor (&cv2x_LteRlcAm::m_txOpportunityForRetxAlwaysBigEnough),
                   MakeBooleanChecker ())
...
void
cv2x_LteRlcAm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
  NS_LOG_FUNCTION (this << m_rnti << (uint32_t) m_lcid << p->GetSize ());

  /** Store arrival time */
  Time now = Simulator::Now ();
  cv2x_RlcTag timeTag (now);
  p->AddPacketTag (timeTag);

  /** Store PDCP PDU */

  cv2x_LteRlcSduStatusTag tag;
  tag.SetStatus (cv2x_LteRlcSduStatusTag::FULL_SDU);
  p->AddPacketTag (tag);
  ...
  m_rbsTimer.Cancel ();
  m_rbsTimer = Simulator::Schedule (m_rbsTimerValue, &cv2x_LteRlcAm::ExpireRbsTimer, this);
}
...
void
cv2x_LteRlcAm::DoNotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId, uint8_t componentCarrierId, uint16_t rnti, uint8_t lcid)
{
  ...
  if ( m_statusPduRequested && ! m_statusProhibitTimer.IsRunning () )
    {
      ...
      rlcAmHeader.SetControlPdu (cv2x_LteRlcAmHeader::STATUS_PDU);
     
      NS_LOG_LOGIC ("Check for SNs to NACK from " << m_vrR.GetValue() << " to " << m_vrMs.GetValue());
      cv2x_SequenceNumber10 sn;
      ...
      NS_ASSERT_MSG (!packet->PeekPacketTag (rlcTag), "cv2x_RlcTag is already present");
      ...
      m_statusProhibitTimer = Simulator::Schedule (m_statusProhibitTimerValue,
                                                   &cv2x_LteRlcAm::ExpireStatusProhibitTimer, this);
      return;
    }
            ...
            {            
              ...
              if (( packet->GetSize () <= bytes )
                  || m_txOpportunityForRetxAlwaysBigEnough)
                {
                  ...
                  rlcAmHeader.SetPollingBit (cv2x_LteRlcAmHeader::STATUS_REPORT_NOT_REQUESTED);
                  ...
                  if (((m_txonBuffer.empty ()) && (m_retxBufferSize == packet->GetSize () + rlcAmHeader.GetSerializedSize ())) 
                    ...
                    {
                      m_pollRetransmitTimerJustExpired = false;
                      rlcAmHeader.SetPollingBit (cv2x_LteRlcAmHeader::STATUS_REPORT_IS_REQUESTED);
                      m_pduWithoutPoll = 0;
                      m_byteWithoutPoll = 0;
                      ...
                      if (! m_pollRetransmitTimer.IsRunning () )
                        {
                          ...
                          m_pollRetransmitTimer = Simulator::Schedule (m_pollRetransmitTimerValue,
                                                                       &cv2x_LteRlcAm::ExpirePollRetransmitTimer, this);
                        }
                      else
                        {
                  ...
                  // Sender timestamp
                  cv2x_RlcTag rlcTag (Simulator::Now ());
                  NS_ASSERT_MSG (packet->PeekPacketTag (rlcTag), "cv2x_RlcTag is missing");
  ...
  while ( firstSegment && (firstSegment->GetSize () > 0) && (nextSegmentSize > 0) )
    {
      ...
      if ( (firstSegment->GetSize () > nextSegmentSize) ||
         ...
         )
        {
          ...
          cv2x_LteRlcSduStatusTag oldTag, newTag;
          firstSegment->RemovePacketTag (oldTag);
          newSegment->RemovePacketTag (newTag);
          if (oldTag.GetStatus () == cv2x_LteRlcSduStatusTag::FULL_SDU)
            {
              newTag.SetStatus (cv2x_LteRlcSduStatusTag::FIRST_SEGMENT);
              oldTag.SetStatus (cv2x_LteRlcSduStatusTag::LAST_SEGMENT);
            }
          else if (oldTag.GetStatus () == cv2x_LteRlcSduStatusTag::LAST_SEGMENT)
            {
              newTag.SetStatus (cv2x_LteRlcSduStatusTag::MIDDLE_SEGMENT);
              //oldTag.SetStatus (cv2x_LteRlcSduStatusTag::LAST_SEGMENT);
            }
          ...
          if (firstSegment->GetSize () > 0)
          ...
          else
            {
              ...
              if (newTag.GetStatus () == cv2x_LteRlcSduStatusTag::FIRST_SEGMENT)
                {
                  newTag.SetStatus (cv2x_LteRlcSduStatusTag::FULL_SDU);
                }
              else if (newTag.GetStatus () == cv2x_LteRlcSduStatusTag::MIDDLE_SEGMENT)
                {
                  newTag.SetStatus (cv2x_LteRlcSduStatusTag::LAST_SEGMENT);
                }
            }
          ...
          rlcAmHeader.PushExtensionBit (cv2x_LteRlcAmHeader::DATA_FIELD_FOLLOWS);
      ...
      else // (firstSegment->GetSize () < m_nextSegmentSize) && (m_txBuffer.size () > 0)
        {
          ...
          rlcAmHeader.PushExtensionBit (cv2x_LteRlcAmHeader::E_LI_FIELDS_FOLLOWS);
  ...
  rlcAmHeader.SetResegmentationFlag (cv2x_LteRlcAmHeader::PDU);
  rlcAmHeader.SetLastSegmentFlag (cv2x_LteRlcAmHeader::LAST_PDU_SEGMENT);
  ...
  // FIRST SEGMENT
  cv2x_LteRlcSduStatusTag tag;
  NS_ASSERT_MSG ((*it)->PeekPacketTag (tag), "cv2x_LteRlcSduStatusTag is missing");
  (*it)->PeekPacketTag (tag);
  if ( (tag.GetStatus () == cv2x_LteRlcSduStatusTag::FULL_SDU) ||
       (tag.GetStatus () == cv2x_LteRlcSduStatusTag::FIRST_SEGMENT)
     )
    {
      framingInfo |= cv2x_LteRlcAmHeader::FIRST_BYTE;
    }
  else
    {
      framingInfo |= cv2x_LteRlcAmHeader::NO_FIRST_BYTE;
    }
  ...
  it--;
  if ( (tag.GetStatus () == cv2x_LteRlcSduStatusTag::FULL_SDU) ||
        (tag.GetStatus () == cv2x_LteRlcSduStatusTag::LAST_SEGMENT) )
    {
      framingInfo |= cv2x_LteRlcAmHeader::LAST_BYTE;
    }
  else
    {
      framingInfo |= cv2x_LteRlcAmHeader::NO_LAST_BYTE;
    }
...
void
cv2x_LteRlcAm::DoNotifyHarqDeliveryFailure ()
{
  NS_LOG_FUNCTION (this);
}
...
void
cv2x_LteRlcAm::DoReceivePdu (Ptr<Packet> p, uint16_t rnti, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << m_rnti << (uint32_t) m_lcid << p->GetSize ());
  ...
  if ( rlcAmHeader.IsDataPdu () )
    {
      ...
      cv2x_SequenceNumber10 seqNumber = rlcAmHeader.GetSequenceNumber ();
      seqNumber.SetModulusBase (m_vrR);

      if ( rlcAmHeader.GetResegmentationFlag () == cv2x_LteRlcAmHeader::SEGMENT )
        {
          NS_LOG_LOGIC ("PDU segment received ( SN = " << seqNumber << " )");
        }
      else if ( rlcAmHeader.GetResegmentationFlag () == cv2x_LteRlcAmHeader::PDU )
        {
      ...
      if ( rlcAmHeader.GetPollingBit () == cv2x_LteRlcAmHeader::STATUS_REPORT_IS_REQUESTED )
        {
          ...
          if ( m_vrH > m_vrR )
            {
              ...
              m_reorderingTimer = Simulator::Schedule (m_reorderingTimerValue,
                                                       &cv2x_LteRlcAm::ExpireReorderingTimer ,this);
              m_vrX = m_vrH;
  ...
  else if ( rlcAmHeader.IsControlPdu () )
    {
      ...
      cv2x_SequenceNumber10 ackSn = rlcAmHeader.GetAckSn ();
      cv2x_SequenceNumber10 sn;
...
bool
cv2x_LteRlcAm::IsInsideReceivingWindow (cv2x_SequenceNumber10 seqNumber)
{
...
void
cv2x_LteRlcAm::ReassembleAndDeliver (Ptr<Packet> packet)
{
  cv2x_LteRlcAmHeader rlcAmHeader;
  packet->RemoveHeader (rlcAmHeader);
  uint8_t framingInfo = rlcAmHeader.GetFramingInfo ();
  cv2x_SequenceNumber10 currSeqNumber = rlcAmHeader.GetSequenceNumber ();
  bool expectedSnLost;
      ...
      switch (m_reassemblingState)
        {
          case WAITING_S0_FULL:
                  switch (framingInfo)
                    {
                      case (cv2x_LteRlcAmHeader::FIRST_BYTE | cv2x_LteRlcAmHeader::LAST_BYTE):
                              m_reassemblingState = WAITING_S0_FULL;
                      ...
                      case (cv2x_LteRlcAmHeader::FIRST_BYTE | cv2x_LteRlcAmHeader::NO_LAST_BYTE):
                      ...
                      case (cv2x_LteRlcAmHeader::NO_FIRST_BYTE | cv2x_LteRlcAmHeader::LAST_BYTE):
                      case (cv2x_LteRlcAmHeader::NO_FIRST_BYTE | cv2x_LteRlcAmHeader::NO_LAST_BYTE):
                      default:
                              /**
...
void
cv2x_LteRlcAm::DoReportBufferStatus (void)
{
  NS_LOG_FUNCTION (this);
  ...
  if ( m_txonBufferSize > 0 )
    {
      cv2x_RlcTag txonQueueHolTimeTag;
  ...
  cv2x_RlcTag retxQueueHolTimeTag;
...
void
cv2x_LteRlcAm::ExpireReorderingTimer (void)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_LteRlcAm::ExpirePollRetransmitTimer (void)
{
  NS_LOG_FUNCTION (this);
  ...
  if ((m_txonBufferSize == 0 && m_retxBufferSize == 0)
      || (m_vtS == m_vtMs))
    {
      ...
      for (cv2x_SequenceNumber10 sn = m_vtA; sn < m_vtS; sn++)
        {
...
void 
cv2x_LteRlcAm::ExpireStatusProhibitTimer (void)
{
  NS_LOG_FUNCTION (this);
}

void
cv2x_LteRlcAm::ExpireRbsTimer (void)
{
