...
#ifndef CV2X_EPC_ENB_S1_SAP_H
#define CV2X_EPC_ENB_S1_SAP_H
 ...
 * cv2x_LteEnbRrc and the cv2x_EpcEnbApplication. In particular, this class implements the
 * Provider part of the SAP, i.e., the methods exported by the
 * cv2x_EpcEnbApplication and called by the cv2x_LteEnbRrc.
 * 
 */
class cv2x_EpcEnbS1SapProvider
{
public:
  virtual ~cv2x_EpcEnbS1SapProvider ();  
 ...
 * This class implements the Service Access Point (SAP) between the
 * cv2x_LteEnbRrc and the cv2x_EpcEnbApplication. In particular, this class implements the
 * User part of the SAP, i.e., the methods exported by the cv2x_LteEnbRrc
 * and called by the cv2x_EpcEnbApplication.
 * 
 */
class cv2x_EpcEnbS1SapUser
{
public:
  virtual ~cv2x_EpcEnbS1SapUser ();
  ...
  struct DataRadioBearerSetupRequestParameters
  {
    ...
    cv2x_EpsBearer bearer; /**< the characteristics of the bearer to be set
                         up */
...
/**
 * Template for the implementation of the cv2x_EpcEnbS1SapProvider as a member
 * of an owner class of type C to which all methods are forwarded
 * 
 */
template <class C>
class cv2x_MemberEpcEnbS1SapProvider : public cv2x_EpcEnbS1SapProvider
{
public:
  /**
   * Constructor
   ...
   */
  cv2x_MemberEpcEnbS1SapProvider (C* owner);

  // inherited from cv2x_EpcEnbS1SapProvider
...
private:
  cv2x_MemberEpcEnbS1SapProvider ();
  C* m_owner; ///< owner class
};

template <class C>
cv2x_MemberEpcEnbS1SapProvider<C>::cv2x_MemberEpcEnbS1SapProvider (C* owner)
  : m_owner (owner)
{
}

template <class C>
cv2x_MemberEpcEnbS1SapProvider<C>::cv2x_MemberEpcEnbS1SapProvider ()
{
}


template <class C>
void cv2x_MemberEpcEnbS1SapProvider<C>::InitialUeMessage (uint64_t imsi, uint16_t rnti)
{
  m_owner->DoInitialUeMessage (imsi, rnti);
}

template <class C>
void cv2x_MemberEpcEnbS1SapProvider<C>::DoSendReleaseIndication (uint64_t imsi, uint16_t rnti, uint8_t bearerId)
{
  m_owner->DoReleaseIndication (imsi, rnti, bearerId);
}

template <class C>
void cv2x_MemberEpcEnbS1SapProvider<C>::PathSwitchRequest (PathSwitchRequestParameters params)
{
  m_owner->DoPathSwitchRequest (params);
}

template <class C>
void cv2x_MemberEpcEnbS1SapProvider<C>::UeContextRelease (uint16_t rnti)
{
  m_owner->DoUeContextRelease (rnti);
}

/**
 * Template for the implementation of the cv2x_EpcEnbS1SapUser as a member
 * of an owner class of type C to which all methods are forwarded
 * 
 */
template <class C>
class cv2x_MemberEpcEnbS1SapUser : public cv2x_EpcEnbS1SapUser
{
public:
  /**
   * Constructor
   ...
   * \param owner the owner class
   */
  cv2x_MemberEpcEnbS1SapUser (C* owner);

  // inherited from cv2x_EpcEnbS1SapUser
...
private:
  cv2x_MemberEpcEnbS1SapUser ();
  C* m_owner; ///< owner class
};

template <class C>
cv2x_MemberEpcEnbS1SapUser<C>::cv2x_MemberEpcEnbS1SapUser (C* owner)
  : m_owner (owner)
{
}

template <class C>
cv2x_MemberEpcEnbS1SapUser<C>::cv2x_MemberEpcEnbS1SapUser ()
{
}

template <class C>
void cv2x_MemberEpcEnbS1SapUser<C>::DataRadioBearerSetupRequest (DataRadioBearerSetupRequestParameters params)
{
  m_owner->DoDataRadioBearerSetupRequest (params);
}

template <class C>
void cv2x_MemberEpcEnbS1SapUser<C>::PathSwitchRequestAcknowledge (PathSwitchRequestAcknowledgeParameters params)
{
