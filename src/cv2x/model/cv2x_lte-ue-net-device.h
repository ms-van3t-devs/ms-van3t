...
#ifndef CV2X_LTE_UE_NET_DEVICE_H
#define CV2X_LTE_UE_NET_DEVICE_H
...
class cv2x_EpcUeNas;
class cv2x_EpcTft;
class cv2x_LteUeComponentCarrierManager;

/**
 * \ingroup lte
 * The cv2x_LteUeNetDevice class implements the UE net device
 */
class cv2x_LteUeNetDevice : public cv2x_LteNetDevice
{
...
public:
  ...
  cv2x_LteUeNetDevice (void);
  virtual ~cv2x_LteUeNetDevice (void);
   ...
   */
  Ptr<cv2x_LteUeRrc> GetRrc () const;
   ...
   */
  Ptr<cv2x_EpcUeNas> GetNas (void) const;
  
  /**
   * \brief Get the componentn carrier manager.
   * \return the LTE UE component carrier manager
   */
  Ptr<cv2x_LteUeComponentCarrierManager> GetComponentCarrierManager (void) const;
   ...
   * \param enb
   */
  void SetTargetEnb (Ptr<cv2x_LteEnbNetDevice> enb);

  /**
   * \brief Get the target eNB where the UE is registered
   * \return the pointer to the enb
   */
  Ptr<cv2x_LteEnbNetDevice> GetTargetEnb (void);

  /**
   * \brief Set the cv2x_ComponentCarrier Map for the UE
   * \param ccm the map of cv2x_ComponentCarrierUe
   */
  void SetCcMap (std::map< uint8_t, Ptr<cv2x_ComponentCarrierUe> > ccm);

  /**
   * \brief Get the cv2x_ComponentCarrier Map for the UE
   * \returns the map of cv2x_ComponentCarrierUe
   */
  std::map< uint8_t, Ptr<cv2x_ComponentCarrierUe> >  GetCcMap (void);
...
private:
   ...
   * cv2x_LteUeNetDevice from a user perspective, but are actually used also in other
   ...
   * values are in sync with the one in the cv2x_LteUeNetDevice.
   */
  void UpdateConfig ();

  Ptr<cv2x_LteEnbNetDevice> m_targetEnb; ///< target ENB

  Ptr<cv2x_LteUeRrc> m_rrc; ///< the RRC
  Ptr<cv2x_EpcUeNas> m_nas; ///< the NAS
  Ptr<cv2x_LteUeComponentCarrierManager> m_componentCarrierManager; ///< the component carrier manager
  ...
  std::map < uint8_t, Ptr<cv2x_ComponentCarrierUe> > m_ccMap; ///< CC map

}; // end of class cv2x_LteUeNetDevice

} // namespace ns3

#endif /* CV2X_LTE_UE_NET_DEVICE_H */
