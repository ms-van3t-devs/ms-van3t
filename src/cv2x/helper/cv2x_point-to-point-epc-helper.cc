...
NS_LOG_COMPONENT_DEFINE ("cv2x_PointToPointEpcHelper");

NS_OBJECT_ENSURE_REGISTERED (cv2x_PointToPointEpcHelper);


cv2x_PointToPointEpcHelper::cv2x_PointToPointEpcHelper () 
  : m_gtpuUdpPort (2152)  // fixed by the standard
{
  ...
  // create cv2x_EpcSgwPgwApplication
  m_sgwPgwApp = CreateObject<cv2x_EpcSgwPgwApplication> (m_tunDevice, sgwPgwS1uSocket);
  m_sgwPgw->AddApplication (m_sgwPgwApp);
  
  // connect SgwPgwApplication and virtual net device for tunneling
  m_tunDevice->SetSendCallback (MakeCallback (&cv2x_EpcSgwPgwApplication::RecvFromTunDevice, m_sgwPgwApp));


  // Create MME and connect with SGW via S11 interface
  m_mme = CreateObject<cv2x_EpcMme> ();
...
TypeId
cv2x_PointToPointEpcHelper::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_PointToPointEpcHelper")
    .SetParent<cv2x_EpcHelper> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_PointToPointEpcHelper> ()
    .AddAttribute ("S1uLinkDataRate", 
                   "The data rate to be used for the next S1-U link to be created",
                   DataRateValue (DataRate ("10Gb/s")),
                   MakeDataRateAccessor (&cv2x_PointToPointEpcHelper::m_s1uLinkDataRate),
                   MakeDataRateChecker ())
    .AddAttribute ("S1uLinkDelay", 
                   "The delay to be used for the next S1-U link to be created",
                   TimeValue (Seconds (0)),
                   MakeTimeAccessor (&cv2x_PointToPointEpcHelper::m_s1uLinkDelay),
                   MakeTimeChecker ())
    .AddAttribute ("S1uLinkMtu", 
                   ...
                   UintegerValue (2000),
                   MakeUintegerAccessor (&cv2x_PointToPointEpcHelper::m_s1uLinkMtu),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("X2LinkDataRate",
                   "The data rate to be used for the next X2 link to be created",
                   DataRateValue (DataRate ("10Gb/s")),
                   MakeDataRateAccessor (&cv2x_PointToPointEpcHelper::m_x2LinkDataRate),
                   MakeDataRateChecker ())
    .AddAttribute ("X2LinkDelay",
                   "The delay to be used for the next X2 link to be created",
                   TimeValue (Seconds (0)),
                   MakeTimeAccessor (&cv2x_PointToPointEpcHelper::m_x2LinkDelay),
                   MakeTimeChecker ())
    .AddAttribute ("X2LinkMtu",
                   ...
                   UintegerValue (3000),
                   MakeUintegerAccessor (&cv2x_PointToPointEpcHelper::m_x2LinkMtu),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("S1uLinkPcapPrefix",
                   "Prefix for Pcap generated by S1-U link",
                   StringValue ("s1-u"),
                   MakeStringAccessor (&cv2x_PointToPointEpcHelper::m_s1uLinkPcapPrefix),
                   MakeStringChecker ())
    .AddAttribute ("X2LinkPcapPrefix",
                   "Prefix for Pcap generated by X2 link",
                   StringValue ("x2"),
                   MakeStringAccessor (&cv2x_PointToPointEpcHelper::m_x2LinkPcapPrefix),
                   MakeStringChecker ())
    .AddAttribute ("X2LinkEnablePcap",
                   "Enable Pcap for X2 link",
                   BooleanValue (false),
                   MakeBooleanAccessor (&cv2x_PointToPointEpcHelper::m_enablePcapOverX2),
                   MakeBooleanChecker ())
    .AddAttribute ("S1uLinkEnablePcap",
                   "Enable Pcap for X2 link",
                   BooleanValue (false),
                   MakeBooleanAccessor (&cv2x_PointToPointEpcHelper::m_enablePcapOverS1U),
                   MakeBooleanChecker ())
  ;
  return tid;
}
...
TypeId
cv2x_PointToPointEpcHelper::GetInstanceTypeId () const
{
  return GetTypeId ();
}

void
cv2x_PointToPointEpcHelper::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_PointToPointEpcHelper::AddEnb (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint16_t cellId)
{
  ...
  NS_LOG_INFO ("create cv2x_EpcEnbApplication");
  Ptr<cv2x_EpcEnbApplication> enbApp = CreateObject<cv2x_EpcEnbApplication> (enbLteSocket, enbLteSocket6, enbS1uSocket, enbAddress, sgwAddress, cellId);
  enb->AddApplication (enbApp);
  NS_ASSERT (enb->GetNApplications () == 1);
  NS_ASSERT_MSG (enb->GetApplication (0)->GetObject<cv2x_EpcEnbApplication> () != 0, "cannot retrieve cv2x_EpcEnbApplication");
  NS_LOG_LOGIC ("enb: " << enb << ", enb->GetApplication (0): " << enb->GetApplication (0));

  
  NS_LOG_INFO ("Create cv2x_EpcX2 entity");
  Ptr<cv2x_EpcX2> x2 = CreateObject<cv2x_EpcX2> ();
...
void
cv2x_PointToPointEpcHelper::AddX2Interface (Ptr<Node> enb1, Ptr<Node> enb2)
{
  ...
  Ptr<cv2x_EpcX2> enb1X2 = enb1->GetObject<cv2x_EpcX2> ();
  Ptr<cv2x_LteEnbNetDevice> enb1LteDev = enb1->GetDevice (0)->GetObject<cv2x_LteEnbNetDevice> ();
  uint16_t enb1CellId = enb1LteDev->GetCellId ();
  NS_LOG_LOGIC ("cv2x_LteEnbNetDevice #1 = " << enb1LteDev << " - CellId = " << enb1CellId);

  Ptr<cv2x_EpcX2> enb2X2 = enb2->GetObject<cv2x_EpcX2> ();
  Ptr<cv2x_LteEnbNetDevice> enb2LteDev = enb2->GetDevice (0)->GetObject<cv2x_LteEnbNetDevice> ();
  uint16_t enb2CellId = enb2LteDev->GetCellId ();
  NS_LOG_LOGIC ("cv2x_LteEnbNetDevice #2 = " << enb2LteDev << " - CellId = " << enb2CellId);
...
void 
cv2x_PointToPointEpcHelper::AddUe (Ptr<NetDevice> ueDevice, uint64_t imsi)
{
...
uint8_t
cv2x_PointToPointEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, uint64_t imsi, Ptr<cv2x_EpcTft> tft, cv2x_EpsBearer bearer)
{
  ...
  if (ueLteDevice)
    {
      Simulator::ScheduleNow (&cv2x_EpcUeNas::ActivateEpsBearer, ueLteDevice->GetNas (), bearer, tft);
    }
  return bearerId;
}

void
cv2x_PointToPointEpcHelper::ActivateSidelinkBearer (Ptr<NetDevice> ueDevice, Ptr<cv2x_LteSlTft> tft)
{
...
void
cv2x_PointToPointEpcHelper::DeactivateSidelinkBearer (Ptr<NetDevice> ueDevice, Ptr<cv2x_LteSlTft> tft)
{
  NS_LOG_FUNCTION (this << ueDevice);
...
void
cv2x_PointToPointEpcHelper::StartDiscovery (Ptr<NetDevice> ueDevice, std::list<uint32_t> apps, bool rxtx)
{
  NS_LOG_FUNCTION (this << ueDevice);
  ueDevice->GetObject<cv2x_LteUeNetDevice> ()->GetNas ()->AddDiscoveryApps (apps, rxtx);
}

void
cv2x_PointToPointEpcHelper::StopDiscovery (Ptr<NetDevice> ueDevice, std::list<uint32_t> apps, bool rxtx)
{
  NS_LOG_FUNCTION (this << ueDevice);
  ueDevice->GetObject<cv2x_LteUeNetDevice> ()->GetNas ()->RemoveDiscoveryApps (apps, rxtx);
}

Ptr<Node>
cv2x_PointToPointEpcHelper::GetPgwNode ()
{
  return m_sgwPgw;
}

Ipv4InterfaceContainer 
cv2x_PointToPointEpcHelper::AssignUeIpv4Address (NetDeviceContainer ueDevices)
{
  return m_uePgwAddressHelper.Assign (ueDevices);
}

Ipv6InterfaceContainer 
cv2x_PointToPointEpcHelper::AssignUeIpv6Address (NetDeviceContainer ueDevices)
{
...
Ipv4Address
cv2x_PointToPointEpcHelper::GetUeDefaultGatewayAddress ()
{
...
Ipv6Address
cv2x_PointToPointEpcHelper::GetUeDefaultGatewayAddress6 ()
{
  // return the address of the tun device
