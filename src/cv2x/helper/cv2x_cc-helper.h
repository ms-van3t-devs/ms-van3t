...
#ifndef CV2X_CC_HELPER_H
#define CV2X_CC_HELPER_H
 ...
 * \ingroup lte
 *
 * Creation and configuration of Component Carrier entities. One cv2x_CcHelper instance is
 ...
 *
 *
 * This helper it is also used within the cv2x_LteHelper in order to maintain backwards compatibility
 * with previous user simulation script. 
 */
class cv2x_CcHelper : public Object
{
public:
  cv2x_CcHelper (void);
  virtual ~cv2x_CcHelper (void);
  ...
  */
  cv2x_ComponentCarrier DoCreateSingleCc (uint16_t ulBandwidth, uint16_t dlBandwidth, uint32_t ulEarfcn, uint32_t dlEarfcn, bool isPrimary);
  ...
  */
  ...
  /**
  * EquallySpacedCcs() create a valid std::map< uint8_t, Ptr<cv2x_ComponentCarrier> >
  ...
  * Currently it is limited to maximum 2 cv2x_ComponentCarrier
  * Since, only a cv2x_LteEnbPhy object is available just symmetric Carrier Aggregation scheme
  ...
  *
  * \returns std::map< uint8_t, Ptr<cv2x_ComponentCarrier> >
  */

  std::map< uint8_t, cv2x_ComponentCarrier > EquallySpacedCcs ();
...
private:
   ...
   */
  cv2x_ComponentCarrier CreateSingleCc (uint16_t ulBandwidth, uint16_t dlBandwidth, uint32_t ulEarfcn, uint32_t dlEarfcn, bool isPrimary);
