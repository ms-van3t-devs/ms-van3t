...
#include <ns3/cv2x_emu-epc-helper.h>
#include <ns3/log.h>
...
NS_LOG_COMPONENT_DEFINE ("cv2x_EmuEpcHelper");

NS_OBJECT_ENSURE_REGISTERED (cv2x_EmuEpcHelper);


cv2x_EmuEpcHelper::cv2x_EmuEpcHelper () 
...
TypeId
cv2x_EmuEpcHelper::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_EmuEpcHelper")
    .SetParent<cv2x_EpcHelper> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_EmuEpcHelper> ()
    .AddAttribute ("sgwDeviceName", 
                   "The name of the device used for the S1-U interface of the SGW",
                   StringValue ("veth0"),
                   MakeStringAccessor (&cv2x_EmuEpcHelper::m_sgwDeviceName),
                   MakeStringChecker ())
    .AddAttribute ("enbDeviceName", 
                   "The name of the device used for the S1-U interface of the eNB",
                   StringValue ("veth1"),
                   MakeStringAccessor (&cv2x_EmuEpcHelper::m_enbDeviceName),
                   MakeStringChecker ())
    .AddAttribute ("SgwMacAddress", 
                   "MAC address used for the SGW ",
                   StringValue ("00:00:00:59:00:aa"),
                   MakeStringAccessor (&cv2x_EmuEpcHelper::m_sgwMacAddress),
                   MakeStringChecker ())
    .AddAttribute ("EnbMacAddressBase", 
                   "First 5 bytes of the Enb MAC address base",
                   StringValue ("00:00:00:eb:00"),
                   MakeStringAccessor (&cv2x_EmuEpcHelper::m_enbMacAddressBase),
                   MakeStringChecker ())
    ;
  return tid;
}
...
TypeId
cv2x_EmuEpcHelper::GetInstanceTypeId () const
{
  return GetTypeId ();
}

void
cv2x_EmuEpcHelper::DoInitialize ()
{
  NS_LOG_LOGIC (this);   
  ...
  cv2x_EpcHelper::DoInitialize ();
}

void
cv2x_EmuEpcHelper::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_EmuEpcHelper::AddEnb (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint16_t cellId)
{
...
void
cv2x_EmuEpcHelper::AddX2Interface (Ptr<Node> enb1, Ptr<Node> enb2)
{
...
void 
cv2x_EmuEpcHelper::AddUe (Ptr<NetDevice> ueDevice, uint64_t imsi)
{
...
uint8_t
cv2x_EmuEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, uint64_t imsi, Ptr<cv2x_EpcTft> tft, cv2x_EpsBearer bearer)
{
...
void
cv2x_EmuEpcHelper::ActivateSidelinkBearer (Ptr<NetDevice> ueDevice, Ptr<cv2x_LteSlTft> tft)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_EmuEpcHelper::DeactivateSidelinkBearer (Ptr<NetDevice> ueDevice, Ptr<cv2x_LteSlTft> tft)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_EmuEpcHelper::StartDiscovery (Ptr<NetDevice> ueDevice, std::list<uint32_t> apps, bool rxtx)
{
  NS_LOG_FUNCTION (this);
...
void
cv2x_EmuEpcHelper::StopDiscovery (Ptr<NetDevice> ueDevice, std::list<uint32_t> apps, bool rxtx)
{
  NS_LOG_FUNCTION (this);
...
cv2x_EmuEpcHelper::AssignUeIpv4Address (NetDeviceContainer ueDevices)
{
  return m_uePgwAddressHelper.Assign (ueDevices);
}

Ipv6InterfaceContainer 
cv2x_EmuEpcHelper::AssignUeIpv6Address (NetDeviceContainer ueDevices)
{
...
Ipv4Address
cv2x_EmuEpcHelper::GetUeDefaultGatewayAddress ()
{
...
Ipv6Address
cv2x_EmuEpcHelper::GetUeDefaultGatewayAddress6 ()
{
