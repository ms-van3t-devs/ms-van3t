...
#include "cv2x_phy-stats-calculator.h"
...
NS_LOG_COMPONENT_DEFINE ("cv2x_PhyStatsCalculator");

NS_OBJECT_ENSURE_REGISTERED (cv2x_PhyStatsCalculator);

cv2x_PhyStatsCalculator::cv2x_PhyStatsCalculator ()
...
TypeId
cv2x_PhyStatsCalculator::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::cv2x_PhyStatsCalculator")
    .SetParent<cv2x_LteStatsCalculator> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_PhyStatsCalculator> ()
    .AddAttribute ("DlRsrpSinrFilename",
                   ...
                   MakeStringAccessor (&cv2x_PhyStatsCalculator::SetCurrentCellRsrpSinrFilename),
                   MakeStringChecker ())
    .AddAttribute ("UlSinrFilename",
                   "Name of the file where the UE SINR statistics will be saved.",
                   StringValue ("UlSinrStats.txt"),
                   MakeStringAccessor (&cv2x_PhyStatsCalculator::SetUeSinrFilename),
                   MakeStringChecker ())
    .AddAttribute ("UlInterferenceFilename",
                   ...
                   MakeStringAccessor (&cv2x_PhyStatsCalculator::SetInterferenceFilename),
                   MakeStringChecker ())
  ;
  return tid;
}
...
void
cv2x_PhyStatsCalculator::SetCurrentCellRsrpSinrFilename (std::string filename)
{
  m_RsrpSinrFilename = filename;
}

std::string
cv2x_PhyStatsCalculator::GetCurrentCellRsrpSinrFilename (void)
{
  return m_RsrpSinrFilename;
}

void
cv2x_PhyStatsCalculator::SetUeSinrFilename (std::string filename)
{
  m_ueSinrFilename = filename;
}

std::string
cv2x_PhyStatsCalculator::GetUeSinrFilename (void)
{
  return m_ueSinrFilename;
}

void
cv2x_PhyStatsCalculator::SetInterferenceFilename (std::string filename)
{
  m_interferenceFilename = filename;
}

std::string
cv2x_PhyStatsCalculator::GetInterferenceFilename (void)
{
...
void
cv2x_PhyStatsCalculator::ReportCurrentCellRsrpSinr (uint16_t cellId, uint64_t imsi, uint16_t rnti,
...
void
cv2x_PhyStatsCalculator::ReportUeSinr (uint16_t cellId, uint64_t imsi, uint16_t rnti,  double sinrLinear, uint8_t componentCarrierId)
{
...
void
cv2x_PhyStatsCalculator::ReportInterference (uint16_t cellId, Ptr<SpectrumValue> interference)
{
...
void
cv2x_PhyStatsCalculator::ReportCurrentCellRsrpSinrCallback (Ptr<cv2x_PhyStatsCalculator> phyStats,
...
void
cv2x_PhyStatsCalculator::ReportUeSinr (Ptr<cv2x_PhyStatsCalculator> phyStats, std::string path,
                                  uint16_t cellId, uint16_t rnti, double sinrLinear, uint8_t componentCarrierId)
{
  ...
  std::string pathEnbMac  = path.substr (0, path.find ("/cv2x_ComponentCarrierMap"));
  pathEnbMac += "/cv2x_LteEnbMac/DlScheduling";
...
void
cv2x_PhyStatsCalculator::ReportInterference (Ptr<cv2x_PhyStatsCalculator> phyStats, std::string path,
