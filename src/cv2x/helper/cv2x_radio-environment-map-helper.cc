...
#include "cv2x_radio-environment-map-helper.h"
...
NS_LOG_COMPONENT_DEFINE ("cv2x_RadioEnvironmentMapHelper");

NS_OBJECT_ENSURE_REGISTERED (cv2x_RadioEnvironmentMapHelper);

cv2x_RadioEnvironmentMapHelper::cv2x_RadioEnvironmentMapHelper ()
{
}
...
void
cv2x_RadioEnvironmentMapHelper::DoDispose ()
{
  NS_LOG_FUNCTION (this);
}

TypeId
cv2x_RadioEnvironmentMapHelper::GetTypeId (void)
{
  NS_LOG_FUNCTION ("cv2x_RadioEnvironmentMapHelper::GetTypeId");
  static TypeId tid = TypeId ("ns3::cv2x_RadioEnvironmentMapHelper")
    .SetParent<Object> ()
    .SetGroupName("Lte")
    .AddConstructor<cv2x_RadioEnvironmentMapHelper> ()
    .AddAttribute ("ChannelPath", "The path to the channel for which the Radio Environment Map is to be generated",
                   ...
                   MakeStringAccessor (&cv2x_RadioEnvironmentMapHelper::m_channelPath),
                   MakeStringChecker ())
    .AddAttribute ("OutputFile", "the filename to which the Radio Environment Map is saved",
                   StringValue ("rem.out"),
                   MakeStringAccessor (&cv2x_RadioEnvironmentMapHelper::m_outputFile),
                   MakeStringChecker ())
    .AddAttribute ("XMin", "The min x coordinate of the map.",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&cv2x_RadioEnvironmentMapHelper::m_xMin),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("YMin", "The min y coordinate of the map.",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&cv2x_RadioEnvironmentMapHelper::m_yMin),
                   MakeDoubleChecker<double> ())
   .AddAttribute ("XMax", "The max x coordinate of the map.",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&cv2x_RadioEnvironmentMapHelper::m_xMax),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("YMax", "The max y coordinate of the map.",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&cv2x_RadioEnvironmentMapHelper::m_yMax),
                   MakeDoubleChecker<double> ())
   .AddAttribute ("XRes", "The resolution (number of points) of the map along the x axis.",
                   UintegerValue (100),
                   MakeUintegerAccessor (&cv2x_RadioEnvironmentMapHelper::m_xRes),
    ...
    .AddAttribute ("YRes", "The resolution (number of points) of the map along the y axis.",
                   UintegerValue (100),
                   MakeUintegerAccessor (&cv2x_RadioEnvironmentMapHelper::m_yRes),
    ...
    .AddAttribute ("Z", "The value of the z coordinate for which the map is to be generated",
		   DoubleValue (0.0),
                   MakeDoubleAccessor (&cv2x_RadioEnvironmentMapHelper::m_z),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("StopWhenDone", "If true, Simulator::Stop () will be called as soon as the REM has been generated",
		   BooleanValue (true),
                   MakeBooleanAccessor (&cv2x_RadioEnvironmentMapHelper::m_stopWhenDone),
                   MakeBooleanChecker ())
    .AddAttribute ("NoisePower",
                   ...
                   MakeDoubleAccessor (&cv2x_RadioEnvironmentMapHelper::m_noisePower),
                   MakeDoubleChecker<double> ())
...
uint8_t 
cv2x_RadioEnvironmentMapHelper::GetBandwidth () const
{
  return m_bandwidth;
}

void 
cv2x_RadioEnvironmentMapHelper::SetBandwidth (uint8_t bw)
{
  switch (bw)
    { 
    case 6:
...
void 
cv2x_RadioEnvironmentMapHelper::Install ()
{
  NS_LOG_FUNCTION (this);
  if (!m_rem.empty ())
    {
      NS_FATAL_ERROR ("only one REM supported per instance of cv2x_RadioEnvironmentMapHelper");
    }
  ...
  Simulator::Schedule (Seconds (startDelay),
                       &cv2x_RadioEnvironmentMapHelper::DelayedInstall,
                       this);
}
...
cv2x_RadioEnvironmentMapHelper::DelayedInstall ()
{
  ...
  for (uint32_t i = 0; i < m_maxPointsPerIteration; ++i)
    {
      RemPoint p;
      p.phy = CreateObject<cv2x_RemSpectrumPhy> ();
      ...
      p.phy->SetRxSpectrumModel (cv2x_LteSpectrumValueHelper::GetSpectrumModel (m_earfcn, m_bandwidth));
          ...
          if ((numPointsCurrentIteration == m_maxPointsPerIteration)
              || ((x > m_xMax - 0.5*m_xStep) && (y > m_yMax - 0.5*m_yStep)) )
            {
              Simulator::Schedule (Seconds (remIterationStartTime), 
                                   &cv2x_RadioEnvironmentMapHelper::RunOneIteration,
  ...
  Simulator::Schedule (Seconds (remIterationStartTime), 
                       &cv2x_RadioEnvironmentMapHelper::Finalize,
                       this);
}

  
void 
cv2x_RadioEnvironmentMapHelper::RunOneIteration (double xMin, double xMax, double yMin, double yMax)
{
  ...
  if (remIt != m_rem.end ())
    {
      ...
      NS_LOG_LOGIC ("deactivating cv2x_RemSpectrumPhys that are unneeded in the last iteration");
  ...
  Simulator::Schedule (Seconds (0.0005), &cv2x_RadioEnvironmentMapHelper::PrintAndReset, this);  
}

void 
cv2x_RadioEnvironmentMapHelper::PrintAndReset ()
{
  NS_LOG_FUNCTION (this);
