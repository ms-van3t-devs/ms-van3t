...
#include "ns3/nr_cni-urbanmicrocell-propagation-loss-model.h"
...
static void
InitCV2XUrban (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (nr_CniUrbanmicrocellPropagationLossModel::GetTypeId());
  channelConditionModelFactory->SetTypeId (ThreeGppV2vHighwayChannelConditionModel::GetTypeId ());
}
...
NrHelper::InitializeOperationBand (OperationBandInfo *band, uint8_t flags)
{
  ...
  static std::unordered_map<BandwidthPartInfo::Scenario, InitPathLossFn, std::hash<int>> initLookupTable
  {
    ...
    {BandwidthPartInfo::CV2X_UrbanMicrocell, std::bind (&InitCV2XUrban, std::placeholders::_1, std::placeholders::_2)},
  };
      ...
      for (const auto & bwp : cc->m_bwp)
        {
          ...
          if(bwp->m_scenario != BandwidthPartInfo::CV2X_UrbanMicrocell)
            {
              ...
              //PATCH
              bwp->m_propagation = m_pathlossModelFactory.Create <PropagationLossModel> ();
...
NrHelper::DoAssignStreamsToChannelObjects (Ptr<NrSpectrumPhy> phy, int64_t currentStream)
{
  ...
  //PATCH
  Ptr<ThreeGppPropagationLossModel> propagationLossModel = StaticCast<ThreeGppPropagationLossModel> (phy->GetSpectrumChannel ()->GetPropagationLossModel ());
