...
LteSpectrumPhy::EndRxData ()
{
  ...
  for (it = m_sinrPerceived.ConstValuesBegin (); it != m_sinrPerceived.ConstValuesEnd (); it++)
    {
      sum += (*it);
      rbNum++;
    }
      ...
      for (std::list<Ptr<Packet> >::const_iterator j = (*i)->Begin (); j != (*i)->End (); ++j)
        {
          ...
          if (LteUePhy::s_last_rsrp != LteUePhy::s_sentinel_value)
            {
              RsrpTag rsrp_t;
              rsrp_t.Set(LteUePhy::s_last_rsrp);
              (*j)->AddPacketTag (rsrp_t);
            }

          if (LteUePhy::s_last_rssi != LteUePhy::s_sentinel_value)
            {
              RssiTag rssi_t;
              rssi_t.Set(LteUePhy::s_last_rssi);
              (*j)->AddPacketTag (rssi_t);
            }
